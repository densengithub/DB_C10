		...
		if (len)
		{
			while (len--)
			{
				...
				case GET_CHECKSUM:
					...
					if (CRC8(sendstr, framedatalength + 6 - 2) == getstr[count])
					{
						sendstr[sendPos++] = getstr[count];
						gState = GET_END;
					}
				...
				case GET_END:
					...
					if (isCmdCompleted)
					{
						...
						// 	printf("sendstr[%d] = 0x%02x\n", i, sendstr[i]);
						CheckReceiveFrameHandle(sendstr, sendPos);
						isCmdCompleted = false;
					}
					gState = STATUS_INIT;
					break;
...
int CreateAppComunicationTask(void)
{
	...
	if (res)
	{
		printf("ERROR:create UartttlComunicationTask() thread fail! res=%ld\n", res);
		return 1;
	}
