...
void Layer_TestMode_TaskLog(ExternalEvent_layer *ev_testmode)
{
...
void Layer_TestMode_Task(ExternalEvent_layer *ev_testmode)
{
    ...
    Layer_TestMode_TaskLog(ev_testmode);
    if (testmode & 0x4000)
    {
        module_test = true;
        board_test = true;
...
void Layer_MainMode_TaskLog(void)
{
    int i = 0;
...
void Layer_MainMode_Task(ExternalEvent_layer *ev_testmode)
{
    ...
    if (!charge_status)
    {
        if (!upgrade_status)
        {
            ...
            status_menu = status_temp1 & (CAR_PARKING | CAR_RUN);
        ...
        {
            if(status_temp1 & CAR_VCU_UPDATE)
            {
                UpgradeLayer_Precent_Display(ev_layer2.layer_displaydata.ScooterInfo.car_speed);
                UpgradeLayer_Remaintime_Display(5-(ev_layer2.layer_displaydata.ScooterInfo.car_speed/20));
            }
        }        
    }
    else
    ...
    {
    case CAR_GPS_NO:
        type_action = (int)STATUS_SIGNAL_LEVEL1;//无信号：信号格无显示，网络图标闪烁
        break;
    case CAR_GPS_WEAK:
        type_action = (int)STATUS_SIGNAL_LEVEL2;//弱信号：信号格闪烁，网络图标常亮
        break;
    case CAR_GPS_STRONG:
        type_action = (int)STATUS_SIGNAL_LEVEL3;//强信号：信号格+网络图标常亮
        break;
    case CAR_GPS_NULL:
        type_action = (int)STATUS_SIGNAL_LEVEL0;//未联网
        break;
    default:
        type_action = 0;
        break;
