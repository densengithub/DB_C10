...
bool Layer_time_Init(void)
{
	...
	if(!timeTitleContainer)
    {
		...
		Layer_Sprite_Init(timeWeatherSprite, 0, 0);				
		...
		Layer_Sprite_Init(timeScoErrSprite, 0, 1);
		ituWidgetSetVisible(timeScoErrSprite, false);

		Layer_Sprite_Init(timeModeSprite, 0, 1);
...
bool Layer_bottom_Init(void)
{
    if(!bottomBatContainer)
    {
		...
		Layer_Sprite_Init(bottomBatTagSprite, 0, 0);
		...
		Layer_Sprite_Init(bottomBatSprite, 0, 0);		
...
bool Layer_recoveryprogress_Init(void)
{
    if(!progressRecoveryPowerContainer)
    {
		...
		Layer_Sprite_Init(progressRecoveryPowerSprite, 0, 0);
...
bool Layer_dynamicprogress_Init(void)
{
    if(!dynamicprogressContainer)
    {
		...
		Layer_Sprite_Init(progressDynamicPowerSprite, 0, 0);
		Layer_Sprite_Init(progressDynamicSpeedSprite, 0, 0);
...
bool Layer_esaveprogress_Init(void)
{
    if(!esaveprogressContainer)
    {
		...
		Layer_Sprite_Init(progressEsavePowerSprite, 0, 0);
		Layer_Sprite_Init(progressEsaveSpeedSprite, 0, 0);
...
bool Layer_sportprogress_Init(void)
{
    if(!sportprogressContainer)
    {
		...
		Layer_Sprite_Init(progressSportPowerSprite, 0, 0);
		Layer_Sprite_Init(progressSportSpeedSprite, 0, 0);
...
{	
	...
	tempx_test_curve++;
	tempx_test_curve_s++;
	if (tempx_test_curve>20)
	{
		tempx_test_curve = 0;
		speedCurveDraw((tempx_test_curve_s&0x01)?(20):(140));
	}
#if 0
	if (tempx_test>5)
	{
		tempx_test = 0;
		index_weather_test++;
		ituSpriteGoto(timeWeatherSprite, index_weather_test%(timeWeatherSprite->childCount));
		// ituSpriteGoto(timeModeSprite, index_weather_test % (timeModeSprite->childCount));
		// ituSpriteGoto(bottomBatSprite, index_weather_test % (bottomBatSprite->childCount));
		mainLayer_Battery_DisplayConfig(index_weather_test % 101, index_weather_test % 101, 1);
		mainLayer_BatteryCapacity_DisplayConfig(index_weather_test % 101);
		ituWidgetSetAlpha(sp_GSMSignalIcon[index_weather_test % 4],(index_weather_test / 4) & 0x01?255:102);
		ituWidgetSetAlpha(sp_GPSSignalIcon[index_weather_test % 4], (index_weather_test / 4) & 0x01 ? 255 : 102);
		// ituWidgetSetVisible(sp_GSMSignalIcon[index_weather_test % 4], (index_weather_test / 4) & 0x01);
		// ituWidgetSetVisible(sp_GPSSignalIcon[index_weather_test % 4], (index_weather_test / 4) & 0x01);
		
#if 0
		index_ground_test++;
		index_ground_test %= 5;
		temp_f = (1.0 + index_ground_test / 10.0);
		pw_f = 760 * temp_f;
		ph_f = 521 * temp_f;
        // px_f = index_ground_test * 80;
        ituWidgetSetDimension(mainDynamicBackground, (int)pw_f, (int)ph_f);
		...
		py_f = (int)temp_f;
		py_f >>= 1;
		ituWidgetSetPosition(mainDynamicBackground, px_f, py_f);
        // printf("px_f=%d,py_f=%d.\t\n", px_f,py_f);
#endif
		mainLayer_Total_DisplayConfig(index_weather_test, STATUS_METRIC_UNIT);

		mainLayer_SpeedValue_DisplayConfig((index_weather_test % 101), STATUS_METRIC_UNIT);
		mainLayer_RealtimeEnergy_DisplayConfig((index_weather_test % 40) * 10);

		// speedCurveDraw(rand() % 140 + 5);
		// mainLayer_LastTripTime_DisplayConfig(index_weather_test + 3600);
		// mainLayer_RealtimeEnergy_DisplayConfig((100 % 40) * 10 * (index_weather_test&0x01?1:(-1)));
        return false;		
	}
#endif
...
bool mainlayer_RealtimeEnergy_DisplayConfig_Beta1(int dis_realtime_energy,int dis_rtenergy)
{
	char buffer[10] = {0};
	int length = 0;
...
bool mainlayer_RealtimeEnergy_DisplayConfig_Beta2(int dis_realtime_energy,int dis_rtenergy)
{
	char buffer[10] = {0};
	int length = 0;
	int temp_realtime_energy = dis_realtime_energy / 10;
	...
	// if ((dis_rtenergy > 0) && (goto_spriteindex > 0))
	// {
	// 	ituSpriteGoto(progressRecoveryPowerSprite, goto_spriteindex - 1);
	// 	ituWidgetSetVisible(Current_powerSprite, false);
	// 	ituWidgetSetVisible(progressRecoveryPowerContainer, true);
...
bool mainLayer_RealtimeEnergy_DisplayConfig(int dis_realtime_energy,int dis_rtenergy)
{
	...
	mainlayer_RealtimeEnergy_DisplayConfig_Beta1(dis_realtime_energy,dis_rtenergy);

	#else	

	mainlayer_RealtimeEnergy_DisplayConfig_Beta2(dis_realtime_energy,dis_rtenergy);
...
bool mainLayer_NetworksSignal_DisplayConfig_Beta1(user_indicationlist_type dis_signallist,user_indicationaction_type dis_action)
{
	int i = 0;
	int j = 0;
...
void layer_Signal_Display_Beta1(void)
{	
...
bool mainLayer_NetworksSignal_DisplayConfig_Beta2(user_indicationlist_type dis_signallist,user_indicationaction_type dis_action)
{
	int j = 0;
...
void layer_Signal_Display_Beta2(void)
{	
...
{
	#if 0
	mainLayer_NetworksSignal_DisplayConfig_Beta1(dis_signallist,dis_action);
	#else
	mainLayer_NetworksSignal_DisplayConfig_Beta2(dis_signallist,dis_action);
	#endif
}
...
{
	#if 0
	layer_Signal_Display_Beta1();
	#else
	layer_Signal_Display_Beta2();
	#endif
}
