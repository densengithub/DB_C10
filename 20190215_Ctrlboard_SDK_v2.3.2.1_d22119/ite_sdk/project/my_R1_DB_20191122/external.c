		...
		//  sem_wait(&ExternaltaskSem);
		 ...
		 if (ret)
		 {
			 ...
			 {				
				...
				//  niu_data_read_value_loc( NIU_ID_DB_LOC_STATUS, (ev_layer2.layer_displaydata.DisplayDatabuffer), sizeof(ScooterInfo_Typedef));
				//  for (i = 0; i < (NIU_ID_DB_LOC_MAX - NIU_ID_DB_LOC_STATUS); i++)
				//  {
				// 	 num = niu_data_get_datalen_loc(NIU_ID_DB_LOC_STATUS + i);
				// 	 // printf("rd_id + i=%d,num=%d....\n", NIU_ID_DB_LOC_STATUS + i, num);
				// 	 niu_data_read_value_loc(NIU_ID_DB_LOC_STATUS + i, (ev_layer2.layer_displaydata.DisplayDatabuffer + j), num);
				// 	 j += num;
				//  }
				#if 0
				if (SceneReadReady())
				{
					...
					{
						memcpy(ev_layer2.layer_displaydata.DisplayDatabuffer, niu_data_get_pdata_loc( NIU_ID_DB_LOC_STATUS), sizeof(NIU_DATA_BODY_DB_LOC));
...
void ExternalInit(void)
{
	...
	if(extOutQueue == -1)
	{
		printf("Create external_out fail!.......\n");
	}
	extQuit = false;
	...
	if (res)
	{
		printf("ERROR, create ExternalCheckTask() thread fail! res=%ld\n", res);
	}
   return;
}
