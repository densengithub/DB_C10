 ...
 */
NBOOL niu_eqp_id_is_vaild_loc(NUINT32 id)
{
	if(id < NIU_ID_DB_LOC_MAX)
	{
		return NTRUE;
	}	
	return NFALSE;		
 ...
 * @return 返回字段类型
 */
NIU_DATA_TYPE niu_data_get_type_loc(NUINT32 id)
{
    if(niu_eqp_id_is_vaild_loc(id) == NFALSE)
    {
        ITE_TRACE("niu_data_get_type,niu_eqp_id_is_vaild(%d).\n", id);
        return NIU_NULL;
	}
 ...
 * @return 返回长度
 */
NUINT32 niu_data_get_datalen_loc(NUINT32 id)
{
    if(niu_eqp_id_is_vaild_loc(id) == NFALSE)
    {
        ITE_TRACE("niu_data_get_datalen,niu_eqp_id_is_vaild(%d).\n", id);
        return 0;
	}
 ...
 * @return 返回 整形数值
 */
NUINT32 niu_data_read_data_loc(NUINT32 id)
{
    NVOID* temp = NNULL;
    NUINT32 result = 0;
    ...
    temp = niu_data_get_pdata_loc(id);
	TAKE_WRITE_KEY_LOC();
    if (temp)
    {
        switch (niu_data_get_type_loc(id))
        {
            case NIU_INT8:
                result = *((NINT8 *)temp);
                break;
 ...
 * @return 返回已写入长度
 */
NUINT32 niu_data_write_data_loc(NUINT32 id, NVOID* data)
{
    NVOID* temp = NNULL;
    NUINT32 cpylen = 0;
    ...
    temp = niu_data_get_pdata_loc(id);
    cpylen = niu_data_get_datalen_loc(id);
	TAKE_WRITE_KEY_LOC();
    if (temp)
    {
        //cpylen = (len > cpylen) ? cpylen : len;
...
NBOOL niu_data_clean_value_loc(NUINT32 id)
{
    NBOOL ret = NFALSE;
    NUINT8 *value = NNULL;
    NUINT32 len = 0;
    ...
    if(niu_eqp_id_is_vaild_loc(id) == NFALSE)
    {
        ITE_TRACE("niu_data_clean_value,niu_eqp_id_is_vaild(%d).\n", id);
        return NFALSE;
	}	
 ...
 * @return 返回 读取缓存指针地址
 */
NVOID* niu_data_read_value_loc(NUINT32 id, NVOID* data, NUINT32 len)
{
    NVOID* temp = NNULL;
    NUINT32 cpylen = 0;
 ...
 * @return 返回void*指针
 */
NVOID* niu_data_get_pdata_loc(NUINT32 id)
{
    if(niu_eqp_id_is_vaild_loc(id) == NFALSE)
    {
        ITE_TRACE("niu_data_get_pdata,niu_eqp_id_is_vaild(%d).\n", id);
        return NNULL;
	}	
				
    return (niu_data_value_db_ite[id].addr);
}
...
{		
	...
	niu_data_write_data_loc(NIU_ID_DB_LOC_DB_CMD,&uint16_config);
	uint16_config = (unsigned short)0x0000;
	niu_data_write_data_loc(NIU_ID_DB_LOC_99_TIMEOUT,&uint16_config);		
	niu_data_write_data_loc(NIU_ID_DB_LOC_MODULE_VER, APP_VERSION);
}
...
NVOID niu_create_respone_loc(NUINT8 start_id,NUINT8 id_cnt,NVOID* dst_str)
{
	...
	local_data = niu_data_get_pdata_loc(start_id);
	id_data_len = niu_data_get_datalen_loc(start_id);
	...
	if(status_temp==0)
	{
		...
		{
			local_data = niu_data_get_pdata_loc(start_id+j);
			id_data_len = niu_data_get_datalen_loc(start_id+j);
