...
static void RS485Callback(void* arg1, uint32_t arg2)
{
	...
	len = read(ITP_DEVICE_RS485_1,getstr1+totalcount,256);
	totalcount += len;
	
	if(totalcount >= RS485CommandLen)
	{	  
		write(ITP_DEVICE_RS485_1,sendtr1,8);
		totalcount =0;
...
void* TestFunc(void* arg)
{
    ...
    ioctl(ITP_DEVICE_RS485_1, ITP_IOCTL_ON, (void*)&RS485_port);
	ioctl(ITP_DEVICE_RS485_1, ITP_IOCTL_RESET, (void*)&RS485_UartParity);
	ioctl(ITP_DEVICE_RS485_1, ITP_IOCTL_REG_RS485_CB, (void*)RS485Callback);
	//ioctl(ITP_DEVICE_RS485_1, ITP_IOCTL_REG_RS485_DEFER_CB , (void*)RS485Callback);
	...
	{	
		...
		sem_wait(&RS485Sem);
      	len = read(ITP_DEVICE_RS485_1,getstr1+count,256);
		count += len;
	
		if(count >= RS485CommandLen)
		{
			printf("rs485 read: %s,count=%d\n",getstr1,count);
			sprintf(sendtr,"%s",getstr1);
			write(ITP_DEVICE_RS485_1,sendtr,256);
			count =0;
			memset(getstr1, 0, 256);
		}
	}
