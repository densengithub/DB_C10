    ...
    int lcd_index)
{
...
#endif
    {
    	if(itv_rcs.curr_video->src_w == 640)
    	{
        	out_info.width    = 800;
        	out_info.height   = 480;
    	}
	...
	mmpIspSetMode(gIspDev1, MMP_ISP_MODE_PLAY_VIDEO);
#endif
#endif
	if(itv_rcs.curr_video->src_w == 640)
	{
	...
	{
    	mmpIspSetOutputWindow(gIspDev1, &out_info);
		mmpIspSetVideoWindow(gIspDev1, 0, 0, out_info.width, out_info.height);
    	mmpIspPlayImageProcess(gIspDev1, &isp_share);
    	SHORT_WAIT_UNTIL(mmpIspIsEngineIdle(gIspDev1));
	}
...
static void itv_change_display_format()
{
#ifdef CFG_LCD_ENABLE
    {
		...
		mmpIspSetOutputFormat(gIspDev1, MMP_ISP_OUT_DITHER565A);
    ...
    uint32_t height)
{
	...
	mmpIspSetVideoWindow(gIspDev1, startX, startY, width, height);
}
