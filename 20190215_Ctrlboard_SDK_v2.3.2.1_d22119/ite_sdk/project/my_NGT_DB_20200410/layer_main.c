...
volatile bool sidestand_temple_status = false;
int delay_count = 0;
...
{
	bool result = false;
	result |= layer_SideStand_Display();
	result |= layer_Signal_Display();
    return result;
}
...
bool mainLayer_RemainMileage_DisplayConfig(int dis_range,user_unitlist_type dis_unit)
{
	...
	if (dis_unit == STATUS_IMPERIAL_UNIT)
	{
		...
		range_dis += 50 ;
		range_dis /= 100 ;
		// range_dis = (int)((float)range_dis * 0.62137 + 0.5);
	}
...
bool layer_Signal_Display(void)
{
	bool ret = false;
...
bool layer_SideStand_Display(void)
{
	bool ret = false;
...
{
	static bool mtkSta_Bp = false;
	if((mtkSta_Bp!=status_mistake)&&(!mtkSta_Bp))
	{
		if(!sidestand_temple_status)
		sidestand_temple_status = true;
	}
	mtkSta_Bp = status_mistake;
	return true;
}
...
bool mainLayer_Sidestand_DisplayConfig(bool status_ready,bool status_sidestand)
{    
    // if(!standsideIcon)
	// {
	// 	return false;
