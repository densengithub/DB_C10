...
#define CUSTOM_COUNT 0

static ITUScene scene;

static ITUTableListBox* tlistbox1;
static ITUTableListBox* tlistbox2;
static ITUTableIconListBox* tilistbox1;
...
static bool OnLoad(ITUWidget* widget, char* param)
{
    ...
    {
		if ((CUSTOM_COUNT > 0) && (listbox->itemCount == 0))
		{
			count = CUSTOM_COUNT;
	...
	if (CUSTOM_COUNT > 0)
	    ...
	    count = ituTableListBoxGetItemCount(tlistbox);
...
static bool OnLoadIcon(ITUWidget* widget, char* param)
{
    ITCTree* node;
    ITUListBox* listbox = (ITUListBox*)widget;
...
static bool OnSelect(ITUWidget* widget, char* param)
{
    ...
    if (checkedIndex >= 0)
        ituTableIconListBoxSetIconIndex(tilistbox1, checkedIndex, 0);

    checkedIndex = listbox->focusIndex;

    if (checkedIndex >= 0)
        ituTableIconListBoxSetIconIndex(tilistbox1, checkedIndex, 1);

    return true;
}

static bool OnMouseLongPress(ITUWidget* widget, char* param)
{
    ITUListBox* listbox = (ITUListBox*)widget;
    ITUText* item = (ITUText*) ituListBoxGetFocusItem(listbox);

    printf("%s mouse long press.\n", item->string);
    ituListBoxSelect((ITUListBox*)tlistbox1, listbox->focusIndex);
    ituListBoxSelect((ITUListBox*)tlistbox2, listbox->focusIndex);
    ituListBoxSelect((ITUListBox*)tilistbox1, listbox->focusIndex);

    return true;
}

static void UnSelect(int arg)
{
    ituListBoxSelect((ITUListBox*)tlistbox1, -1);
    ituListBoxSelect((ITUListBox*)tlistbox2, -1);
    ituListBoxSelect((ITUListBox*)tilistbox1, -1);

    if (checkedIndex >= 0)
        ituTableIconListBoxSetIconIndex(tilistbox1, checkedIndex, 2);
}

static bool OnMouseUp(ITUWidget* widget, char* param)
{
    ITUListBox* listbox = (ITUListBox*)widget;
    ITUText* item = (ITUText*) ituListBoxGetFocusItem(listbox);
    ...
    ituSceneExecuteCommand(&scene, 6, UnSelect, 0);
...
int SDL_main(int argc, char *argv[])
{
    ...
    window = SDL_CreateWindow("ITU tablelistbox2 Test",
    ...
    ituSceneLoadFile(&scene, CFG_PRIVATE_DRIVE ":/tablelistbox2.itu");
    ...
    tlistbox1 = (ITUTableListBox*)ituSceneFindWidget(&scene, "TableListBox1");
    tlistbox2 = (ITUTableListBox*)ituSceneFindWidget(&scene, "TableListBox2");
    tilistbox1 = (ITUTableIconListBox*)ituSceneFindWidget(&scene, "TableIconListBox1");
