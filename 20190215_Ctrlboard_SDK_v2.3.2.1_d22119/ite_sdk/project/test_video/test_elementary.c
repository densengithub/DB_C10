               ...
               AVPacket       *avpkt)
{
    ...
    while (!bEOS)
    {
        ...
        while (avpkt->size > 0)
        {
            ...
            printf("[%s] decode = %u ms, frame %d, got_picture = %d\n", __FUNCTION__, PalGetDuration(dec_clock), c->frame_number, got_picture);
