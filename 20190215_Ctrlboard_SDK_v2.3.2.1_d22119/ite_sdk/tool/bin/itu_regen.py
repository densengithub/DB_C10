...
def partList(ts, gs, p, r):
    ...
    for index in range(len(ts)):
        if (size >= lIndex) and (size < rIndex):
            ...
            print('{0: >11}  {1: <}'.format(sz, ts[index][14:-4] + ".xml"))
            result.append(f)
...
outputPath = workPath[0:-8] + "project\\"
ratio      = "1/1"
...
try:
    ...
    for i in range(len(ts)):
        f = ts[i][0:-4]
        print('{0: <11}  {1: <}'.format("[ts]", ts[i][14:-4]))

    for i in range(len(ts_itu)):
        f = ts_itu[i][0:-4]
        print('{0: <11}  {1: <}'.format("[ts_itu]", ts_itu[i][14:-4]))
    ...
    for i in range(len(ts)):
        if os.path.exists(ts[i]):
            ...
            print("{0:>11,}/{1:<} [{2:>7,.2%}]: {3:<}".format(i, c, percent, f))
            writeIni(ts[i])
