...
{
    sReportPhysicalAddressTransfer,		// 0
    sReportPhysicalAddressReceived,		// 1
    sPollingMessage,						// 2
    sPollingResult,						// 3
    sLANone,							// 4
...
typedef union {
    struct
    {
        unsigned char 	AudioVolumeStatus	:7;	//bit 0~6	Volume status
        unsigned char	AudioMuteStatus		:1;	//bit 7 	Mute status
    }Map;
...
typedef union {
    struct
    {
        unsigned char 	AudioFormatCode	:6;	//bit 0~5	Audio Format Code
        unsigned char	AudioFormatID		:2;	//bit 6~7 	Audio Format ID
    }Map;
...
#define	REG_INT_STATUS		0x4C
    ...
    #define	B_TXFAIL_INT			0x20	//xxxxx 2014-0912
    #define	B_RXDONE_INT			0x10
    #define	B_TXDONE_INT			0x08
    #define	B_RXFAIL_INT			0x04
    #define	B_RX_INT				0x02
    #define	B_TX_INT				0x01	//xxxxx 2014-0912


void cecclrint( void );
void cecConfigure( unsigned char LogicAdr, unsigned char TimeUnit, unsigned char CECRst, unsigned char EnNAck );
unsigned char cecfirerdy( void );
unsigned char cecfirecmd(unsigned char TxCmdNum, unsigned char TxCmdBuf[] );
void cecmonitor( void );	//unsigned char mode, unsigned char LogicAdr )

void IT6802_ResetCEC(void);
void IT6802_CECInit(void);
void CEC_InactiveSource(unsigned char port);
void CEC_RoutingChange(unsigned char port);
...
void CECUICommandCode(unsigned char UIKey);
...
void IT6802_CECTxOutStatusUpdate(void);
void IT6802_CECCheckFollower(void);

extern void CECManager(void);
extern void DumpIT6802_CECReg(void);
...
typedef void (* Callback_CECReceivePowerOn)(unsigned char eCECActivePortNum);
typedef void (* Callback_CECReceivePowerOff)(void);

typedef struct ICECCallbacks
{
  Callback_CECReceivePowerOn CECReceivePowerOn_Callback;
  Callback_CECReceivePowerOff CECReceivePowerOff_Callback;
} CECCallbacks;
