...
MS2_PUBLIC void ring_set_mix_level(RingStream *stream, double level);
MS2_PUBLIC void ring_mix_pcm(RingStream *stream, char *file);
MS2_PUBLIC int get_wav_time(RingStream *stream);
...
MS2_PUBLIC void media_stream_free(MediaStream *stream);
...
struct _AudioStream
{
    ...
    int             ecdelay;       //set aec delay (when use_ec==1 ecdelay is valid)
    bool_t          dual_mic;
...
MS2_PUBLIC void audio_stream_start_record_wav(AudioStream *st, const char *name);
MS2_PUBLIC void audio_stream_stop_record_wav(AudioStream *st);
...
MS2_PUBLIC VideoStream *cam_playback_start();
MS2_PUBLIC void cam_playback_stop(VideoStream *stream);
...
MS2_PUBLIC void video_stream_link_audio(VideoStream *v_stream, AudioStream *a_stream);
MS2_PUBLIC void video_stream_unlink_audio(VideoStream *stream, AudioStream *audio);
