...
typedef struct{
    ...
    bool          TTCAN_Enable;             //time trigged mode CAN
    uint32_t      TTCAN_RefID;              //time triffed use Reference ID(TTCAN  mode used)
	bool          TTCAN_RefIDE;             // 0 - standard ID 1 - extended ID
    /* debug mode */
    ...
    bool          TransceiverStandbyMode;   //Transceiver Standby Mode
    bool          ListenOnlyMode;           //Listen Only Mode
    bool          TPSingleShotMode;         //Transmission Primary Single Shot mode for PTB
    bool          TSSingleShotMode;         //Transmission Secondary Single Shot mode for STB
}CAN_HANDLE;
