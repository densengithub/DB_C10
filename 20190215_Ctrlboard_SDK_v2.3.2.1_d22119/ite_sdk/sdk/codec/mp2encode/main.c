...
#if defined(AUDIO_PLUGIN_MESSAGE_QUEUE)
    ithPrintf("[Mp2 Enc] ch %d sampleRate %d mode 0x%x , 0x%x %d\n",s->nb_channels,s->sample_rate,mode,gOutBuf,gOutBuf[0]);
...
#if defined(AUDIO_PLUGIN_MESSAGE_QUEUE)
//    ithPrintf("[Mp2 Enc] nFrames %d , frameSize %d 0x%x 0x%x 0x%x 0x%x \n",(gnFrames+1),frameSize,gOutFrameBuf[0],gOutFrameBuf[1],gOutFrameBuf[2],gOutFrameBuf[3]);
//    AudioPluginAPI(SMTK_AUDIO_PLUGIN_CMD_ID_PRINTF);    
#else
