...
void SetFrameNo(int frameno)
{
        ...
        #if !defined(API_V2)
            MMIO_Write(DrvDecode_Frame_Lo, frameno & 0xffff);
            MMIO_Write(DrvDecode_Frame_Hi, frameno >> 16);
        #else
...
//        #endif
        ...
        #endif
        #if !defined(API_V2)
        MMIO_Write(DrvDecode_Frame_Lo, (short)( ((unsigned int)decTime) & 0xffff));
        MMIO_Write(DrvDecode_Frame_Hi, (short)( ((unsigned int)decTime) >> 16)   );
        #else
...
void GetFrameNo(int *frameno)
{
    ...
    #if defined(ENABLE_SET_FRAME)
        #if !defined(API_V2)
            *frameno = (MMIO_Read(DrvDecode_Frame_Hi) << 16) | MMIO_Read(DrvDecode_Frame_Lo);
        #else
        ...
        int time;
        #if !defined(API_V2)
            time = (MMIO_Read(DrvDecode_Frame_Hi) << 16) | MMIO_Read(DrvDecode_Frame_Lo);
        #else
...
void ClearRdBuffer(void) {
    ...
    MMIO_Write(MMIO_PTS_HI, 0);
    MMIO_Write(MMIO_PTS_LO, 0);
    MMIO_Write(DrvDecode_Frame_Lo, 0);
    MMIO_Write(DrvDecode_Frame_Hi, 0);
    #endif // USE_PTS_EXTENSION
