...
static int EncFillReadBuffer(int nReadBytes) {
    ...
    if (nReadBytes > 0)
    {
        ...
        decTime = decTime + (wavFrameStep >> 15) + (wavFrameAccu >> 15);
        wavFrameAccu = wavFrameAccu & 0x7fff;
        MMIO_Write(DrvEncode_Frame_Lo, (short)( ((unsigned int)decTime) & 0xffff));
        MMIO_Write(DrvEncode_Frame_Hi, (short)( ((unsigned int)decTime) >> 16)   );
...
static void EncClearRdBuffer(void) {
    ...
    #if !defined(WIN32) && !defined(__CYGWIN__)
    MMIO_Write(DrvEncode_Frame_Lo, 0);
    MMIO_Write(DrvEncode_Frame_Hi, 0);
    #endif // !defined(WIN32) && !defined(__CYGWIN__)
