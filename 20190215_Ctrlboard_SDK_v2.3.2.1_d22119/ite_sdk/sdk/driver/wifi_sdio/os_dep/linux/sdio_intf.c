...
static const struct sdio_device_id sdio_ids[] = {
		...
		{ SDIO_DEVICE(0x024c, 0x8179),.driver_data = (void *)RTL8188E},
...
struct sdio_drv_priv {
	...
	int drv_registered;
	_mutex hw_init_mutex; //eason add
};
...
_adapter *rtw_sdio_if1_init(struct dvobj_priv *dvobj, const struct sdio_device_id  *pdid)
{
	...
	padapter->bDriverStopped=_FALSE; //eason
	...
	padapter->hw_init_mutex = &sdio_drvpriv.hw_init_mutex; //eason add
...
static int __init rtw_drv_entry(void)
{
	...
	_rtw_mutex_init(&sdio_drvpriv.hw_init_mutex); //eason add
