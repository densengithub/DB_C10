...
s32 rtl8188es_init_recv_priv(PADAPTER padapter)
{
	...
	for (i = 0; i < NR_RECVBUFF; i++)
	{
		...
		allocsize = _RND(n, adapter_to_dvobj(padapter)->intf_data.block_transfer_len);

		//precvbuf->pskb = rtw_skb_alloc(MAX_RECVBUF_SZ + RECVBUFF_ALIGN_SZ);
		precvbuf->pskb = rtw_skb_alloc(allocsize);
	...
	do {
		...
		do {
			...
			if ((pattrib->pkt_len==0) || (pkt_offset>transfer_len)) {
				DBG_8192C("%s()-%d: RX Warning!,pkt_len==0 or pkt_offset(%d)> transfer_len(%d) \n", __FUNCTION__, __LINE__, pkt_offset, transfer_len);
				rtw_free_recvframe(precvframe, &precvpriv->free_recv_queue);
				break;
			}
