...
typedef enum CAPTURE_DEVICE_TAG
{
    ...
    CAPTURE_DEV_EXTERNAL_HDMIRX = MMP_CAP_DEV_EXTERNAL_HDMIRX,
    ...
    void)
{
    ...
    /* 3. Frontend Device Init */
    ithCapDeviceInitialize(CAPTURE_DEV_SENSOR);
    ...
    void)
{
    ...
    /* Allocate Cap device share mem */
    capctxt = (MMP_CAP_SHARE*)calloc(1, sizeof(MMP_CAP_SHARE));
    if (capctxt == MMP_NULL)
    ...
    ithCapGetDeviceInfo(&cap0, capctxt);
    ...
    cap_scale                           = (MMP_FLOAT)capctxt->OutWidth / gptAVCEncoder[0]->frameWidth;
    ...
    ispctxt->In_PitchY      = CAPTURE_MAXWIDTH;
    ispctxt->In_PitchUV     = CAPTURE_MAXWIDTH;
    ...
    ithCapSetSkipMode(  &cap0,  MMP_CAPTURE_SKIP_BY_TWO);
#elif defined(SENSOR_AR0130)
    ithCapSetSkipMode(  &cap0,  MMP_CAPTURE_NO_DROP);
#endif
    /* Enable Capture Engine. */
    ithCapFire(&cap0, capctxt);
lab_end:
    if (capctxt)
        free(capctxt);
...
CloseEngine(ENCODER_TYPE type)
{
    ...
    ithCapEnableInterrupt(&cap0, MMP_FALSE);
    ithCapPowerDown();
    _WaitAllQueue_Empty();
    mmpVPPowerDown();
    ...
    void *arg)
{
    ...
    for (i = 0; i < VIDEO_STREAM_NUM; i++)
        bSkipFrame[i]   = _Chk_Skip_Frame(i);
