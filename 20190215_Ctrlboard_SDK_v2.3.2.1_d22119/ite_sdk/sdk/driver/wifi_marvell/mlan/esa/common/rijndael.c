		...
		u8 ct[16])
{
	...
	s0 = (Te4[(t0 >> 24)] << 24) ^
		(Te4[(t1 >> 16) & 0xff] << 16) ^
		(Te4[(t2 >> 8) & 0xff] << 8) ^ (Te4[(t3) & 0xff]) ^ rk[0];
	PUTU32(ct, s0);
	s1 = (Te4[(t1 >> 24)] << 24) ^
		(Te4[(t2 >> 16) & 0xff] << 16) ^
		(Te4[(t3 >> 8) & 0xff] << 8) ^ (Te4[(t0) & 0xff]) ^ rk[1];
	PUTU32(ct + 4, s1);
	s2 = (Te4[(t2 >> 24)] << 24) ^
		(Te4[(t3 >> 16) & 0xff] << 16) ^
		(Te4[(t0 >> 8) & 0xff] << 8) ^ (Te4[(t1) & 0xff]) ^ rk[2];
	PUTU32(ct + 8, s2);
	s3 = (Te4[(t3 >> 24)] << 24) ^
		(Te4[(t0 >> 16) & 0xff] << 16) ^
		(Te4[(t1 >> 8) & 0xff] << 8) ^ (Te4[(t2) & 0xff]) ^ rk[3];
	PUTU32(ct + 12, s3);
}
...
void
rijndael_set_key(rijndael_ctx * ctx, UINT8 * key, int bits, int encrypt)
{
	ctx->Nr = rijndaelKeySetupEnc(ctx->key, key, bits);
	if (encrypt) {
		ctx->decrypt = 0;
	} else {
		ctx->decrypt = 1;
		rijndaelKeySetupDec(ctx->key, key, bits, ctx->Nr);
	}
}

void
rijndael_decrypt(rijndael_ctx * ctx, UINT8 * src, UINT8 * dst)
{
	rijndaelDecrypt(ctx->key, ctx->Nr, src, dst);
}

//#ifdef AES_WRAP_FUNCTION
void
rijndael_encrypt(rijndael_ctx * ctx, UINT8 * src, UINT8 * dst)
{
	rijndaelEncrypt(ctx->key, ctx->Nr, src, dst);
}
