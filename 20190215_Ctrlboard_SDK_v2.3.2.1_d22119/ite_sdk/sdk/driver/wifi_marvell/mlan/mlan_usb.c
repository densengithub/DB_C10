			 ...
			 usb_tx_aggr_params * pusb_tx_aggr)
{
	...
	pmbuf_aggr =
		wlan_alloc_mlan_buffer(pmadapter, buf_len, 0,
				       MOAL_MALLOC_BUFFER);
	if (pmbuf_aggr) {
	...
	if (pmbuf_aggr && pmbuf_aggr->data_len) {
		...
		if (pusb_tx_aggr->aggr_ctrl.aggr_mode == MLAN_USB_AGGR_MODE_NUM)
			max_aggr_size *=
				MAX(MLAN_USB_MAX_PKT_SIZE,
				    pusb_tx_aggr->aggr_ctrl.aggr_align);
		/* if len is exactly multiple of USB_BLOCK_SIZE, add 1 to
		   length, to avoid FW waiting on another usb block */
...
wlan_usb_deaggr_rx_pkt(IN pmlan_adapter pmadapter, IN pmlan_buffer pmbuf)
{
	...
	while (aggr_len >= sizeof(RxPD)) {
		...
		/* make new buffer and copy packet to it (including RxPD).
		   Also, reserve headroom so that there must have space to
		   change RxPD to TxPD for bridge packet in uAP mode */
			   ...
			   IN usb_tx_aggr_params * pusb_tx_aggr)
{
...

	{
		...
		if ((aggr_len_counter + pmbuf->data_len) < max_aggr_size) {
			f_precopy_cur_buf = 1;	/* can fit current packet in
						   aggr */
			if (next_pkt_len) {
				aggr_len_counter +=
	...
	if (f_postcopy_cur_buf) {
		...
		if (!pmbuf_aggr) {	/* this is possible if just sent
					   (above) */
			/* create new buffer to hold next pkts */
			pmbuf_aggr =
				wlan_usb_new_tx_aggr_buf(pmadapter,
							 pusb_tx_aggr);
			if (pmbuf_aggr == MNULL) {
				PRINTM(MERROR,
				       "Error allocating [usb_tx] aggr mlan_buffer.\n");
