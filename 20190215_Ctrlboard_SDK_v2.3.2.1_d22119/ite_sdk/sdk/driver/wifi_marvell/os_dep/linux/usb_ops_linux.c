...
#if 0 //def CONFIG_USB_SUPPORT_ASYNC_VDN_REQ
	...
	u16 value, u16 index, void *pdata, u16 len, u8 requesttype)
{
	...
	//urb = usb_alloc_urb(0, GFP_ATOMIC); //James
	urb = usb_alloc_urb(0);
	if (!urb) {
	...
	//rc = usb_submit_urb(urb, GFP_ATOMIC); //James
...
{	
	...
	//status = usb_submit_urb(purb, GFP_ATOMIC); //James
...
void usb_read_port_cancel(struct intf_hdl *pintfhdl)
{
	...
	for (i=0; i < NR_RECVBUFF ; i++) {
		...
		if (precvbuf->purb)	 {
			...
			//usb_kill_urb(precvbuf->purb); //James
			usb_unlink_urb(precvbuf->purb);
		}		
		precvbuf++;
	}
	...
	usb_unlink_urb(padapter->recvpriv.int_in_urb)); //James
#endif
}
	...
	for (i=0; i<NR_XMITBUFF; i++) {
		for (j=0; j<8; j++) {
			if (pxmitbuf->pxmit_urb[j]) {
				//usb_kill_urb(pxmitbuf->pxmit_urb[j]); //James
				usb_unlink_urb(pxmitbuf->pxmit_urb[j]);
			}
		}
		pxmitbuf++;
	}
