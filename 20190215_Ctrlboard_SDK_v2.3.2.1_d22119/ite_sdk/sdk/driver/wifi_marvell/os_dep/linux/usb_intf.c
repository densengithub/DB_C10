...
int const  rtw_usb_id_tbl_len = sizeof(rtl8188e_usb_id_tbl) / sizeof(struct usb_device_id);
...
static u32 usb_dvobj_init(struct dvobj_priv *pdvobjpriv, struct usb_interface *usb_intf)
{
	int	i;
	u8	val8;
	int	status = _FAIL;
	...
	struct usb_interface *pusb_intf, const struct usb_device_id *pdid)
{
	...
	if((pnetdev = rtw_init_netdev(NULL)) == NULL) {
		goto handle_dualmac;
	}
   ...
   /* Initialize dvobj_priv */
	if (!usb_dvobj_init(dvobj, pusb_intf)) {
...
static int rtw_drv_init(struct usb_interface *pusb_intf, const struct usb_device_id *pdid)
{
	...
	struct usb_device* dev = pusb_intf->usb_dev;
	...
	//James Add
	for (i = 0; i < rtw_usb_id_tbl_len; i++)
	{
	   if (dev->descriptor.idVendor == rtw_usb_id_tbl[i].idVendor &&
		   dev->descriptor.idProduct == rtw_usb_id_tbl[i].idProduct)
	   {
		   printk("idVendor = 0x%x, idProduct = 0x%x \n",dev->descriptor.idVendor, dev->descriptor.idProduct);
		   break;
	   }
	}
	if (i == rtw_usb_id_tbl_len) {
	   printk("Device Descriptor not matching\n");
	   return NULL;
	}

    //James Modify
    dev->type = USB_DEVICE_TYPE_WIFI;
...
//
// James : Driver module load function
//
