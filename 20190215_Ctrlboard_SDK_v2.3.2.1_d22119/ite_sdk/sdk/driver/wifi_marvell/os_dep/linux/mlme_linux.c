...
_func_enter_;
	...
	if(authmode==_WPA_IE_ID_)
	{
		...
		wireless_send_event(adapter->pnetdev,IWEVCUSTOM,lens,buff,ap_addr,addr);
#endif
...
void rtw_indicate_sta_assoc_event(_adapter *padapter, struct sta_info *psta)
{
	...
	//printf("+%s, %02x:%02x:%02x:%02x:%02x:%02x netlink\n", __func__, MAC2STR( psta->hwaddr));
	_rtw_memcpy(addr.sa_data, psta->hwaddr, ETH_ALEN);

	//printf("+%s, %02x:%02x:%02x:%02x:%02x:%02x netlinkcopy\n", __func__, MAC2STR(addr.sa_data));

	DBG_871X("+rtw_indicate_sta_assoc_event\n");
	lens = 0;
	
#ifndef CONFIG_IOCTL_CFG80211
	wireless_send_event(padapter->pnetdev, IWEVREGISTERED, lens, NULL,ap_addr,addr);
#endif
...
#ifndef CONFIG_IOCTL_CFG80211
	wireless_send_event(padapter->pnetdev, IWEVEXPIRED, lens, NULL,ap_addr,addr);
#endif
	
}
#endif

#if 0 //def CONFIG_HOSTAPD_MLME
