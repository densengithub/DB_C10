...
woal_send_iwevcustom_event(moal_private * priv, char *str)
{
	...
	lens = iwrq.data.length;

	/* Send Event to upper layer */
	//wireless_send_event(priv->netdev, IWEVCUSTOM, &iwrq, buf);
	wireless_send_event(priv->netdev, IWEVCUSTOM, lens, buf, ap_addr, addr);
...
woal_send_mic_error_event(moal_private * priv, t_u32 event)
{
	...
	//wireless_send_event(priv->netdev, IWEVMICHAELMICFAILURE, &iwrq, (char *)&mic);
	wireless_send_event(priv->netdev, IWEVMICHAELMICFAILURE, lens, (char *)&mic, ap_addr, addr);
