...
moal_recv_packet(IN t_void * pmoal_handle, IN pmlan_buffer pmbuf)
{
	...
	if (pmbuf) {
		...
		if (priv) {
			...
			/* This is only required only in case of 11n and USB as
			   we alloc a buffer of 4K only if its 11N (to be able
			   to receive 4K AMSDU packets). In case of SD we
			   allocate buffers based on the size of packet and
			   hence this is not needed. */
			/* Modifying the truesize here as our allocation for
			   each skb is 4K but we only receive 2K packets and
			   this cause the kernel to start dropping packets in
			   case where application has allocated buffer based on
			   2K size i.e. if there a 64K packet received (in IP
			   fragments and application allocates 64K to receive
			   this packet but this packet would almost double up
			   because we allocate each 1.5K fragment in 4K and
			   pass it up. As soon as the 64K limit hits kernel
			   will start to drop rest of the fragments. Currently
			   we fail the Filesndl-ht.scr script for UDP, hence
			   this fix */
...
moal_recv_event(IN t_void * pmoal_handle, IN pmlan_event pmevent)
{
	...
	struct eventsockaddr ap_addr, addr;
	unsigned int lens;
	...
	case MLAN_EVENT_ID_DRV_CONNECTED:
		...
		if (IS_STA_WEXT(cfg80211_wext) &&
			...
			memset(ap_addr.sa_data, 0x00, ETH_ALEN);
			memcpy(ap_addr.sa_data, pmevent->event_buf, ETH_ALEN);
			ap_addr.sa_family = ARPHRD_ETHER;
			lens = wrqu.data.length;
			wireless_send_event(priv->netdev, SIOCGIWAP, lens, NULL, ap_addr, addr);
		}
#endif
#ifdef STA_CFG80211
	...
	case MLAN_EVENT_ID_DRV_SCAN_REPORT:
		...
		if (priv->report_scan_result) {
			...
			if (IS_STA_WEXT(cfg80211_wext)) {
				...
				lens = 0;
				wireless_send_event(priv->netdev, SIOCGIWSCAN, lens, NULL, ap_addr, addr);
			}
#endif
	...
	case MLAN_EVENT_ID_DRV_OBSS_SCAN_PARAM:
		...
		if (IS_STA_WEXT(cfg80211_wext)) {
			...
			//wrqu.data.length = pmevent->event_len + strlen(CUS_EVT_OBSS_SCAN_PARAM) + 1;
			//wireless_send_event(priv->netdev, IWEVCUSTOM, &wrqu, pmevent->event_buf);
			memset(&ap_addr, 0, sizeof(ap_addr));
			memset(&addr, 0, sizeof(addr));
			lens = pmevent->event_len + strlen(CUS_EVT_OBSS_SCAN_PARAM) + 1;
			wireless_send_event(priv->netdev, IWEVCUSTOM, lens, pmevent->event_buf, ap_addr, addr);
		}
#endif
		break;
	case MLAN_EVENT_ID_FW_BW_CHANGED:
		...
		if (IS_STA_WEXT(cfg80211_wext)) {
			...
			memset(&ap_addr, 0, sizeof(ap_addr));
			memset(&addr, 0, sizeof(addr));
			lens = pmevent->event_len + strlen(CUS_EVT_BW_CHANGED) + 1;
			wireless_send_event(priv->netdev, IWEVCUSTOM, lens, pmevent->event_buf, ap_addr, addr);
		}
#endif
		break;
