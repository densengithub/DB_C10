...
struct usb_device_id woal_usb_table[] = {
	...
	{MARVELL_USB_DEVICE
	 (USB8801_VID_1, USB8801_PID_1, "Marvell WLAN USB Adapter")},
	{MARVELL_USB_DEVICE
	 (USB8801_VID_1, USB8801_PID_2, "Marvell WLAN USB Adapter")},
	/* Terminating entry */
	{},
};
	...
	if (recv_length) {
		if (urb->status || (handle->surprise_removed == MTRUE)) {
			if (handle->surprise_removed || handle->is_suspended) {
				...
				/* Do not free mlan_buffer in case of command
				   ep */
				if (cardp->rx_cmd_ep != context->ep)
					woal_free_mlan_buffer(handle, pmbuf);
				goto setup_for_next;
		...
		if (status == MLAN_STATUS_PENDING) {
			...
			if (status == MLAN_STATUS_FAILURE) {
				...
				queue_work(handle->workqueue,
					   &handle->main_work);
				queue_work(handle->rx_workqueue,
					   &handle->rx_work);
			}
...
woal_usb_submit_rx_urb(urb_context * ctx, int size)
{
	...
	if (cardp->rx_cmd_ep != ctx->ep) {
		...
		ctx->pmbuf->data_offset =
			MLAN_NET_IP_ALIGN + MLAN_RX_HEADER_LEN;
...
woal_usb_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (woal_usb_table[i].idVendor) {
		...
		/* Note: From 2.6.34.2 onwards, remote wakeup is NOT enabled by
		   ...
		   this using - device_set_wakeup_enable(&udev->dev, 1); It has
		   been observed that some cards having device attr = 0xa0 do
		   not support remote wakeup. These cards come immediately out
		   of the suspend when power/wakeup file is set to 'enabled'. To
		   support all types of cards i.e. with/without remote wakeup,
		   we are NOT setting the 'power/wakeup' file from here. Also in
		   principle, we are not supposed to change the wakeup policy,
		   which is purely a userspace decision. */
...
woal_usb_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	/* The flag must be set here before the usb_kill_urb() calls. Reason:
	   In the complete handlers, urb->status(= -ENOENT) and 'is_suspended'
	   flag is used in combination to distinguish between a suspended
	   state and a 'disconnect' one. */
...
woal_usb_resume(struct usb_interface *intf)
{
	...
	/* Indicate device resumed. The netdev queue will be resumed only
	   after the urbs have been resubmitted */
	...
	/* Resume handler may be called due to remote wakeup, force to exit
	   suspend anyway */
...
#else
	pm_runtime_set_autosuspend_delay(&udev->dev, 0);	/* Autosuspend
								   delay in
								   jiffies */
#endif /* < 2.6.38 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 34)
