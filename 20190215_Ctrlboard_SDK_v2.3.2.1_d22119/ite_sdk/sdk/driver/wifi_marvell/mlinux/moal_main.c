...
int wq_sched_policy = 0;  // SCHED_NORMAL;
 ...
 */
static void
woal_hang_work_queue(job_t *work)
{
	int i;
	ENTER();
	if (!reset_handle) {
...
woal_reassociation_thread(void *data)
{
	...
	for (;;) {
		...
		if (kthread_should_stop(pmoal_thread->task))
			break;
 ...
 */
t_void
woal_main_work_queue(job_t * work)
{
