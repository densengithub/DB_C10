...
//#define OUTPUT_24BIT_YUV444_DUAL
//#define OUTPUT_24BIT_RGB444_DUAL
...
#define DISABLE_HWRESET
#define DISABLE_RESET_REFCLK
...
/* Add by Tim: For test */
#define SyncSeparate_RGB444   0
#define SyncSeparate_YUV444   1
#define SyncSeparate_24YUV422 2
#define SyncSeparate_16YUV422 3
#define SyncEmbedded_16YUV422 4
#define SyncEmbedded_8_YUV422 5
#define SyncEmbeded_8_YUV422_dual 6
#define SyncSeparate_8_YUV422_dual 7
#define SyncSeparate_dual_YUV444 8
#define SyncSeparate_dual_RGB444 9
#define SyncEmbedded_RGB444 10
...
#elif defined OUTPUT_24BIT_YUV444_DUAL
    #pragma message("OUTPUT_24BIT_YUV444_DUAL defined")
    #define HDMIRX_OUTPUT_MAPPING   0
    #define HDMIRX_OUTPUT_TYPE  (B_DDR_OUTPUT)
    #define HDMIRX_OUTPUT_COLORMODE (B_OUTPUT_YUV444<<O_OUTPUT_COLOR_MODE)  
#elif defined(OUTPUT_24BIT_RGB444_DUAL)
    #pragma message("OUTPUT_24BIT_RGB444_DUAL defined")
    #define HDMIRX_OUTPUT_MAPPING   0
    #define HDMIRX_OUTPUT_TYPE  (B_DDR_OUTPUT)
    #define HDMIRX_OUTPUT_COLORMODE (B_OUTPUT_RGB24<<O_OUTPUT_COLOR_MODE)   
#else// defined(OUTPUT_24BIT_RGB444)
...
// Global Data
///////////////////////////////////////////////////////////
static _IDATA unsigned int VideoCountingTimer = 0 ;
static _IDATA unsigned int AudioCountingTimer = 0 ;
static _IDATA unsigned int MuteResumingTimer = 0 ;
...
static _XDATA unsigned int SWResetTimeOut = FORCE_SWRESET_TIMEOUT;
...
static _XDATA unsigned int currHTotal ;
...
void RXINT_CheckVendorSpecInfo() ;
...
void AssignVideoTimerTimeout(unsigned int TimeOut);
void ResetVideoTimerTimeout();
void SwitchVideoState(Video_State_Type state);

void AssignAudioTimerTimeout(unsigned int TimeOut);
...
InitHDMIRX(BOOL bFullInit)
{
    ...
    HDMIRX_WriteI2C_Byte(0x08, 0xCF);
    ...
    if(bFullInit)
    {
        ...
        HDMIRX_DEBUG_PRINTF(("reg reset\n"));
        uc = HDMIRX_ReadI2C_Byte(0x10);
        HDMIRX_DEBUG_PRINTF(("hdmirx after reset val = 0x%x\n", uc));
    }
    #endif
    ...
    if( bFullInit )
    {
        ...
        delay1ms(200);/*TIM*/ // delay 0.2 sec by TPV project experience.
    }
    ...
    uc = HDMIRX_ReadI2C_Byte(0x07);
    HDMIRX_DEBUG_PRINTF(("hdmirx after reg_07 = 0x%x\n", uc));
    uc = HDMIRX_ReadI2C_Byte(0x05);
    HDMIRX_DEBUG_PRINTF(("hdmirx after reg_05 = 0x%x\n", uc));
}

void Turn_HDMIRX(BOOL bEnable)
{
    HDMIRX_DEBUG_PRINTF(("Turn_HDMIRX(%s)\n",bEnable?"ON":"OFF")) ;
    if( bEnable )
    {
        SWReset_HDMIRX() ;
    }
...
BOOL IsHDMIRX_Signal_Detect ()
{
    if (HDMIRX_ReadI2C_Byte(REG_RX_SYS_STATE) & 0x08)
    {
        return TRUE;
    }
    return FALSE;
...
SWReset_HDMIRX()
{
    Check_RDROM();
    //HDMIRX_WriteI2C_Byte(REG_RX_RST_CTRL, B_SWRST|B_EN_AUTOVDORST); // sw reset
    HDMIRX_WriteI2C_Byte(REG_RX_RST_CTRL, B_SWRST|B_AUDRST|B_VDORST); // sw reset
    delay1ms(1);
    SetALLMute();
    HDMIRX_WriteI2C_Byte(REG_RX_RST_CTRL, B_EN_AUTOVDORST); // normal operation
...
static REGPAIR _CODE acCustomizeValue[] =
{
    ...
    {REG_RX_VCLK_CTRL, 0x20}, // request by Clive for IT6603 board .... !@#$!@$#! .
...
static REGPAIR _CODE acDefaultValue[] =
{
    ...
    {REG_RX_VIO_CTRL,0x9B},// Reg08 /* mylin */
    //~jau-chih.tseng@ite.com.tw
    ...
    {REG_RX_I2CIO_CTRL, 0xDA}, /* mylin */
    {REG_RX_AIO_CTRL, 0x99},/* mylin */
...
Interrupt_Handler()
{
    ...
    if(int1data /*||(int4data&B_RXCKON_DET) */)
    {
        ...
        {
            HDMIRX_DEBUG_PRINTF3(("5V Power On interrupt\n"));
            RXINT_5V_PwrOn();
        }
...
RXINT_AVMute_Set()
{
    ...
    SetIntMask2((BYTE)(~(B_PKT_CLR_MUTE)),(BYTE)(B_PKT_CLR_MUTE)); // enable the CLR MUTE interrupt.
...
RXINT_AVMute_Clear()
{
    ...
    SetIntMask2((BYTE)(~(B_PKT_CLR_MUTE)),0); // clear the CLR MUTE interrupt.
}
...
VideoTimerHandler()
{
    ...
    if(VState == VSTATE_SyncWait)
    {
        ...
        else
        {
            ...
            uc=HDMIRX_ReadI2C_Byte(REG_RX_SYS_STATE);
            HDMIRX_DEBUG_PRINTF3(("REG_RX_SYS_STATE = %X\r",(int)uc));
...
{
    BYTE RxChEn ;
    RxChEn = (HDMIRX_ReadI2C_Byte(REG_RX_AUD_CHSTAT2)& M_CH_NUM) >> 4;
...
void
AssignVideoTimerTimeout(unsigned int TimeOut)
{
    VideoCountingTimer = TimeOut ;
}

void
AssignAudioTimerTimeout(unsigned int TimeOut)
{
...
void HDMIRX_SetVidStrength(int type)
{
    BYTE reg;
    BYTE value = 0x0;
    switch (type)
    ...
    HDMIRX_DEBUG_PRINTF(("set video ctrl value = 0x%x\n", value));
    HDMIRX_WriteI2C_Byte(REG_RX_VIO_CTRL, value);
    reg = HDMIRX_ReadI2C_Byte(REG_RX_VIO_CTRL);
    HDMIRX_DEBUG_PRINTF(("read vio ctrl value = 0x%x\n", reg));

}

void HDMIRX_SetVidClkCtrl(int type)
{
    BYTE reg;
    BYTE value = 0;
    ...
    HDMIRX_DEBUG_PRINTF(("set video CLK value = 0x%x\n", value));
    HDMIRX_WriteI2C_Byte(0x1D, value);
    reg = HDMIRX_ReadI2C_Byte(0x1D);
    HDMIRX_DEBUG_PRINTF(("read vio CLK value = 0x%x\n", reg));
}

#if 1
void HDMI_SetColorMode(int type)
{

    BYTE bOutputMapping, bOutputType;
    BYTE bOutputColorMode;


    switch (type)
    {
        
        case SyncSeparate_YUV444:
            printf("*************************\n");
            printf("1. YUV444 Separate \n");
            bOutputMapping = 0x00;/* Reg1B */
            bOutputType = 0x00; /*Reg1C */
            bOutputColorMode = B_OUTPUT_YUV444<<O_OUTPUT_COLOR_MODE; /* Reg3D */
            break;
        case SyncSeparate_24YUV422:
            printf("*************************\n");
            printf("2. YUV422 Separate 24bit\n");
            bOutputMapping = 0x00;
            bOutputType = 0x00;
            bOutputColorMode = B_OUTPUT_YUV422<<O_OUTPUT_COLOR_MODE;
            break;
        case SyncSeparate_16YUV422:
            printf("*************************\n");
            printf("3. YUV422 Separate 16bits\n");
            bOutputMapping = B_OUTPUT_16BIT;
            bOutputType = 0x00;
            bOutputColorMode = B_OUTPUT_YUV422<<O_OUTPUT_COLOR_MODE;
            break;
        case SyncEmbedded_16YUV422:
            printf("*************************\n");
            printf("4.YUV422 Embedded 16bit\n");
            bOutputMapping = B_OUTPUT_16BIT;
            bOutputType = B_SYNC_EMBEDDED;
            bOutputColorMode = B_OUTPUT_YUV422<<O_OUTPUT_COLOR_MODE;
            break;
        case SyncEmbedded_8_YUV422:
            printf("*************************\n");
            printf("5.YUV422 Embedded 8bit\n");
            bOutputMapping = B_OUTPUT_16BIT;
            bOutputType = (B_SYNC_EMBEDDED|B_CCIR565);
            bOutputColorMode = B_OUTPUT_YUV422<<O_OUTPUT_COLOR_MODE;
            break;
        case SyncEmbeded_8_YUV422_dual: /* no 8bit ddr */
            printf("*************************\n");
            printf("6.YUV422 Embedded 8bit dual\n");
            /*
            bOutputMapping = B_OUTPUT_16BIT;
            bOutputType = (B_SYNC_EMBEDDED|B_CCIR565);
            bOutputColorMode = B_OUTPUT_YUV422<<O_OUTPUT_COLOR_MODE;
            *///
            ...
            bOutputMapping = B_OUTPUT_16BIT;
            bOutputType = (B_SYNC_EMBEDDED|B_CCIR565|B_DDR_OUTPUT);
            bOutputColorMode = B_OUTPUT_YUV422<<O_OUTPUT_COLOR_MODE;
            break;
        case SyncSeparate_8_YUV422_dual:/* no 8bit ddr */
            printf("*************************\n");
            printf("7.YUV422 Separate 8bit dual\n");
            bOutputMapping = B_OUTPUT_16BIT;
            bOutputType = (B_CCIR565);
            bOutputColorMode = B_OUTPUT_YUV422<<O_OUTPUT_COLOR_MODE;
            break;
        case SyncSeparate_dual_YUV444:
            printf("*************************\n");
            printf("8.YUV444 Separate dual \n");
            bOutputMapping = 0x00;/* Reg1B */
            bOutputType = (B_DDR_OUTPUT); /*Reg1C */
            bOutputColorMode = B_OUTPUT_YUV444<<O_OUTPUT_COLOR_MODE; /* Reg3D */
            break;
        case SyncSeparate_dual_RGB444:
            printf("*************************\n");
            printf("9.RGB444 Separate dual \n");
            bOutputMapping = 0x00;
            bOutputType = (B_DDR_OUTPUT);
            bOutputColorMode = B_OUTPUT_RGB24<<O_OUTPUT_COLOR_MODE;
            break;
        case SyncEmbedded_RGB444:
            printf("*************************\n");
            printf("10.RGB444 Embedded\n");
            bOutputMapping = 0x00;
            bOutputType = (B_SYNC_EMBEDDED);
            bOutputColorMode = B_OUTPUT_RGB24<<O_OUTPUT_COLOR_MODE;
            break;
        case SyncSeparate_RGB444:
        default:
            printf("*************************\n");
            printf("11.RGB444 Separate\n");
            bOutputMapping = 0x00;
            bOutputType = 0x00;
            bOutputColorMode = B_OUTPUT_RGB24<<O_OUTPUT_COLOR_MODE;
            break;
    ...
    printf("*************************\n");
    SetHDMIRXVideoOutputFormat(bOutputMapping, bOutputType, bOutputColorMode);
}
#endif
...
SwitchVideoState(Video_State_Type state)
{
    ...
    case VSTATE_ColorDetectReset:
        ...
        AssignVideoTimerTimeout(MS_TimeOut(160)); //mylin
    ...
    case VSTATE_VideoOn:
        ...
        else
        {
            ...
            SetIntMask2((BYTE)~(B_NEW_AVI_PKG|B_PKT_SET_MUTE|B_PKT_CLR_MUTE),(BYTE)(B_NEW_AVI_PKG|B_PKT_SET_MUTE|B_PKT_CLR_MUTE));
            SetIntMask1(~(B_SCDTOFF|B_PWR5VOFF),(B_SCDTOFF|B_PWR5VOFF));
            SetIntMask4(0,B_M_RXCKON_DET);
...
Video_Handler()
{
    ...
    if(VState == VSTATE_ModeDetecting)
    {
        ...
        if( uc == 0xFF )
        {
            ...
            HDMIRX_WriteI2C_Byte(REG_RX_RST_CTRL, B_EN_AUTOVDORST) ;
            SwitchVideoState(VSTATE_SyncWait);
            HDMIRX_DEBUG_PRINTF(("[DBG]: %s %d SwitchVideoState \n", __FUNCTION__, __LINE__));
            return ;
        }
        ClearIntFlags(B_CLR_MODE_INT);
        ...
        {
            HDMIRX_DEBUG_PRINTF(("[DBG]: %s %d VidModeChange \n", __FUNCTION__, __LINE__));
            SwitchVideoState(VSTATE_SyncWait);
        ...
        else
        {
            ...
            SwitchVideoState(VSTATE_VideoOn);
            HDMIRX_DEBUG_PRINTF(("[DBG] Set Video on\n"));
...
SetColorSpaceConvert()
{
    ...
    case F_MODE_YUV422:
         HDMIRX_DEBUG_PRINTF(("Output mode is YUV422\n"));
        switch(bInputVideoMode&F_MODE_CLRMOD_MASK)
        {
        case F_MODE_YUV444:
            // HDMIRX_DEBUG_PRINTF(("Input mode is YUV444\n"));
        ...
        case F_MODE_RGB24:
            ...
            if(bOutputVideoMode & F_MODE_EN_UDFILT)// RGB24 to YUV422 need up/dn filter.
            {
                HDMIRX_DEBUG_PRINTF(("filter set\n"));
                filter |= B_RX_EN_UDFILTER ;
            }
            csc = B_CSC_RGB2YUV ;
            break ;
    ...
    uc |= filter ;
    HDMIRX_DEBUG_PRINTF(("[DBG] write reg1C =%02x\n", uc));
    HDMIRX_WriteI2C_Byte(REG_RX_VIDEO_CTRL1, uc);

    uc = HDMIRX_ReadI2C_Byte(REG_RX_VIDEO_MAP);
    HDMIRX_DEBUG_PRINTF(("[DBG] Reg1B value = %02x\n", uc));
    uc = HDMIRX_ReadI2C_Byte(REG_RX_VIDEO_CTRL1);
    HDMIRX_DEBUG_PRINTF(("[DBG] Reg1C value = %02x\n", uc));
    uc = HDMIRX_ReadI2C_Byte(REG_RX_PG_CTRL2);
    HDMIRX_DEBUG_PRINTF(("[DBG] Reg3D value = %02x\n", uc));


    HDMIRX_DEBUG_PRINTF(("[DBG] IsHDMIRXInterlace ? %s\n", IsHDMIRXInterlace()?"True":"FALSE"));
    //dump_vidmode();
    //dump_InfoFrame();
    //printf("[DBG] Inputmode = %s, OutputMode=%s\n")
 ...
 SetHDMIRXVideoOutputFormat(BYTE bOutputMapping, BYTE bOutputType, BYTE bOutputColorMode)
{
    ...
    HDMIRX_DEBUG_PRINTF(("SetHDMIRXVideoOutputFormat(Reg1B=%02X,Reg1C=%02X,Reg3D=%02X)\n",
    ...
    //printf("====bOutputVideoMode= 0x%02x\n", bOutputVideoMode);
    bOutputVideoMode |= (bOutputColorMode >> 6)&F_MODE_CLRMOD_MASK;
    ...
    case F_MODE_YUV444:
        ...
        break ;
    case F_MODE_YUV422:
        HDMIRX_DEBUG_PRINTF(("[DBG] === F_MODE_YUV422 =====\n"));
        uc |= B_OUTPUT_YUV422 << O_OUTPUT_COLOR_MODE ;
        break ;
    }
    HDMIRX_DEBUG_PRINTF(("write %02X %02X\n",(int)REG_RX_PG_CTRL2,(int)uc));
    HDMIRX_WriteI2C_Byte(REG_RX_PG_CTRL2, uc);
    //lv = HDMIRX_ReadI2C_Byte(REG_RX_PG_CTRL2);
    //printf("Reg3D = %02x\n", lv);
    ...
    if(VState == VSTATE_VideoOn)
    {
        if(IsHDMIRXHDMIMode())
        {
            HDMIRX_DEBUG_PRINTF(("[DBG]===== SetNewInfoVideoOutPut =====\n"));
            SetNewInfoVideoOutput();
        }
        else
        {
            HDMIRX_DEBUG_PRINTF(("[DBG]===== SetDVIVideoOutput =====\n"));
            SetDVIVideoOutput();
        }
        SetVideoMute(MuteByPKG);
    }
...
void
SetHDMIRX_Tri_State_Enable ()
{
    HDMIRX_WriteI2C_Byte(REG_RX_TRISTATE_CTRL, 0x5F);
}
...
// Tri-State Enable
// ***************************************************************************
void Set_IT6604_Tri_State_Enable()
{
    BYTE uc;
...
// Tri-State Disable
// ***************************************************************************
void Set_IT6604_Tri_State_Disable()
{
    BYTE uc;
