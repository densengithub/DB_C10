...
//eason edit, only support 8821AU
	...
	if (dvobj) {
		...
		if ((dvobj->NumInterfaces != 2 && dvobj->NumInterfaces != 3)
			...
			if (interface_to_usbdev(usb_intf)->state != USB_STATE_NOTATTACHED) {
				...
				//usb_reset_device(interface_to_usbdev(usb_intf)); //eason mark
			}
		}
