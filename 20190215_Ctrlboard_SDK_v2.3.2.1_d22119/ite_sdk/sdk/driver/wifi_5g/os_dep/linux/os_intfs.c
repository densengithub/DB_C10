...
#if 0//(LINUX_VERSION_CODE>=KERNEL_VERSION(2,6,29))
...
void rtw_hook_if_ops(struct net_device *ndev)
{
	...
	ndev->wireless_control = rtw_ioctl_smCtrl;      /*Set behavior*/
    ndev->wireless_information = rtw_ioctl_smInfo;  /*Get behavior*/
#endif
}
...
struct net_device *rtw_init_netdev(_adapter *old_padapter)
{
	...
	//pnetdev = alloc_netdevice(sizeof(_adapter)); //eason

	if(pnetdev == NULL)
		printf("pnetdev is NULL...\n");

	if (!pnetdev)
		return NULL;

	padapter = rtw_netdev_priv(pnetdev);
	//_rtw_memset(padapter, 0, sizeof(_adapter)); //eason add
	...
	//pnetdev->get_wireless_stats = rtw_get_wireless_stats;
	pnetdev->wireless_control = rtw_ioctl_smCtrl;      /*Set behavior*/
    pnetdev->wireless_information = rtw_ioctl_smInfo;  /*Get behavior*/
...
void rtw_os_ndev_free(_adapter *adapter)
{
	...
	if (adapter->pnetdev) {
		free_netdevice(adapter->pnetdev);
...
u32 rtw_start_drv_threads(_adapter *padapter)
{
	...
	if (is_primary_adapter(padapter)) {
		padapter->cmdThread = kernel_thread(rtw_cmd_thread, padapter, 4);
...
u8 rtw_free_drv_sw(_adapter *padapter)
{
	...
	if(padapter->rereg_nd_name_priv.old_pnetdev) {
		free_netdevice(padapter->rereg_nd_name_priv.old_pnetdev);
