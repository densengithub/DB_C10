...
void BlinkTimerCallback(void *data)
{
	...
	if(ATOMIC_READ(&pLed->bCancelWorkItem) == _FALSE)
		BlinkWorkItemCallback(pLed); //eason
...
//		We dispatch acture LED blink action according to LedStrategy.
//
void BlinkWorkItemCallback(PLED_USB pLed)
{
	...
	)
{
	...
	//_init_workitem(&(pLed->BlinkWorkItem), BlinkWorkItemCallback, pLed); //eason mark
}
	...
	)
{
	...
	//_cancel_workitem_sync(&(pLed->BlinkWorkItem)); //eason mark
	_cancel_timer_ex(&(pLed->BlinkTimer));
	ResetLedStatus(pLed);
}
