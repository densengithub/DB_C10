...
)
{
	...
	{// 2.4G band
		...
		/* 20160224 yiwei ,  one antenna  module don't support antenna  div , so driver must to control antenna  band , otherwise one of the band will has issue */
		if (Adapter->registrypriv.drv_ant_band_switch == 1 && pHalData->AntDivCfg == 0) {
				...
				/* DBG_871X("\n\nSwitch  ant band to ODM_BAND_2_4G\n\n"); */
		}
	...
	else	//5G band
	{
		...
		if (Adapter->registrypriv.drv_ant_band_switch == 1 && pHalData->AntDivCfg == 0) {
				...
				/* DBG_871X("\n\nSwitch  ant band to ODM_BAND_5G\n\n"); */
		}
...
)
{
		...
		) 
	{
		if(IS_HARDWARE_TYPE_8812(Adapter))
		{
			...
			//PHY_IQCalibrate_8812A(Adapter, _FALSE); //eason
#endif 
		}
		else if(IS_HARDWARE_TYPE_8821(Adapter))
		{
#if (RTL8821A_SUPPORT == 1)
			//PHY_IQCalibrate_8821A(pDM_Odm, _FALSE); //eason
#endif
		}	
