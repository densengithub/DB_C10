...
static ITUSurface      *gRoateSurf[MAX_ROTATE_BUFFER_COUNT];
static ITUSurface      *_gRoateSurf;
static ITUSurface      *_gLcdSurf[3];
    ...
    int lcd_index)
{
    ...
    {                	
        ...
        out_info.addrRGB = (MMP_UINT32)((((M2dSurface *)_gRoateSurf)->m2dSurf)->imageData);
    ...
    if (IS_ROTATE_DISPLAY())
    {
        ...
        m2dtempSurf = (M2dSurface *)_gLcdSurf[lcd_index];
...
void itv_set_rotation(ITURotation rot)
{
    ...
    if (IS_ROTATE_DISPLAY() && (!gRotateBuffer[0]))
    {
        ...
        {
            gRoateSurf[i] = ituCreateSurface(ithLcdGetHeight(), 
                                             ithLcdGetWidth(), 
                                             ithLcdGetHeight() * 2, 
                                             ITU_RGB565, 
        ...
        for (i = 0; i < 3; ++i)
        {
            _gLcdSurf[i] = ituCreateSurface(ithLcdGetWidth(), 
                                             ithLcdGetHeight(), 
                                             ithLcdGetWidth() * 2, 
                                             ITU_RGB565, 
                                             itv_rcs.dbuf[i], 
