...
static int axispi_read(uint8_t *cBuf, uint32_t cLen, uint8_t *dBuf, uint32_t dLen)
{
...
#ifdef EN_OLD_MALLOC_FLOW
    uint8_t *aBase = NULL;
    uint8_t *tmpAddr;
    
    aBase = (uint8_t*)malloc(5 + AXISPI_ALIGN_SIZE - 1);
    tmpAddr = (uint8_t*)((uint32_t)((uint32_t)aBase + AXISPI_ALIGN_SIZE - 1) & (uint32_t)~(AXISPI_ALIGN_SIZE - 1));
    addr = (uint32_t*)tmpAddr;
    memset(tmpAddr, 0, 5);
#else
    ...
    if(cBuf[0] == 0xEB)
    {
        mmpAxiSpiSetDummyByte(SPI_PORT, 0);// Set Quad mode dummy byte	
        iomode = SPI_IOMODE_4;
    }
    if(cBuf[0] == 0x6B)
    {
        if(gHasDummyByteAddr)   mmpAxiSpiSetDummyByte(SPI_PORT, 0);// Set Quad mode dummy byte	
        else                    mmpAxiSpiSetDummyByte(SPI_PORT, 8);// Set Quad mode dummy byte	
        iomode = SPI_IOMODE_2;
    }
    #endif
    
	if(dLen<4)	rst = mmpAxiSpiPioRead(SPI_PORT, iomode, SPI_DTRMODE_0, cBuf, (SPI_CMD_LEN)1, (uint32_t*)addr, (SPI_ADDR_LEN)aLen, (uint32_t*)dBuf, dLen);
	else	rst = mmpAxiSpiDmaRead(SPI_PORT, iomode, SPI_DTRMODE_0, cBuf, (SPI_CMD_LEN)1, (uint32_t*)addr, (SPI_ADDR_LEN)aLen, (uint32_t*)dBuf, dLen);
    
#ifdef EN_OLD_MALLOC_FLOW
    if(aBase != NULL)    free(aBase);
#endif
...
static int axispi_write(uint8_t *cBuf, uint32_t cLen, uint8_t *dBuf, uint32_t dLen)
{
    ...
    dBase = (uint8_t*)malloc(dLen + AXISPI_ALIGN_SIZE);
    tmpAddr = (uint8_t*)((uint32_t)((uint32_t)dBase + AXISPI_ALIGN_SIZE - 1) & (uint32_t)~(AXISPI_ALIGN_SIZE - 1));
    pData = (uint32_t*)tmpAddr;
    memcpy(tmpAddr, dBuf, dLen);
#else
    addr = (uint32_t*)gSpiAddrBuf;
    memset(gSpiAddrBuf, 0, 5);
	...
	if(dLen<4)	rst = mmpAxiSpiPioWrite(SPI_PORT, iomode, cBuf, (SPI_CMD_LEN)1, (uint32_t*)addr, (SPI_ADDR_LEN)aLen, (uint32_t*)pData, dLen);
    else	rst = mmpAxiSpiDmaWrite(SPI_PORT, iomode, cBuf, (SPI_CMD_LEN)1, (uint32_t*)addr, (SPI_ADDR_LEN)aLen, (uint32_t*)pData, dLen);
        
#ifdef EN_OLD_MALLOC_FLOW
    if(aBase != NULL)    free(aBase);
    if(dBase != NULL)    free(dBase);
#endif
...
#else
{
    ...
    *s = NF_STATUS_OIP;
	...
	{
	    if(spiNf_getFeature(0xC0, s))   printf(" get feature FAIL(PgPg)!!\n");
		if(_waitTimeOut())
		{
...
uint8_t spiNf_Initial(SPI_NF_INFO *info)
{
    ...
    {
        result = mmpAxiSpiInitialize(SPI_PORT, SPI_OP_MASTR, CPO_0_CPH_0, SPI_CLK_20M, SPI_CLK_DIV_0);
...
uint8_t spiNf_PageRead(uint32_t blk, uint32_t ppo, uint8_t *dBuf)	//(uint32_t addr, uint8_t *dBuf)
{
    ...
    if( spiNf_waitReady(&status,2000) ) goto errRdEnd;
...
uint8_t spiNf_BlockErase(uint32_t blk)
{
	...
	if(status&0x04)    printf(" erase FAIL(blk=%x, s=%x)!!\n", blk, status);
