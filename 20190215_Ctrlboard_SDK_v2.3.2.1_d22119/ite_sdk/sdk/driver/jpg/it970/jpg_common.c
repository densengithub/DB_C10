    ...
    JPG_COMM_DEV    *pJCommDev)
{
    ...
    pJFrmComp->alphaPlane = pJCommInitParam->alphaPlane;
    ...
    JPG_COMM_DEV    *pJCommDev)
{
    ...
    do{
		...
		pJFrmCompInfo->exColorSpace  = pJCommDev->jCommInitParam.exColorSpace;
            ...
            if( pJCommDev->hJCodec.bInitialed == false )
            {
                ...
                if( pJCommDev->jCodecDesc.init )
                {
                	if(pJCommInitParam->codecType != JPG_CODEC_DEC_MJPG && 
						pJCommInitParam->codecType != JPG_CODEC_DEC_JPG_CMD)
