    ...
    JPG_DITHER_KEY     *ditherKeyInfo)
{
    ...
    if( !ditherKeyInfo->bDisableDitherKey ) 
    {
        if(b_JPG_REGS_USE_CMDQ)
        {
    		w_data = jpgWriteRegMaskUsingCmdQ(REG_JPG_DISABLE_DITHER_KEY, ditherKeyInfo->bDisableDitherKey << JPG_SHT_TO_HIGH_WORD , 0xFFFF0000, jpg_regs.reg31);
    		jpg_regs.reg31 = w_data;
    		w_data = jpgWriteRegMaskUsingCmdQ(REG_JPG_SET_DITHER_KEY_MODE, ditherKeyInfo->ditherKeyMode, 0xFFFF, jpg_regs.reg31);
    		jpg_regs.reg31 = w_data;
        }
        else
        {
