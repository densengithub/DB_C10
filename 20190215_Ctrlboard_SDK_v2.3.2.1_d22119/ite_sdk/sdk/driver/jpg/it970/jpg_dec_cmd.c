...
//=============================================================================
static uint32_t	 alpha_vram_addr =0;
static uint8_t	 *alpha_sys_addr =NULL;
    ...
    JPG_CODEC_HANDLE    *pHJCodec)
{
    ...
    do{
		...
		JPG_YUV_TO_RGB      *pJYuvToRgb = &pHJCodec->yuv2RgbMatrix;
		JPG_DITHER_KEY		*pJDitherKey = &pHJCodec->jDitherKey;
		JPG_ALPHA_PLANE     *pJAlphaPlane = &pHJCodec->jAlphaPlane;
		...
		if( pHJCodec->ctrlFlag & JPG_FLAGS_OUTPUT_RGB_MODE )
		{
			JPG_SetYuv2RgbMatrix(pJYuvToRgb);	
			JPG_SetRgb565DitherKey(pJDitherKey);	

            if (pHJCodec->yuv2RgbMatrix.VIPMode != JPG_OUTPUT_RGB565)
			    JPG_SetAlphaPlaneReg(pJAlphaPlane);
		}
    ...
    void                *extraData)
{
    ...
    do{    
		...
		if(pHJComm->pJDispInfo->outColorSpace >= JPG_COLOR_SPACE_RGB565  && 
			pHJComm->pJDispInfo->outColorSpace <= JPG_COLOR_SPACE_ARGB4444)
			pHJCodec->ctrlFlag |= JPG_FLAGS_OUTPUT_RGB_MODE;
        ...
        if( pHJCodec->ctrlFlag & JPG_FLAGS_MJPG_FIRST_FRAME )
        {
            ...
            if( pHJCodec->ctrlFlag & JPG_FLAGS_OUTPUT_RGB_MODE )
            {
                ...
                if( pJFrmComp->decColorSpace != JPG_COLOR_SPACE_YUV411 )
                {
                    ...
                    pJHwCtrl->codecCtrl = (JPG_OP_DECODE_RGB_MODE | pJDecoder->triggerMode);

					switch(pHJComm->pJDispInfo->outColorSpace)
					{
						case JPG_COLOR_SPACE_RGB565:
							pHJCodec->yuv2RgbMatrix.VIPMode = JPG_OUTPUT_RGB565;
							pHJCodec->jAlphaPlane.bEnConstAlpha = false;
                            //b_JPG_REGS_USE_CMDQ = true;
						break;

						case JPG_COLOR_SPACE_ARGB4444:
							pHJCodec->yuv2RgbMatrix.VIPMode = JPG_OUTPUT_RGB4444;
							pHJCodec->jAlphaPlane.bEnConstAlpha = pJFrmComp->alphaPlane.bEnConstAlpha; //Note.bEnConstAlpha is true is ConstAlpha, bEnConstAlpha is false is per pixel Alpha.
						break;

						case JPG_COLOR_SPACE_ARGB8888:
							pHJCodec->yuv2RgbMatrix.VIPMode = JPG_OUTPUT_RGB8888;
							pHJCodec->jAlphaPlane.bEnConstAlpha = pJFrmComp->alphaPlane.bEnConstAlpha;
						break;		
					}

					if(pHJComm->pJDispInfo->outColorSpace == JPG_COLOR_SPACE_ARGB4444 || 
						pHJComm->pJDispInfo->outColorSpace == JPG_COLOR_SPACE_ARGB8888 )
					{
						if(pHJCodec->jAlphaPlane.bEnConstAlpha)	
						{
							pHJCodec->jAlphaPlane.ConstAlpha = pJFrmComp->alphaPlane.ConstAlpha; //just for test.
						}
						else
						{
						    pHJCodec->jAlphaPlane.AlphaPlaneAddr = pJFrmComp->alphaPlane.AlphaPlaneAddr;
                            pHJCodec->jAlphaPlane.AlphaPlanePitch = pJFrmComp->alphaPlane.AlphaPlanePitch;    
						}
					}
                }
                else
                {
                    jpg_msg(JPG_MSG_TYPE_ERR, "Jpg not support YUV411 to RGB565. ");
    ...
    void                *extraData)
{
    ...
    do{
        ...
        if(WriteBuf)
        {
            mappedSysRam = (uint8_t *)ithMapVram((uint32_t)WriteBuf, (pHJComm->jInBufInfo[pHJComm->actIOBuf_idx].bufLength) , ITH_VRAM_WRITE);
            memcpy(mappedSysRam, pHJComm->jInBufInfo[pHJComm->actIOBuf_idx].pBufAddr, (pHJComm->jInBufInfo[pHJComm->actIOBuf_idx].bufLength) );
            ithUnmapVram((void*)mappedSysRam, (pHJComm->jInBufInfo[pHJComm->actIOBuf_idx].bufLength) );
            ithFlushDCacheRange((void*)mappedSysRam, (pHJComm->jInBufInfo[pHJComm->actIOBuf_idx].bufLength) );
            ithFlushMemBuffer();
        }

        JPG_LogReg(false);
		JPG_RGBModeReg(false);
        JPG_StartReg();
