...
#endif
    int32_t   count = ITH_COUNT_OF(maps);
    uint32_t  addrBase = addr & 0xFFF00000;
    uint32_t  hostAddr = 0;
    int       i;

    for (i = 0; i < count; ++i)
    {
        if (addrBase == maps[i].ahbBaseAddr)
        {
            hostAddr = addr - maps[i].ahbBaseAddr + maps[i].hostBaseAddr;
            break;
        }
    }
...
uint16_t ithReadShortRegA(uint32_t addr)
{
	...
	uint16_t dataLo = ithReadRegH(hostAddr);
