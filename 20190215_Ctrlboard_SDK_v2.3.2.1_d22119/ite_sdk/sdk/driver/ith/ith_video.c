...
//#if (CFG_CHIP_FAMILY == 9850)
    ...
    if (TILING_MODE_OFF)
    {
        ...
        Y_BUFFER_OFFSET        = (1088 * 2048 * 3/2);
        U_BUFFER_OFFSET        = (1088 * 2048);
        V_BUFFER_OFFSET        = (1088 * 2048 + 960);
        VIDEO_BUFFER_HEIGHT    = (1632 * FRAME_BUFFER_COUNT);
        BLANK_BUFFER_HEIGHT    = (16);
        BLANK_U_BUFFER_OFFSET  = (128);
        BLANK_V_BUFFER_OFFSET  = (128 * 2);
        DB_BUFFER_HEIGHT       = (8);                             // (frame_pitch/16) * 2 * 8 * 8
        ...
        CMD_DATA_BUFFER_COUNT  = (3);
        CMD_DATA_BUFFER_HEIGHT = (250);
        MEMORY_POOL_HEIGHT     = (1632 * FRAME_BUFFER_COUNT + 8 + 0 + 4 + 64 + 180 + 16 + 250 * CMD_DATA_BUFFER_COUNT + 1);
        // FrmBuf + MV + TC + VLD + DB + CMD + Reserved
    ...
    AVC_DECODER *ptDecoder)
{
	...
	ithWriteRegA(REG_VIDEO_SPECIAL_FUNC_SETTING_F0H, 0x180);
    ...
    uint8_t *pv)
{
    ...
    } else
    {
        ...
        for (y = 0; y < 16; y++)
        {
            for (x = 0; x < 2; x++)
            {
                orgAdr   = (uint32_t)py + y * VIDEO_BUFFER_PITCH + x * 8;
        ...
        for (y = 0; y < 8; y++)
        {
            orgAdr   = (uint32_t)pu + y * VIDEO_BUFFER_PITCH;
        ...
        for (y = 0; y < 8; y++)
        {
            orgAdr   = (uint32_t)pv + y * VIDEO_BUFFER_PITCH;
        ...
        ithFlushDCacheRange((void*)py, 256);
        ithFlushDCacheRange((void*)pu, 64);
        ithFlushDCacheRange((void*)pv, 64);
        ithFlushMemBuffer();
    #endif
#endif
    }
