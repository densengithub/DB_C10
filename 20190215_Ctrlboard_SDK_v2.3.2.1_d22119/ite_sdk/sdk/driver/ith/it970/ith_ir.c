...
static void ithIrSetTxGpio(unsigned int pin)
{
	ithWriteRegMaskA(ITH_IR_BASE + ITH_IR_GPIO_SEL_REG, pin << 8, 0x7F << 8);
	ithSetRegBitA(ITH_IR_BASE + ITH_IR_GPIO_SEL_REG, 15);  //Enable TX GPIO Select
}
...
void ithIrInit(unsigned int pin, unsigned long extClk, unsigned long sample, unsigned int precision)
{
    ...
    ithIrSetGpio(pin);
   	...
   	//ithIrSetCaptureMode(ITH_IR_BOTH);  //Test Tx->Rx Local Case
...
void ithIrTxInit(unsigned int pin, unsigned long extClk, unsigned long sample, unsigned int precision)
{
	...
	ithIrSetTxGpio(pin);
#else
	//IT970 use fixed pin, ex: GPIO51_mode4
    ithGpioSetMode(pin, ITH_GPIO_MODE4);
#endif
