...
void ithLcdEnable(void)
{
    ...
    if (ithLcdGetPanelType() == ITH_LCD_MIPI)
    {
        ...
        usleep(100);                           //100us
        // ithWriteRegA(0xD8000044, 0x400ac001);  //DPHY PORn rst normal
        ithWriteRegA(0xD8000044, 0x400ac081);  //DPHY PORn rst normal
        usleep(75);                            //75us
        // ithWriteRegA(0xD8000044, 0x000ac001);  //DPHY ResetN rst normal
    ...
    // GPIO41~ GPIO44(MODE3), GPIO45-GPIO62 GPIO86-GPIO90(mode 1) 128pin, disable input mode
    if (ithLcdGetPanelType() == ITH_LCD_RGB)
    {
        int i;
        ...
        for(i = 41; i < 45; i++)
        {
            ithGpioSetMode(i, ITH_GPIO_MODE3);
        }
        for (i = 45; i < 63; i++)
        {
            ithGpioSetMode(i, ITH_GPIO_MODE1);
...
void ithLcdDisable(void)
{
    ...
    if (ithLcdGetPanelType() == ITH_LCD_MIPI)
    {
        ...
        // ithWriteRegA(0xD8000044, 0x800ac001);    //[31]=1 DPHY PORn, KCLK
        ithWriteRegA(0xD8000044, 0x800ac081);    //[31]=1 DPHY PORn, KCLK
        usleep(1);                          // > 500ns
        // ithWriteRegA(0xD8000044, 0xC00ac001);    //[30]=1 DPHY ResetN, KCLK
    ...
    // GPIO41~ GPIO44(MODE3), GPIO45-GPIO62 GPIO86-GPIO90(mode 1) 128pin, set input mode
    if (ithLcdGetPanelType() == ITH_LCD_RGB)
    {
        int i;
