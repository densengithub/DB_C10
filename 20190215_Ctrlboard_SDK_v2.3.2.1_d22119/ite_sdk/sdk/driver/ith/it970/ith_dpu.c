...
void ithDpuSetCrcData(unsigned int CrcData)
{
    ...
    #else
	ithWriteRegA( ITH_DPU_REG_DCR, 0x20080000);	
	ithWriteRegA( ITH_DPU_REG_CRCSD, cpu_to_le32(CrcData) ); 
#endif	//WIN32
}
...
/*    
	...
	for(i=0; i<0x40; i+=4)
	{
		printf("%08x ",ithReadRegA( ITH_DPU_REG_DCR + i));
		if( (i&0xC)==0x0C )	printf("\n");
	}
*/
...
void ithDpuSetKey(uint32_t *key, uint8_t KeyNum)
{
	...
	{
		ithWriteRegA( ((ITH_DPU_REG_KEY0)+(KeyIndex*4)), cpu_to_le32(pKey[KeyIndex]) ); 		
	}
}
...
void ithDpuSetVector(uint32_t *vector, uint8_t len)
{
	...
	for(vi=0; vi<len; vi++)
	{
		ithWriteRegA( ((ITH_DPU_REG_VECTOR0) + (vi * 4)), cpu_to_le32(pIV[vi]) ); 
	}
}
...
bool ithDpuWait(void)
{
	...
	do
	{
		...
		if(TomeOutCnt--==0)
		{
			printf("ith DPU timeOut-970, dsr_base=%x, %x!!\n",ITH_DPU_REG_DSR, ithReadRegA(ITH_DPU_REG_DSR) );
			Result = 0;
			break;
		}
		//usleep(100);
		//printf("ith DPU-970, dsr=%x, %x!!\n",ITH_DPU_REG_DSR, ithReadRegA(ITH_DPU_REG_DSR) );
	} while((StatusReg32&0x00000001) == 0x00000000);

	printf("DCR5:0x%08x\n",ithReadRegA(ITH_DPU_BASE+0x00));
