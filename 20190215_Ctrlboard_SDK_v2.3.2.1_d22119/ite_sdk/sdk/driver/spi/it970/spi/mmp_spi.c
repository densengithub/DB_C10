...
{
    //SPI_0
    {SPI_0, 20, 3}, {SPI_0, 28, 3}, {SPI_0, 58, 3},
    //SPI_1
    {SPI_1, 24, 3}, {SPI_1, 35, 3}, {SPI_1, 39, 3}, {SPI_1, 50, 4}
};

static PORT_IO_MAPPING_ENTRY tSpiMosiMappingTable[] =
{
    //SPI_0
    {SPI_0, 19, 3}, {SPI_0, 27, 3}, {SPI_0, 57, 3},
    //SPI_1
    {SPI_1, 23, 3}, {SPI_1, 34, 3}, {SPI_1, 38, 3}, {SPI_1, 49, 4}
};

static PORT_IO_MAPPING_ENTRY tSpiClockMappingTable[] =
{
    //SPI_0
    {SPI_0, 17, 3}, {SPI_0, 25, 3}, {SPI_0, 55, 3},
    //SPI_1
    {SPI_1, 21, 3}, {SPI_1, 32, 3}, {SPI_1, 36, 3}, {SPI_1, 47, 4}
};

static PORT_IO_MAPPING_ENTRY tSpiChipSelMappingTable[] =
{
    //SPI_0
    {SPI_0, 18, 3}, {SPI_0, 26, 3}, {SPI_0, 29, 3}, {SPI_0, 56, 3},
    //SPI_1
    {SPI_1, 22, 3}, {SPI_1, 33, 3}, {SPI_1, 37, 3}, {SPI_1, 48, 4}
};
...
_GetWclkClock()
{
    ...
    case 1: // From PLL1 output2
        reg_Val = ithReadRegA(0xD8000108);
        PLL_numerator = (reg_Val & 0x7F0000) >> 16;
        PLL_pre_divider = reg_Val & 0x1F;
        PLL_post_divider = (reg_Val & 0x7F00) >> 8;
        break;
    ...
    uint32_t    dataSize)
{
    ...
    ithDmaSetRequest(DmaSpiChannel, ITH_DMA_NORMAL_MODE, ITH_DMA_MEM, ITH_DMA_HW_HANDSHAKE_MODE, (port == SPI_0) ? ITH_DMA_SPI0_TX : ITH_DMA_SPI1_TX);
    ...
    uint32_t    outDataSize)
{
    ...
    ithDmaSetRequest(DmaSpiChannel, ITH_DMA_HW_HANDSHAKE_MODE, (port == SPI_0) ? ITH_DMA_SPI0_RX : ITH_DMA_SPI1_RX, ITH_DMA_NORMAL_MODE, ITH_DMA_MEM);
    ithDmaStart(DmaSpiChannel);                     // Fire DMA
