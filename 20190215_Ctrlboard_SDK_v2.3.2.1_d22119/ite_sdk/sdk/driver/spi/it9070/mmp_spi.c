    ...
    SPI_PORT port)
{
    ...
    case SPI_0:
        // SSP CLK SEL
        ithSetRegBitA(ITH_GPIO_BASE + ITH_GPIO_MISC_SET_REG, ITH_GPIO_SSP_CLK_SEL_POS);
        ithWriteRegMaskA(ITH_GPIO_BASE + ITH_GPIO_MISC_SET_REG, 6 << ITH_GPIO_STOR_SEL_POS, ITH_GPIO_STOR_SEL_MSK);
        break;

    case SPI_1:
        ithWriteRegMaskA(ITH_GPIO_BASE + ITH_GPIO_MISC_SET_REG, (0x4 << ITH_GPIO_HOST_SEL_POS), ITH_GPIO_HOST_SEL_MSK);
        ithSetRegBitA(ITH_GPIO_BASE + ITH_GPIO_MISC_SET_REG, ITH_GPIO_SSP1_MODE_POS);
        break;
    }
}
    ...
    uint8_t     dataLength)
{
    ...
    if (g_SpiContext[port].dma_ch >= 0)
    {
    ...
    #if defined(_WIN32)
        {
            ...
            ithFlushDCacheRange((void*)mappedSysRam, SSP_DMA_TEMPBUF);
            ithUnmapVram((void*)mappedSysRam, SSP_DMA_TEMPBUF);
        }
    #else
        {
            unsigned int i = 0;
    ...
    #ifdef __OPENRTOS__
        // Flush DC
        ithFlushDCacheRange((void*)tempBuf, SSP_DMA_TEMPBUF);
    #endif
...
//#if defined(_WIN32)
    #if 1
        {
            ...
            ithFlushDCacheRange((void*)mappedSysRam, dmaBufferSize);
            ithUnmapVram((void*)mappedSysRam, dmaBufferSize);
        }
    #else
        {
            unsigned int i = 0;
    ...
    #ifdef __OPENRTOS__
            // Flush DC
            ithFlushDCacheRange((void*)tempBuf, dmaBufferSize);
    #endif
    ...
    uint8_t     dataLength)
{
    ...
    #ifdef __OPENRTOS__
        // Flush DC
        ithFlushDCacheRange((void*)tempRxBuf, SSP_DMA_TEMPBUF);
    #endif
...
#ifdef __OPENRTOS__
    // Flush DC
    ithFlushDCacheRange((void*)inputData,  inputSize);
    ithFlushDCacheRange((void*)pbuf,       size);
#endif
...

{
    ...
    // Flush DC
    ithFlushDCacheRange((void*)inputData,  inputSize);
    ithFlushDCacheRange((void*)outputBuf,  size);
#endif
    ...
    int         size)
{
...

    {
        ...
        //GPIO9
        ithGpioSetMode(ITH_GPIO_PIN9, ITH_GPIO_MODE2);
        //MUDEX
        data    = ithReadRegA(ITH_GPIO_BASE + ITH_GPIO_MISC_SET_REG);
        //GPIO9
        data    &= 0x7;
        data    |= 0x0086;
        ithWriteRegA(ITH_GPIO_BASE + ITH_GPIO_MISC_SET_REG, data);
    }
 ...
 */
    ...
    #ifdef __OPENRTOS__
        // Flush DC
        ithFlushDCacheRange((void*)tx_buf, buflen);
    #endif
