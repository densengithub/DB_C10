...
#else
#define ITE_AUDIO_READ_REG          ithReadRegH
#define ITE_AUDIO_WRITE_REG         ithWriteRegH
#define ITE_AUDIO_WRITE_REG_MASK    ithWriteRegMaskH
#endif
...
#define HAVE_FLAC                     1
#define HAVE_SBC                      1
#define HAVE_MP2ENC               1
#define HAVE_AACENC               1
#define HAVE_BSAC                     0
#define HAVE_MIDI                     0
...
static uint32_t  AUD_nKeepI2SRP = 0;
...
// encode data
static uint8_t gEncodeFrame[MMP_AUDIO_ENCODE_SIZE];
static uint32_t  gEncodeTimeStamp=0; //ms
static uint32_t  gPreEncodeTime=0;
static uint32_t  gEncodeTimeAdding=0;
    ...
    uint32_t *status)
{
    ...
    for (i = 5; i > 0; i--)
    {
        ...
        ITE_AUDIO_READ_REG(RISC_PC,   &pcLowDataRd1);
        ITE_AUDIO_READ_REG(RISC_PC + 2, &pcHighDataRd1);
        ithDelay(2);
        ITE_AUDIO_READ_REG(RISC_PC,   &pcLowDataRd2);
        ITE_AUDIO_READ_REG(RISC_PC + 2, &pcHighDataRd2);
...
CHECK_IS_IDLE:
    for (i = 4; i > 0; i--)
    {
        ...
        ITE_AUDIO_READ_REG(DrvDecode_RdPtr, &decRdPtr1);
        ITE_AUDIO_READ_REG(DrvDecode_WrPtr, &encWrPtr1);
        ithDelay(40);
        ITE_AUDIO_READ_REG(DrvDecode_RdPtr, &decRdPtr2);
        ITE_AUDIO_READ_REG(DrvDecode_WrPtr, &encWrPtr2);
    ...
    uint32_t reverbAddress)
{
    ...
    {
        enReverb = 1;
        ITE_AUDIO_WRITE_REG(DrvDecode_RevbBase_Lo,   (reverbAddress) & 0xffff);
        ITE_AUDIO_WRITE_REG(DrvDecode_RevbBase_Hi, (reverbAddress) >> 16);
        //ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl, DrvDecode_EnReverb, 1<<4);
    }
    else
    {
        enReverb = 0;
        ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl, 0, 1 << 4);
    }
    return 0;
}
    ...
    int32_t enable)
{
    ...
    if (enable == 1)
    {
        ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl, 0xffff, DrvDecode_EnDRC);
    }
    else if (enEQ == 0 && enReverb == 0)
    {
        ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl, 0x0, DrvDecode_EnDRC);
    }
    return 0;
}
    ...
    bool enable)
{
    ...
    if (enable == 1)
    {
        ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl, DrvDecode_EnVoiceOff, 1 << 5 );
    }
    else
    {
        ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl, 0, 1 << 5 );
    }
    ...
    bool enableEq)
{
    ...
    if (enableEq == 1)
    {
        ...
        ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl, 0xffff, DrvDecode_EnEQ);
    }
    else
    {
        enEQ = AUD_eqStatus = 0;
        ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl, 0x0, DrvDecode_EnEQ);
    }
    ...
    uint32_t *rdBuflen)
{
    ...
    hwpr = ITE_AUDIO_READ_REG(DrvEncode_WrPtr);
    ...
    void)
{
    ...
    ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl, 0xffff, DrvDecode_EOF);
...
//    ITE_AUDIO_WRITE_REG(DrvMIDI_BASE, (uint16_t)(address & 0xFFFF));
//    ITE_AUDIO_WRITE_REG((DrvMIDI_BASE + 2), (uint16_t)(address >> 16));
//#endif
    ...
    void)
{
    ...
    if (set_I2C_data == 0)
    {
        ithDelay(10);
        //disable ACLK clock
        //ITE_AUDIO_WRITE_REG_MASK(0x32,0x0,0x0001);
        //ithDelay(0x10);

        /*I2S MMIO*/
        //ITE_AUDIO_WRITE_REG_MASK(0x210,0x1,0x0001);
        //ithDelay(0x10);
        /*I2S interrupt*/
        //ITE_AUDIO_WRITE_REG_MASK(0x6,0x1,0x0001);
        //ithDelay(0x10);
        set_I2C_data = 1;
    }
...
//        printf("[Audio Driver] codec streams ptr 0x%08x 0x%08x 0x%08x 0x%08x 0x%08x  0x%08x  0x%08x 0x%08x 0x%08x\n",__header->pStream,gAudioStream,streamAddress,gAudioStream->codecStreamBuf,gAudioStream->codecStreamLength,gAudioStream->codecStreamBuf,gAudioStream->codecStreamLength,gAudioStream->codecAudioPluginBuf,gAudioStream->codecAudioPluginLength);
...
#ifdef CFG_CPU_WB
        ithFlushDCacheRange((void*)gAudioStream, SHARE_MEM1_SIZE);
    ...
    case ITE_MP2_ENCODE:
        ...
        printf("[Audio Driver] mp2 audio plugin Audio_Plugin_Message_Buflen %d 0x%x 0x%x , %d , %d \n", Audio_Plugin_Message_Buflen, &Audio_Plugin_Message_Buflen, Audio_Plugin_Message_Buf,Audio_Plugin_Message_Buf[0],Audio_Plugin_Message_Buf[1]);
        //__header->codec_info(&Audio_Decode_Bufptr, &Audio_Decode_Buflen, &AUD_freqAddress, &AUD_curPlayPos ,&Audio_Plugin_Message_Buf,&Audio_Plugin_Message_Buflen);
        Audio_Encode_Bufptr  = TOINT(gAudioStream->codecStreamBuf) + CODEC_BASE;
        Audio_Encode_Buflen  = TOINT(gAudioStream->codecStreamLength);
pTemp = Audio_Encode_Bufptr;
        ITE_AUDIO_WRITE_REG(I2S_AD_SET_RD_PTR, 0);
        printf("[Audio Driver] Audio_Encode_Buf 0x%08x %d , Audio_Encode_Buflen %d  \n", Audio_Encode_Bufptr,pTemp[0], Audio_Encode_Buflen);
    #else
        __header->codec_info(&Audio_Decode_Bufptr, &Audio_Decode_Buflen, &AUD_freqAddress, &AUD_curPlayPos);
    #endif
        Audio_Decode_Buflen1 = Audio_Decode_Buflen2 = Audio_Decode_Buflen;
...
#if HAVE_AACENC
    #if defined(ENABLE_AUDIO_PROCESSOR) && defined(AUDIO_PLUGIN_MESSAGE_QUEUE)
        printf("[Audio Driver] aac enc audio plugin Audio_Plugin_Message_Buflen %d 0x%x 0x%x\n", Audio_Plugin_Message_Buflen, &Audio_Plugin_Message_Buflen, Audio_Plugin_Message_Buf);
        //__header->codec_info(&Audio_Decode_Bufptr, &Audio_Decode_Buflen, &AUD_freqAddress, &AUD_curPlayPos ,&Audio_Plugin_Message_Buf,&Audio_Plugin_Message_Buflen);
        Audio_Decode_Bufptr  = TOINT(gAudioStream->codecStreamBuf) + CODEC_BASE;
        Audio_Decode_Buflen  = TOINT(gAudioStream->codecStreamLength);
        Audio_Decode_Bufptr1 = TOINT(gAudioStream->codecStreamBuf1) + CODEC_BASE;
    ...
    void)
{
    ...
    // 1. write enable bit
    ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl, 0xffff, DrvEnable);

    // 2. fire engine
    //ITE_AUDIO_WRITE_REG(RISC_FIRE, 0x1);
    //ITE_AUDIO_WRITE_REG(RISC_FIRE, 0x0);    
    iteRiscFireCpu(RISC1_CPU);
...
int32_t AUD_ResetUserDefineIO3(
    void)
{
    ITE_AUDIO_WRITE_REG(DrvAudioCtrl2,  0);
    return 1;
}
    ...
    unsigned long *bufferLength)
{
    ...
    case 1:
        ...
        hwpr = ITE_AUDIO_READ_REG(DrvDecode_RdPtr1);
        //hwpr = ((hwpr >> 1) << 1); // word align
        // Get the avalible length of buffer
        if (swpr == hwpr)
        {
    ...
    case 2:
        ...
        hwpr = ITE_AUDIO_READ_REG(DrvDecode_RdPtr2);
        //hwpr = ((hwpr >> 1) << 1); // word align
        // Get the avalible length of buffer
        if (swpr == hwpr)
        {
...
//printf("readableQueueLen %d bottomLen %d Audio_Encode_Rdptr %d \n",  readableQueueLen,bottomLen,Audio_Encode_Rdptr);
    ...
    ITE_AUDIO_WRITE_REG(DrvEncode_RdPtr, Audio_Encode_Rdptr);
    ...
    unsigned long length)
{
    ...
    while (residualLength)
    {
        ...
        {
        case 0:
            ITE_AUDIO_WRITE_REG(DrvDecode_WrPtr, nTempWrptr);
        ...
        case 1:
            ITE_AUDIO_WRITE_REG(DrvDecode_WrPtr1, nTempWrptr);
        ...
        case 2:
            ITE_AUDIO_WRITE_REG(DrvDecode_WrPtr2, nTempWrptr);
...
#endif
#ifdef CFG_AUDIO_CODEC_MP2ENC
    { "mp2",    ITE_MP2_ENCODE,  ADDR_AND_SIZE(gMp2Encoder) },
#endif
#ifdef CFG_AUDIO_CODEC_AACENC
    { "aacenc",    ITE_AAC_ENCODE,  ADDR_AND_SIZE(gAacEncoder) },
#endif
...
#ifdef ENABLE_AUDIO_PROCESSOR    
    if(AUD_Init) AUD_ResetAudioProcessor(); //HX stop Audioprocessor
#endif
    ...
    void)
{
    ...
    //    ITE_AUDIO_READ_REG(RISC_PC, &regdatalow);
    //    ITE_AUDIO_READ_REG(RISC_PC + 2, &regdatahigh);
    ...
    void *value)
{
    ...
    case ITE_AUDIO_I2S_OUT_FULL:
        AUD_bI2SOutFull = 0;
#if 0
        data            = ITE_AUDIO_READ_REG(MMIO_I2S_OUT_BUF_LEN);
        I2SBufLen       = (uint32_t)data;
        data            = ITE_AUDIO_READ_REG(MMIO_I2S_OUT_BUF_RDPTR);
        I2SRdPtr        = (uint32_t)data;
        data            = ITE_AUDIO_READ_REG(MMIO_I2S_OUT_BUF_WRPTR);
    ...
    case ITE_AUDIO_IS_PAUSE:
        AUD_bIsPause           = 0;
        data                   = ITE_AUDIO_READ_REG(DrvAudioCtrl);
    ...
    uint32_t *time)
{
    ...
    data     = ITE_AUDIO_READ_REG(DrvEncode_Frame_Hi);
    (*time)  = (uint32_t)data * 1000;

    data     = ITE_AUDIO_READ_REG(DrvEncode_Frame_Lo);
    (*time) += ((uint32_t)(data * 1000) >> 16);
    ...
    uint32_t pts)
{
    ...
    swpr    = ITE_AUDIO_READ_REG(0x16AE);
    pts_low = ITE_AUDIO_READ_REG(0x16B0);
    pts_hi  = ITE_AUDIO_READ_REG(0x16B2);

    if (swpr == 0 && pts_low == 0 && pts_hi == 0)
    {
        uint16_t regdata;
        regdata = ITE_AUDIO_READ_REG(DrvDecode_WrPtr);
        ITE_AUDIO_WRITE_REG(0x16AE, regdata);
        ITE_AUDIO_WRITE_REG(0x16B0, (uint16_t)(pts & 0xFFFFL));
        ITE_AUDIO_WRITE_REG(0x16B2, (uint16_t)(pts >> 16));
        result = 0;
    }
    bPtsSync = 1;
...
iteAudioSetOutputEmpty(
    int32_t nEnable)
{
    ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl, (!!nEnable) << DrvDecode_EnOutputEmpty_Bits, DrvDecode_EnOutputEmpty);
}
...
iteAudioSetWmaWithoutASFHeader(
    int32_t nEnable)
{
    ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl2, (!!nEnable) << DrvWMAWithoutASFHeader_Bits, DrvWMAWithoutASFHeader);
}
...
iteAudioSetMusicShowSpectrum (
    int32_t nEnable)
{
    ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl2, (!!nEnable) << DrvShowSpectrumMode_Bits, DrvShowSpectrum);
}
...
iteAudioSetMp3RdBufPointer (
    int32_t nReset)
{
    ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl2, (!!nReset) << DrvResetMp3RdBufPointer_Bits, DrvResetMp3RdBufPointer);
}
...
iteAudioSetMusicWithoutASFHeader(
    int32_t nEnable)
{
    ITE_AUDIO_WRITE_REG_MASK(DrvAudioCtrl2, (!!nEnable) << DrvMusicWithoutASFHeader_Bits, DrvMusicWithoutASFHeader);
}
    ...
    uint16_t volume_status)
{
    ...
    {
        //  not nSuspend
        ITE_AUDIO_WRITE_REG(0x3A, 0xB000);
        ITE_AUDIO_WRITE_REG(0x3C, 0x800F);
        ITE_AUDIO_WRITE_REG(0x3E, 0x00AA);

        ITE_AUDIO_WRITE_REG(0x1640, 0x000F);
        ITE_AUDIO_WRITE_REG(0x1642, 0x400F);
        ITE_AUDIO_WRITE_REG(0x1644, 0x3003);
        ithDelay(10);

        ITE_AUDIO_WRITE_REG_MASK(0x1672, 0xF5F7, 0xffff);                // Mute

        // 166E ==> discharge
        ITE_AUDIO_WRITE_REG_MASK(0x1670, 0x00C0, 0xFFFF);               // AMP power on
        ithDelay(50);                                           // wait 100ms
        ITE_AUDIO_WRITE_REG_MASK(0x1670, 0x0000, 0xFFFF);               // DACIP power on
        ithDelay(50);                                           // wait 100ms
        ITE_AUDIO_WRITE_REG_MASK(0x166e, 0x0000, 0xFFFF);               // AMP charge
        ithDelay(100);
        ...
        for (i = 15; i >= value; i--)
        {
            ITE_AUDIO_WRITE_REG_MASK(0x1672, ((i << 12) | (volume_status & 0xFFF)), 0xFFFF);
            ithDelay(1);
    ...
    {   // nSuspend
        ...
        ITE_AUDIO_WRITE_REG_MASK(0x1672, 0xF5F7, 0xffff);       // Mute
        ITE_AUDIO_WRITE_REG_MASK(0x166e, 0x1000, 0x1000);       // AMP discharge
        // hw need sleep 100 ms
        ithDelay(100);                                  // wait 100 ms
        ITE_AUDIO_WRITE_REG_MASK(0x1670, 0x00C0, 0x00C0);       // DACIP power down
        ithDelay(10);                                   // wait 10ms
        ITE_AUDIO_WRITE_REG_MASK(0x1670, 0x10C0, 0x10C0);       // AMP power down
    }
}
...
{
    uint16_t nRegData;
    nRegData = ITE_AUDIO_READ_REG(0x0);
...
{
    uint16_t regdata;
    regdata = ITE_AUDIO_READ_REG(AC3_DECODER_PARAMETER);
...
iteAudioSetAC3DrcMode(int32_t mode)
{
    ...
    ITE_AUDIO_WRITE_REG_MASK(AC3_DECODER_PARAMETER, (mode << 0), (3 << 0));
}
...
iteAudioSetAC3DualMonoMode(int32_t mode)
{
    ...
    ITE_AUDIO_WRITE_REG_MASK(AC3_DECODER_PARAMETER, (mode << 2), (3 << 2));
}
...
iteAudioSetAC32ChDownmixMode(int32_t mode)
{
    ...
    ITE_AUDIO_WRITE_REG_MASK(AC3_DECODER_PARAMETER, (mode << 4), (3 << 4));
}
...
iteAudioSetAC3PrintMetaDataMode(int32_t mode)
{
    ...
    ITE_AUDIO_WRITE_REG_MASK(AC3_DECODER_PARAMETER, (mode << 6), (1 << 6));
}
...
iteAudioGetFlashInputStatus(int nInputNumber, int *nFormat, int *nUsing)
{
    ...
    regdata = ITE_AUDIO_READ_REG(FLASH_AUDIO_INPUT_PARAMETER);
    switch (nInputNumber)
    {
    case 0:
...
iteAudioSetFlashInputStatus(int nInputNumber, int nFormat, int nUsing)
{
    ...
    case 0:
        ...
        ITE_AUDIO_WRITE_REG_MASK(FLASH_AUDIO_INPUT_PARAMETER, regdata, regdata);
        break;
    ...
    temp = ITE_AUDIO_READ_REG(FLASH_AUDIO_INPUT_PARAMETER);
...
int iteAudioSetEncodeParam(int nCh,int nSamplerate)
{
    ...
    ITE_AUDIO_WRITE_REG(AUDIO_ENCODE_PARAMETER, command);
...
int iteAudioUpdateI2SADStatus(unsigned short *pRp,unsigned short *pWp)
{
    ...
    ITE_AUDIO_WRITE_REG(I2S_AD_GET_RD_PTR, (unsigned short) rp);
    ITE_AUDIO_WRITE_REG(I2S_AD_GET_WR_PTR, (unsigned short) wp);

    newRp = ITE_AUDIO_READ_REG(I2S_AD_SET_RD_PTR);
    //printf(" newRp %d , AUD_nKeepI2SRP %d  \n",newRp,AUD_nKeepI2SRP);
    
    if (AUD_nKeepI2SRP != newRp){
        
        I2S_AD32_SET_RP(newRp);
        AUD_nKeepI2SRP = newRp;
        //printf("iteAudioUpdateI2SADStatus rp %d \n",AUD_nKeepI2SRP);
    }
    *pRp = ITE_AUDIO_READ_REG(I2S_AD_GET_RD_PTR);
    *pWp = ITE_AUDIO_READ_REG(I2S_AD_GET_WR_PTR);    
}

// return 0 : no encoded data, return 1:get encode data
int
iteAudioGetEncodeData(AUDIO_ENCODE_DATA* ptEncodeData)
{
    int nResult;
    int bufEncSize=0;

    int  streamBufferSize = MMP_AUDIO_ENCODE_SIZE;
    unsigned int* pBuf;


    AUD_GetReadAvailableBufferLength(&bufEncSize);

//printf("iteAudioGetEncodeData %d \n",bufEncSize);
...
#endif
        nResult = iteAudioReadStream((uint8_t*) gEncodeFrame, streamBufferSize,streamBufferSize);
        if (nResult == 0)
        {
            return 0;
        }
...
//
    gPreEncodeTime = TOINT(pBuf[0]);
    //printf("iteAudioGetEncodeData gPreEncodeTime %d \n",gPreEncodeTime);
    gPreEncodeTime = gPreEncodeTime/1000;
    gEncodeTimeAdding +=  TOINT(pBuf[0])-(gPreEncodeTime*1000);
    ...
    ptEncodeData->nDataSize = TOINT(pBuf[1]);
//printf("iteAudioGetEncodeData ptEncodeData->nDataSize %d \n",ptEncodeData->nDataSize);
        
        //ptEncodeData->nDataSize = pBuf[1];
        gEncodeTimeStamp+=gPreEncodeTime;

        //printf("encode gPreEncodeTime %d datasize %d \n",gPreEncodeTime,ptEncodeData->nDataSize);
    ...
    unsigned int *value)
{
    ...
    do
    {
        if (ITE_AUDIO_READ_REG(AEC_COMMAND_SEND) == 0 &&
            ITE_AUDIO_READ_REG(AEC_COMMAND_REPLY) == 0)
            break;
        if (timeout_count == 0)
        {
            printf("aec command process timeout, cmd: %d, reg cmd: %d, reg reply: %d\n",
                   command, ITE_AUDIO_READ_REG(AEC_COMMAND_SEND), ITE_AUDIO_READ_REG(AEC_COMMAND_REPLY));
            return result;
        }
        else
            timeout_count--;
    ...
    do
    {
        if (ITE_AUDIO_READ_REG(AEC_COMMAND_REPLY) == command &&
            ITE_AUDIO_READ_REG(AEC_COMMAND_SEND) == 0)
            break;
        if (timeout_count == 0)
        {
