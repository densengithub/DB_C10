...
#ifdef CFG_NOR_PARTITION3
    if (disk == ITP_DISK_NOR && partitions[3].secnum == 0)
    {
        f = fopen("A:/" ITP_FAT_EXBR_FILE, "rb");
        if (!f)
        {
            LOG_ERR "fopen fail\n" LOG_END
        }
        else
        {
            ...
            if (NorWrite(SPI_0, SPI_CSN_0, (uint32_t)exbr_start, fat_exbr + 8, ITP_FAT_EXBR_SIZE - 8))
            {
                LOG_ERR "fwrite fail\n" LOG_END
            }
        }
...
#define F_DEF_PART_ALIGN 16             /* default partition alignment */
    ...
    for (;;)
    {
        ...
        if (ret == F_ERR_MEDIATOOSMALL)
        {
            if (partitions[par->count - 1].secnum > F_DEF_PART_ALIGN)
            {
                ...
                if (par->count > 3)
                {
                    par->exbr_start = par->start[par->count - 1];
                    par->exbr_start += (phy.number_of_sectors - secnum - partitions[par->count - 1].secnum - F_DEF_PART_ALIGN) * phy.bytes_per_sector;
                }
