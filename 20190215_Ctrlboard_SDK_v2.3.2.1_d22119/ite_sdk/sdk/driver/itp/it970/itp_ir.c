...
#endif
        transferSize = ithReadRegA(ITH_DMA_BASE+ITH_DMA_SIZE_CH(RxChannel));
    ...
    while(1)
    {
        if(g_IR_TX_HAS_INIT)
        {
            ...
            if(xQueueReceive(irTxQueue, &code, 0))
            {
                ...
                ithDmaSetRequest(TxChannel, ITH_DMA_NORMAL_MODE, ITH_DMA_MEM, ITH_DMA_HW_HANDSHAKE_MODE, ITH_DMA_IR_CAP_TX);
...
static void IrInit(void)
{
    ...
    ithIntrDisableIrq(ITH_INTR_IRRX);
    ithIntrClearIrq(ITH_INTR_IRRX);
    ithIntrRegisterHandlerIrq(ITH_INTR_IRRX, IrIntrHandler, NULL);
    ithIntrEnableIrq(ITH_INTR_IRRX);

    ithIrCtrlEnable(ITH_IR_INT);
    ithIrIntrCtrlEnable(ITH_IR_DATA);

	ithIrRxSetModFilter(0x535, 0xFA0); //20~60k freq.(80M/freq.)
	//ithIrRxSetModFilter(0xCE4, 0x26AC); //20~60k (WCLK/freq.), WCLK=198MHz Minfreq=20KHz Maxfreq=60KHz
...

    {
        ...
        if(RxBuffer == NULL || TxBuffer == NULL) {
            ...
            ithDmaSetRequest(RxChannel, ITH_DMA_HW_HANDSHAKE_MODE, ITH_DMA_IR_CAP_RX, ITH_DMA_NORMAL_MODE, ITH_DMA_MEM);
...

    {
        ...
        ithIrTxSetModFreq(0x13D5);  //39kHz (WCLK/SampleRate), WCLK=198MHz SampleRate=39KHz
