...
static uint32_t sd0_offset = CFG_SD0_RESERVED_SIZE/512;
#else
static uint32_t sd0_offset = 0;
#endif
#endif

#if defined(CFG_SD1_ENABLE)
#if defined(CFG_SD1_RESERVED_SIZE)
static uint32_t sd1_offset = CFG_SD1_RESERVED_SIZE/512;
#else
static uint32_t sd1_offset = 0;
#endif
#endif
...
#if defined(CFG_SD0_ENABLE)
	case ITP_DISK_SD0:
		rc = iteSdReadMultipleSectorEx(SD_0, blockId+sd0_offset, sizeInSector, buf);
		break;
#endif // CFG_SD0_ENABLE

#if defined(CFG_SD1_ENABLE)
	case ITP_DISK_SD1:
		rc = iteSdReadMultipleSectorEx(SD_1, blockId+sd1_offset, sizeInSector, buf);
		break;
#endif // CFG_SD1_ENABLE
...
#if defined(CFG_SD0_ENABLE)
	case ITP_DISK_SD0:
		rc = iteSdWriteMultipleSectorEx(SD_0, blockId+sd0_offset, sizeInSector, buf);
		break;
#endif // CFG_SD0_ENABLE

#if defined(CFG_SD1_ENABLE)
	case ITP_DISK_SD1:
		rc = iteSdWriteMultipleSectorEx(SD_1, blockId+sd1_offset, sizeInSector, buf);
		break;
#endif // CFG_SD1_ENABLE
