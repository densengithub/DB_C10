...
/* =============================================================================
   ...
   Purpose:		Signal handler for fatal signals (SIGBUS, SIGSEGV)
   ============================================================================= */
...
/* =============================================================================
   ...
   ============================================================================= */
#ifdef WITH_FLAGS
static void *SetBlk(void *Blk, size_t size, const char *file, void* ptr, unsigned flags)
#else
static void *SetBlk(void *Blk, size_t size, const char *file, void* ptr)
#endif
{
...
void *Rmalloc(size_t size, const char *file)
{
  ...
  if (ret) {
    /* initialize */
#ifdef WITH_FLAGS
    return SetBlk(ret, size, file, ret, 0);
#else
    return SetBlk(ret, size, file, ret);
#endif
  }
  else {
    fprintf(stderr,
	    HEAD "WARNING: Out of memory! Returning NULL, %d Bytes (in %s)\n", file, size);
    return NULL;
  }
}
...
void *Rmemalign(size_t align, size_t size, const char *file)
{
  uint8_t *ret;			/* ret val */
  uint8_t* ptr;
  const size_t totalsize = align + size + EXTRA_SPACE;

  if (size == 0) {
    fprintf(stderr, HEAD "WARNING: memalign() demands 0 Bytes (in %s)\n", file);
  }

  ptr = malloc(totalsize); /* get extended block  */
  if (ptr) {
    size_t offset;
    ret = ptr + START_SPACE;
    offset = align - (((size_t)ret) % align);
    ret += offset;
    ret -= START_SPACE;

    /* initialize */
#ifdef WITH_FLAGS
    return SetBlk(ret, size, file, ptr, 0);
#else
    return SetBlk(ret, size, file, ptr);
#endif
  }
  else {
    fprintf(stderr,
...
void *Rcalloc(size_t nelem, size_t size, const char *file)
{
  ...
  /* check for overflow here (patch from Fr�d�ric Culot) */
...
#ifdef WITH_FLAGS
    return SetBlk(ret, size, file, ret, flags);
#else
    return SetBlk(ret, size, file, ret);
#endif
  }
...
void Rfree(void *p, const char *file)
{
  ...
  /* free block */
  {
    begin* pp = (begin *)(((char *)p)-START_SPACE);
    free(pp->Ptr);
  }
}
...
char *Rstrndup(const char *s, size_t n, const char *file)
{
  size_t size;	/* needed memory */
  char *ret;
  ...
  if (ret) {
    ...
    memcpy(ret, s, size);
#ifdef WITH_FLAGS
    Rmalloc_set_flags(ret, RM_STRING, "<by strndup>");
#endif
    return ret;
  }
  else {
