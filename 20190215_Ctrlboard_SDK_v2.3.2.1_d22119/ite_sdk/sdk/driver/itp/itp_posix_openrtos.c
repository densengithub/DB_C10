...
void _exit(int status)
{
    ...
    if (status != 0)
    {
        ...
        // memory debug
        #if defined(CFG_MEMDBG_ENABLE) && (CFG_CHIP_FAMILY == 9850)
        {
...
int timer_settime(timer_t timerid, int flags, const struct itimerspec *value, struct itimerspec *ovalue)
{
    ...
    if (t->pxTimer && (uxAutoReload == pdFALSE))
    {
        ...
        {
            LOG_WARN "timer_settime(%d, %d, 0x%X, 0x%X) fail\n", timerid, flags, value, ovalue LOG_END
...
ssize_t mq_receive(mqd_t msgid, char *msg, size_t msg_len, unsigned int *msg_prio)
{
    ...
    {
        errno = EINVAL;
        // ithPrintf("mq_receive() fail (-1)....\n");
        return -1;
    }
    // ithPrintf("mq_receive() sucess (%d)...\n", q->uxItemSize);
    return q->uxItemSize;
}
...
size_t itpFread(void* buf, size_t size, size_t count, FILE* fp)
{
    ...
    if (dev)
    {
        /*  Pass call on to device driver.  Return result.        */
        if (size > 0)
            return dev->read(fp->_file & ITP_FILE_MASK, (char*)buf, size * count, dev->info) / size;
...
size_t itpFwrite(const void* buf, size_t size, size_t count, FILE* fp)
{
    ...
    if (dev)
    {
        /*  Pass call on to device driver.  Return result.        */
        if (size > 0)
            return dev->write(fp->_file & ITP_FILE_MASK, (char*)buf, size * count, dev->info) / size;
