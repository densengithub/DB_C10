...
struct dirent *readdir(DIR *dirp)
{
    ...
    if (dirp->dd_size == sizeof(struct _wfinddata_t))
    {
        ...
        else
        {
            ...
            intptr_t ret;
            char *dirname = &dirp->dd_buf[sizeof(struct _wfinddata_t) + sizeof(struct dirent)];
    ...
    else
    {
        ...
        else
        {
            char               buf[PATH_MAX + 1];
            intptr_t ret;
            char *dirname = &dirp->dd_buf[sizeof(struct _finddata_t) + sizeof(struct dirent)];
...
#if CFG_MMAP_SIZE > 0
    static uint8_t mmapPool[CFG_MMAP_SIZE];
    static unsigned int __mmap_top__ = 0;
#endif // CFG_MMAP_SIZE > 0

void *mmap(void *addr, size_t len, int prot, int flags, int fildes, off_t off)
{
#if CFG_MMAP_SIZE > 0
    if (__mmap_top__ > CFG_MMAP_SIZE)
        return MAP_FAILED;

    unsigned int ret = (unsigned int)mmapPool + __mmap_top__;
    __mmap_top__ += ((len + 31) & (~(0x1F)));
    if (__mmap_top__ > CFG_MMAP_SIZE || __mmap_top__ < 0)
        return MAP_FAILED;
    else
        return ((void *)ret);
#else
...
{
#if CFG_MMAP_SIZE > 0
    if ((uint8_t*)addr - mmapPool == __mmap_top__)
    {
        __mmap_top__ -= len;
        return 0;
    }
