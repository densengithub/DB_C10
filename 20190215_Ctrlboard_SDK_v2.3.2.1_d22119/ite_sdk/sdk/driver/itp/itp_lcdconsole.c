...
static void lcd_drawchars (unsigned short x, unsigned short y, unsigned char *str, int count, char *lcd_console_address)
{
	...
	dest = (unsigned char *)(lcd_console_address + y * lcd_line_length + x * lcd_bpp);
...
static void console_scrollup (void)
{
	...
	/* Clear the last one */
	memset(lcd_base_a + CONSOLE_SIZE - CONSOLE_ROW_SIZE, lcd_color_bg, CONSOLE_ROW_SIZE);
	memset(lcd_base_b + CONSOLE_SIZE - CONSOLE_ROW_SIZE, lcd_color_bg, CONSOLE_ROW_SIZE);
	
#ifdef DRAW_VIDEO_BUF
	memcpy (lcd_base_c, lcd_base_c + CONSOLE_ROW_SIZE, CONSOLE_SCROLL_SIZE);
	memset(lcd_base_c + CONSOLE_SIZE - CONSOLE_ROW_SIZE, lcd_color_bg, CONSOLE_ROW_SIZE);
#endif
}
...
static int LcdConsolePutchar(int c)
{
    ...
    if (c == '\n')
    {
        ithFlushDCacheRange((void*)lcd_base_a, lcd_line_length * ithLcdGetHeight());
        ithFlushDCacheRange((void*)lcd_base_b, lcd_line_length * ithLcdGetHeight());
    #ifdef DRAW_VIDEO_BUF        
        ithFlushDCacheRange((void*)lcd_base_c, lcd_line_length * ithLcdGetHeight());
    #endif
    }
    return c;
}
...
void LcdConsoleInit(void)
{
    ...
    lcd_color_bg	= 0;
	lcd_bpp         = (ithLcdGetFormat() == ITH_LCD_ARGB8888) ? 4 : 2;
	console_col		= 0;
	console_row		= 1;
...
void LcdUserDisplayString(int ptr)
{
	int len = 0,i = 0;
	char buffer[50] = {0};
	char buffer1[] = "Downloading:";
	char buffer2[10] = {0};

	console_col		= 1;
	console_row		= CONSOLE_ROWS-1;	

	sprintf(buffer2, "%d", ptr);
...
void LcdConsoleClear(void)
{
	...
	memset(lcd_base_a, lcd_color_bg, lcd_line_length * ithLcdGetHeight());
    ithFlushDCacheRange((void*)lcd_base_a, lcd_line_length * ithLcdGetHeight());

	memset(lcd_base_b, lcd_color_bg, lcd_line_length * ithLcdGetHeight());
    ithFlushDCacheRange((void*)lcd_base_b, lcd_line_length * ithLcdGetHeight());

#ifdef DRAW_VIDEO_BUF
	memset(lcd_base_c, lcd_color_bg, lcd_line_length * ithLcdGetHeight());
    ithFlushDCacheRange((void*)lcd_base_c, lcd_line_length * ithLcdGetHeight());
...
static int LcdConsoleIoctl(int file, unsigned long request, void* ptr, void* info)
{
	...
	case ITP_IOCTL_CUSTOM_INIT:
		LcdConsoleUserInit();
		break;
		
	case ITP_IOCTL_CUSTOM_UPGRADE:
		LcdUserDisplayString((int)ptr);
        break;	
