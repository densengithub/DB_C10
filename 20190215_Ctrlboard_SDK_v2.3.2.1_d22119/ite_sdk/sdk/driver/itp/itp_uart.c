...
static int UartPutchar(int c)
{
    ...
    if (ithGetCpuMode() == ITH_CPU_SYS)
    {
        ...
        if (uartPutcharPort == ITH_UART0)
            ...
            uartobj = &Uartobj[2];
		else if (uartPutcharPort == ITH_UART3)
			uartobj = &Uartobj[3];
		else if (uartPutcharPort == ITH_UART4)
			uartobj = &Uartobj[4];
		else
			uartobj = &Uartobj[5];
...
#elif defined(CFG_UART_DMA)
    // ithPrintf("boot_mode:%d\n",boot_mode);
    if (!boot_mode)
	{
	    uint32_t transferSize = 0;
	    uint32_t dataSize = 0;

		transferSize = ithReadRegA(ITH_DMA_BASE+ITH_DMA_SIZE_CH(uartobj->readDmaChannel));
...
static void UartStopDMA(ITHUartPort port)
{
	UART_OBJ *uartobj = NULL;
