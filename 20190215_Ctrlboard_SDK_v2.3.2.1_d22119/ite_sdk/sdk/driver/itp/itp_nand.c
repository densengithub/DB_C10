...
static int NandRead(int file, char *ptr, int len, void* info)
{
    ...
    if(databuf==NULL)
    {
   	...
   	for(blk=itpNfInfo.CurBlk; blk<areaBoundary; blk++)
   	{
        ...
        if(blk)
        {
            memcpy(srcbuf, databuf + gBlkGap, itpNfInfo.PageInBlk * itpNfInfo.PageSize - gBlkGap);
...
static int NandWrite(int file, char *ptr, int len, void* info)
{
    ...
    memset((uint8_t*)SprBuf, 0xFF, itpNfInfo.SprSize);
   	...
   	for(blk=itpNfInfo.CurBlk; blk<areaBoundary; blk++)
   	{
        ...
        if(blk)
        {
            memcpy(DataBuf, GoodBlkCodeMark, gBlkGap);
            memcpy(DataBuf + gBlkGap, tmpSrcBuf, itpNfInfo.PageInBlk * itpNfInfo.PageSize - gBlkGap);
        }
        else
        {
...
static int NandIoctl(int file, unsigned long request, void* ptr, void* info)
{
    ...
    switch (request)
    {
	    case ITP_IOCTL_INIT:
	    	#ifdef	CFG_SPI_NAND
			#ifdef CFG_AXISPI_ENABLE
			res = mmpAxiSpiInitialize(NAND_SPI_PORT, SPI_OP_MASTR, CPO_0_CPH_0, NAND_SPI_CLOCK, SPI_CLK_DIV_0);
			#else
	    	mmpSpiInitialize(NAND_SPI_PORT, SPI_OP_MASTR, CPO_0_CPH_0, NAND_SPI_CLOCK);
			#endif
	    	#endif
	        ...
	        if( iteNfInitialize(&itpNfInfo)==true )
	        {
	        	...
	        	{
	        	    char *pBuf;
	        	    pBuf = malloc(itpNfInfo.PageSize + 512);
    	        	if(pBuf!=NULL)
    	        	{
