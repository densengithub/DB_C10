...
#if (CFG_CHIP_FAMILY == 970)
    if ((ITH_READ_REG_A(ITH_HOST_BASE + ITH_REVISION_ID_REG) & 0xFFF) == 0) {
        //Workaround for HW IIC FIFO mode issue, only for IT970 Demo version
        ithSetBusClockRatioMultiple(powerBusRatio, 6);
    }
    else {
...
SleepFunc(register uint32_t *inParam, register uint32_t *outParam, register uint32_t *var)
{
    ...
    ITH_WRITE_REG_MASK_A(ITH_PLL1_2_SET2_REG,
	...
	ITH_WRITE_REG_MASK_A(ITH_PLL3_2_SET2_REG,
                         (0x0 << ITH_PLL_SET2_EN_BIT) | (0x1 << ITH_PLL_SET2_UPD_BIT),
                         ITH_PLL_SET2_EN_MSK | ITH_PLL_SET2_UPD_MSK);

	ITH_WRITE_REG_MASK_A(ITH_PLL3_3_SET2_REG,
                         (0x0 << ITH_PLL_SET2_EN_BIT) | (0x1 << ITH_PLL_SET2_UPD_BIT),
                         ITH_PLL_SET2_EN_MSK | ITH_PLL_SET2_UPD_MSK);

#if (CFG_CHIP_FAMILY == 970)
	//power down video power domain	
	if ((ITH_READ_REG_A(ITH_HOST_BASE + ITH_REVISION_ID_REG) & 0xFFF) != 0)
	{
	    ITH_WRITE_REG_A(0xD8400004, 0x1);
	    ITH_WRITE_REG_A(0xD8400000, 0x301);
	    do {
	    	VAL = ITH_READ_REG_A(0xD8400024);
	    }while(!(VAL&0x4));
  }
#endif	
...
#if (CFG_CHIP_FAMILY == 970)
   // power up H264 video power domain
   if ((ITH_READ_REG_A(ITH_HOST_BASE + ITH_REVISION_ID_REG) & 0xFFF) != 0)
   {
       ITH_WRITE_REG_A(0xD8400000, 0x10300);
       do {
       	VAL = ITH_READ_REG_A(0xD8400024);
       }while(!(VAL&0x1));
   }
