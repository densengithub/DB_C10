...
static int add_tasklist(int task_id)
{
    ...
    snprintf(g_task_name[task_id], configMAX_TASK_NAME_LEN+5, "%04d_%s", task_id, pcTaskGetName(xTaskGetCurrentTaskHandle()));
...
void portTASK_CREATE(TaskHandle_t xTask)
{
    ...
    snprintf(g_task_name[task_id], configMAX_TASK_NAME_LEN+5, "%04d_%s", task_id, pcTaskGetName(xTask));
    pch = g_task_name[task_id];
