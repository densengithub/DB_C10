...
ithCapHWICInit(void)
{
   ...
   ithWriteRegMaskA(_GetAddress(1, CAP_SCALING_SETTING_REGISTER0), 0x40, 0x1FFF);//assign nonzero value cap1
   usleep(1000);
...
{
    #ifndef CFG_CHIP_REV_A0
    ithWriteRegMaskA(_GetAddress(0x0, CAP_CAPTURE_OUTPUT_SETTING) , 0x0, (BIT8));
    ithWriteRegMaskA(_GetAddress(0x1, CAP_CAPTURE_OUTPUT_SETTING) , 0x0, (BIT8));
    #endif
...
{
    #ifndef CFG_CHIP_REV_A0
    ithWriteRegMaskA(_GetAddress(DEV_ID, CAP_CAPTURE_OUTPUT_SETTING) , 0x1 << 8, (BIT8));
    #endif
...
{
    #ifndef CFG_CHIP_REV_A0
    ithWriteRegMaskA(_GetAddress(DEV_ID, CAP_CAPTURE_OUTPUT_SETTING) , 0x0, (BIT8));
    #endif
    ...
    CAPTURE_DEV_ID DEV_ID, CAP_ENFUN_INFO *pFunEn)
{
    ...
    ithWriteRegMaskA(_GetAddress(DEV_ID, CAP_COLOR_SPACE_CONVERSION_SETTING_REGISTER0),  ((pFunEn->EnCSFun | pFunEn->EnCCFun) << 24),    (0x1 << 24));
    ...
    CAPTURE_DEV_ID DEV_ID, const CAP_COLOR_CORRECTION *pColorCorrect)
{
    ...
    ithWriteRegA(_GetAddress(DEV_ID, CAP_COLOR_SPACE_CONVERSION_SETTING_REGISTER6), value);
}
    ...
    CAPTURE_DEV_ID DEV_ID, CAP_LANE_STATUS lanenum)
{
    ...
    case CAP_LANE1_STATUS:
        data    = ithReadRegA(_GetAddress(DEV_ID, CAP_ENGINE_STATUS_REGISTER1));
        break;
    ...
    CAPTURE_DEV_ID DEV_ID)
{
    ...
    data    = ithReadRegA(_GetAddress(DEV_ID, CAP_ENGINE_STATUS_REGISTER0));
    ithPrintf(  "Capture status =0x%x\n",                   data);

    data    = ithReadRegA(_GetAddress(DEV_ID, CAP_ENGINE_STATUS_REGISTER1));
    ithPrintf(  "HTotal = 0x%x ,VTotal = 0x%x\n",           data & 0x00000FFF,  (data & 0x1FFF0000) >> 16);

    data    = ithReadRegA(_GetAddress(DEV_ID, CAP_ENGINE_STATUS_REGISTER2));
    ithPrintf(  "Width = 0x%x ,Height = 0x%x\n",            data & 0x00000FFF,  (data & 0x1FFF0000) >> 16);

    data    = ithReadRegA(_GetAddress(DEV_ID, CAP_ENGINE_STATUS_REGISTER3));
    ithPrintf(  "MRawHTotal = 0x%x,MRawVTotal = 0x%x\n",    data & 0x0000FFFF,  (data & 0xFFFF0000) >> 16);
