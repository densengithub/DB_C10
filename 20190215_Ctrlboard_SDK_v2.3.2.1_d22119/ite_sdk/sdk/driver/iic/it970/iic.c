    ...
    uint32_t     delay)
{
    ...
    do
    {
        if (IicDevices[port].refCount == 0)
        {
            ...
            // Init HW
            if (IicInitDeviceHw(port, initClock) == false)
            {
...
{
    printf("IT9850 not support this function!!!\n");
    return 0;
}
    ...
    uint32_t    clock)
{
    ...
    do
    {
        ...
        if (IicDevices[port].port == IIC_PORT_0)
        {
            ...
            ithGpioSetMode(CFG_GPIO_IIC0_CLK, CFG_GPIO_IIC0_MODE);
            ithGpioSetMode(CFG_GPIO_IIC0_DATA, CFG_GPIO_IIC0_MODE);
        }
#endif
        ...
        if(IicDevices[port].port == IIC_PORT_1)
        {
             ...
             ithGpioSetMode(CFG_GPIO_IIC1_CLK, CFG_GPIO_IIC1_MODE);
             ithGpioSetMode(CFG_GPIO_IIC1_DATA, CFG_GPIO_IIC1_MODE);
        }
#endif
#ifdef CFG_I2C2_ENABLE
        if(IicDevices[port].port == IIC_PORT_2)
        {
            ithWriteRegA(ITH_GPIO_BASE + 0x238, (1 << 15) | (1 << 31) | (CFG_GPIO_IIC2_CLK << 16) | (CFG_GPIO_IIC2_DATA));
        }
#endif
#ifdef CFG_I2C3_ENABLE
        if(IicDevices[port].port == IIC_PORT_3)
        {
            ithWriteRegA(ITH_GPIO_BASE + 0x23C, (1 << 15) | (1 << 31) | (CFG_GPIO_IIC3_CLK << 16) | (CFG_GPIO_IIC3_DATA));
        }
#endif
#endif
        //Reset HW
    ...
    IIC_DATA_MODE   wStop)
{
    ...
    // Enable IIC SCLK
    // Enable FIFO mode
	while(IicWaitTxFifoCount(port, 1, 1000) == false)
		usleep(1);
    //usleep(1000);
    slaveAddr <<= 1;
    ...
    uint32_t    outDataSize)
{
    ...
    while(remainderSize)
    {
        ...
        if (IicWaitRxFifoCount(port, 1, 1000) == false)
        {
            IIC_ERROR_MSG("Wait RX FIFO timeout!\n");
            result = 1;
            break;
        }
    ...
    if (IIC_REG_BIT(regData, 28) == 1)  /* REQUEST DOWN */
    {
        if ( IIC_REG_BIT(regData, 0) == 0 ) /* W */
        {
            printf("Slave in READ\n");
            {
                ...
                uint32_t    recvDataSize = IIC_SLAVE_READ_SIZE;
