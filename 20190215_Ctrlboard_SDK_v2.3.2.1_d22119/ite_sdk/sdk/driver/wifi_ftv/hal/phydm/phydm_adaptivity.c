...
)
{
	...
	struct _ADAPTIVITY_STATISTICS	*p_adaptivity = (struct _ADAPTIVITY_STATISTICS *)phydm_get_structure(p_dm_odm, PHYDM_ADAPTIVITY);
	...
	ODM_RT_TRACE(p_dm_odm, PHYDM_COMP_ADAPTIVITY, ODM_DBG_LOUD, ("IGI_lmt_cnt = %d\n", p_adaptivity->igi_up_bound_lmt_cnt));
}
...
)
{
	...
	p_dm_odm->adaptivity_flag = (p_dm_odm->support_ic_type & ODM_IC_GAIN_IDX_EDCCA) ? false : true;
...
)
{
	...
	phydm_check_adaptivity(p_dm_odm);	/*Check adaptivity enable*/
	phydm_dig_up_bound_lmt_en(p_dm_odm);
