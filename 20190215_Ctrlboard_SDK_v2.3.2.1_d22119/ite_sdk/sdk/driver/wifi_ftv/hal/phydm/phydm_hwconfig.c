...
/* JJ ADD 20161014 */
	...
	if (p_dm_odm->support_ic_type == ODM_RTL8822B) {
		if (config_type == CONFIG_RF_RADIO) {
			...
			else if (p_dm_odm->rfe_type == 15)
				READ_AND_CONFIG_MP(8822b, _txpwr_lmt_type15);
			else if (p_dm_odm->rfe_type == 16)
...
#if RTL8821C_SUPPORT
	if (p_dm_odm->support_ic_type == ODM_RTL8821C) {
		if (p_dm_odm->rfe_type == 0x5)
			READ_AND_CONFIG(8821c, _txpowertrack_type0x28);
		else if (p_dm_odm->rfe_type == 0x4)
