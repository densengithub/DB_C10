...
)
{
	...
	printf("====>Hal_EfuseParsePIDVID AutoLoadFail: %d\n", pHalData->bautoload_fail_flag);

	if (!(pHalData->bautoload_fail_flag)) {
		/* VID, PID */
		memcpy(&buf, &hwinfo[EEPROM_VID_88EU], 2);
		//pHalData->EEPROMVID = ReadLE2Byte(&hwinfo[EEPROM_VID_88EU]);
		pHalData->EEPROMVID = le16_to_cpu(buf);
		memcpy(&buf, &hwinfo[EEPROM_PID_88EU], 2);
		//pHalData->EEPROMPID = ReadLE2Byte(&hwinfo[EEPROM_PID_88EU]);
		pHalData->EEPROMPID = le16_to_cpu(buf);
