...
static int wpa_psk=0;static int securityMode_psk[64];
...
#define ARPHRD_IEEE80211_RADIOTAP 803// copy from net/wireless/wext.c end
...
static int rtw_parse_wpapsk_ie(u8 *in_ie,uint in_len)
{
	u8 authmode, sec_idx, i;
	u8 wpa_oui[4]={0x0,0x50,0xf2,0x01};
	u8 wpapsk_oui[4]={0x0,0x50,0xf2,0x02};
	u8 rsnpsk_oui[4]={0x0,0x0f,0xac,0x02};
	uint 	cnt;
	int psk=0;
	...
	while(cnt<in_len)
	{
		...
		if((authmode==_WPA_IE_ID_)&&(_rtw_memcmp(&in_ie[cnt+2], &wpa_oui[0],4)==_TRUE))
		{
				for(i=0;i<(in_ie[cnt+1]+2);i++){
					if(_rtw_memcmp(&in_ie[cnt+i], &wpapsk_oui[0],4)==_TRUE)
						psk = 1;
				}
		...
		else
		{
			if(authmode==_WPA2_IE_ID_)
			{
				...
				for(i=0;i<(in_ie[cnt+1]+2);i++){
					if(_rtw_memcmp(&in_ie[cnt+i], &rsnpsk_oui[0],4)==_TRUE)
						psk = 1;
				}
...
static void indicate_wx_custom_event(_adapter *padapter, char *msg)
{
	...
	unsigned int lens;
    struct eventsockaddr	ap_addr;
	struct eventsockaddr	addr;
	...
	wireless_send_event(padapter->pnetdev, IWEVCUSTOM, lens, buff, ap_addr, addr);
#endif
...
static void request_wps_pbc_event(_adapter *padapter)
{
	...
	lens = (lens<IW_CUSTOM_MAX) ? lens:IW_CUSTOM_MAX;
...
void indicate_wx_scan_complete_event(_adapter *padapter)
{
	...
	wireless_send_event(padapter->pnetdev, SIOCGIWSCAN, lens, NULL,ap_addr,addr);
#endif
}
...
void rtw_indicate_wx_assoc_event(_adapter *padapter)
{
	...
	_rtw_memcpy(ap_addr.sa_data, pmlmepriv->cur_network.network.MacAddress, ETH_ALEN);
	...
	wireless_send_event(padapter->pnetdev, SIOCGIWAP, lens, NULL,ap_addr,addr);
#endif
}
...
void rtw_indicate_wx_disassoc_event(_adapter *padapter)
{
	...
	ap_addr.sa_family = ARPHRD_ETHER;
	_rtw_memset(ap_addr.sa_data, 0, ETH_ALEN);
		...
		char *start, char *stop, struct iw_event *iwe)
{
	...
	if (pbuf) {
		...
		if (pnetwork->network.Reserved[0] != BSS_TYPE_PROB_REQ) { /* Probe Request */
			...
			wpa_psk = rtw_parse_wpapsk_ie(pnetwork->network.IEs ,pnetwork->network.IELength);
			   ...
			   union iwreq_data *wrqu, char *extra)
{
...
#if 0 /* Wireless Extension use EAGAIN to try */
	...
	while (1) {
		   ...
		   )
		{
			...
			securityMode_psk[apCnt] = wpa_psk;
			apCnt++;
		}
...
#if 1//def CONFIG_WIRELESS_EXT
struct iw_handler_def rtw_handlers_def = {
	.standard = rtw_handlers,
	.num_standard = sizeof(rtw_handlers) / sizeof(iw_handler),
#if 1//(LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 33)) || defined(CONFIG_WEXT_PRIV)
...
int rtw_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	int ret=0;
	if ((SIOCIWFIRST <= cmd) && (cmd <= SIOCIWFIRSTPRIV))
	{
		struct iw_request_info iwInfo;
	    struct iw_handler_def *iwHandler = &rtw_handlers_def;
	    iw_handler *iwHandlerFunc = NULL;

		memset(&iwInfo, 0x00, sizeof(struct iw_request_info));
	    ...
	    if (iwHandlerFunc)
	        ret = (*iwHandlerFunc)(dev,&iwInfo,&wrq->u,wrq->u.data.pointer);

	    //if (ret != 0)
	        //printk("@@@@OMG, this ID control fail! [%d] cmd = 0x%X\n\n", ret,cmd);
...
int rtw_ioctl_smCtrl(struct net_device *dev, struct net_device_config *netDeviceConfig)
{
    ...
    int i, ret = 0;
    char cmdBuf[32];
    _adapter *pAd = (_adapter*)dev->priv;


   /* if(smMutex ==NULL)
    {
		smMutex = malloc(sizeof(pthread_mutex_t));
		pthread_mutex_init(smMutex, NULL);
    }	*/
    ...
    /*Config the device.*/
    if (netDeviceConfig->operationMode != WLAN_MODE_OFF)
    {
        #if 0 //James Mark : unkown how to support
        // Irene : for WPS
        if(netDeviceConfig->securitySuit.securityMode == WLAN_SEC_WPS)
        {
            if (netDeviceConfig->securitySuit.wpsMode == WLAN_WPS_STOP)
            {
                /*Trigger driver to abort WPS action*/
            }
            else
            {
                if (netDeviceConfig->securitySuit.wpsMode == WLAN_WPS_PIN)
                {	/*set WSC_SSID first.*/

                }
                else if (netDeviceConfig->securitySuit.wpsMode == WLAN_WPS_PIN_START)
                {
                }
                else if (netDeviceConfig->securitySuit.wpsMode == WLAN_WPS_PBC)
                {
                }
            }
            goto end;
...
#if 1
        memset(cmdBuf, 0, 32);
		RESET_SETTING(iwInfo, iwReq);
        if (netDeviceConfig->operationMode == WLAN_MODE_STA)
        {
            iwReq.mode = IW_MODE_INFRA;
        }
        else
        {   /*In AdHoc mode, just NOSEC/WEP support*/
            iwReq.mode = IW_MODE_ADHOC;
        }

        rtw_wx_set_mode(dev, NULL,&iwReq, NULL);


        if (netDeviceConfig->operationMode == WLAN_MODE_ADHOC)
        {
            RESET_SETTING(iwInfo, iwReq);
            iwReq.freq.m = netDeviceConfig->channelId;
            rtw_wx_set_freq(dev,NULL,&iwReq,NULL);
        }
		...
		switch(netDeviceConfig->securitySuit.securityMode)
        {
            case WLAN_SEC_NOSEC:
				RESET_SETTING(iwInfo, iwReq);
				iwReq.param.flags = IW_AUTH_80211_AUTH_ALG;
				iwReq.param.value = AUTH_ALG_OPEN_SYSTEM;
                rtw_wx_set_auth(dev,NULL,&iwReq,NULL);
                break;
            ...
            case WLAN_SEC_WEP:
                ...
                if (netDeviceConfig->securitySuit.authMode == WLAN_AUTH_SHAREDKEY)
                    iwReq.param.value = AUTH_ALG_SHARED_KEY;
				else
                    iwReq.param.value = AUTH_ALG_OPEN_SYSTEM;
                ...
                for (i = 0; i < 4; i++)
                {
                    RESET_SETTING(iwInfo, iwReq);
					memset(cmdBuf, 0, 32);
					sprintf(cmdBuf, "%s",netDeviceConfig->securitySuit.wepKeys[i]);
                    //sprintf(cmdBuf, "Key%d=%s",i+1, netDeviceConfig->securitySuit.wepKeys[i]);
                    //ASSERT(IW_PRIV(dev, SIOCIWFIRSTPRIV + 1, NULL, NULL, cmdBuf) == 0);
                    iwReq.encoding.flags = i+1;
					iwReq.encoding.length = strlen(netDeviceConfig->securitySuit.wepKeys[i]);
                    rtw_wx_set_enc(dev,NULL,&iwReq,cmdBuf);
                }

                memset(cmdBuf, 0, 32);
                //sprintf(cmdBuf, "DefaultKeyID=%d", netDeviceConfig->securitySuit.defaultKeyId + 1);
                //ASSERT(IW_PRIV(dev, SIOCIWFIRSTPRIV + 1, NULL, NULL, cmdBuf) == 0);
                iwReq.encoding.flags = netDeviceConfig->securitySuit.defaultKeyId+1;
				iwReq.encoding.length = 0;
                rtw_wx_set_enc(dev,NULL,&iwReq,cmdBuf);
                break;
			...
			case WLAN_SEC_WPAPSK_AES:
				{
				struct iw_encode_ext *extra;
				int len = 0;
                //memset(cmdBuf, 0, 32);
                //strcpy(cmdBuf, "AuthMode=WPAPSK");
                //ASSERT(IW_PRIV(dev, SIOCIWFIRSTPRIV + 1, NULL, NULL, cmdBuf) == 0);
                len = sizeof(struct iw_encode_ext) + strlen(netDeviceConfig->securitySuit.preShareKey);
	            extra = (struct iw_encode_ext *)_rtw_zmalloc(len);

                if(netDeviceConfig->securitySuit.securityMode == WLAN_SEC_WPAPSK_AES)
                {
					extra->alg = IW_ENCODE_ALG_CCMP;
                }
				else
				{
				    extra->alg = IW_ENCODE_ALG_TKIP;
				}
				extra->key_len = strlen(netDeviceConfig->securitySuit.preShareKey);
				_rtw_memcpy(extra->key, netDeviceConfig->securitySuit.preShareKey, extra->key_len);


				wpa_set_param(dev, IEEE_PARAM_WPA_ENABLED, 1);
                RESET_SETTING(iwInfo, iwReq);
				rtw_wx_set_enc_ext(dev,NULL,&iwReq,(char*)extra);

                //memset(cmdBuf, 0, 32);
                //strcpy(cmdBuf, "EncrypType=TKIP");
                //ASSERT(IW_PRIV(dev, SIOCIWFIRSTPRIV + 1, NULL, NULL, cmdBuf) == 0);

                //memset(cmdBuf, 0, 32);
                //sprintf(cmdBuf,"SSID=%s", netDeviceConfig->ssidName);
                //ASSERT(IW_PRIV(dev, SIOCIWFIRSTPRIV + 1, NULL, NULL, cmdBuf) == 0);

                //memset(cmdBuf, 0, 32);
                //sprintf(cmdBuf,"WPAPSK=%s", netDeviceConfig->securitySuit.preShareKey);
                //ASSERT(IW_PRIV(dev, SIOCIWFIRSTPRIV + 1, NULL, NULL, cmdBuf) == 0);
                _rtw_mfree((u8*)extra, len);
            	}
				break;
			...
			case WLAN_SEC_WPA2PSK_TKIP:
				...
				//memset(cmdBuf, 0, 32);
                //strcpy(cmdBuf, "AuthMode=WPA2PSK");
                //ASSERT(IW_PRIV(dev, SIOCIWFIRSTPRIV + 1, NULL, NULL, cmdBuf) == 0);

                len = sizeof(struct iw_encode_ext) + strlen(netDeviceConfig->securitySuit.preShareKey);
	            extra = (struct iw_encode_ext *)_rtw_zmalloc(len);

                if(netDeviceConfig->securitySuit.securityMode == WLAN_SEC_WPA2PSK_TKIP)
                {
					extra->alg = IW_ENCODE_ALG_TKIP;
                }
				else
				...
				wpa_set_param(dev, IEEE_PARAM_WPA_ENABLED, 2);
                RESET_SETTING(iwInfo, iwReq);
				rtw_wx_set_enc_ext(dev,NULL,&iwReq,(char*)extra);

                //memset(cmdBuf, 0, 32);
                //strcpy(cmdBuf, "EncrypType=AES");
                //ASSERT(IW_PRIV(dev, SIOCIWFIRSTPRIV + 1, NULL, NULL, cmdBuf) == 0);
		...
		RESET_SETTING(iwInfo, iwReq);
		memset(cmdBuf, 0, 32);
        if ((netDeviceConfig->securitySuit.securityMode == WLAN_SEC_NOSEC) &&
            (netDeviceConfig->feaFlags & WLAN_FEA_ANYSSID))
        	{
                iwReq.essid.length = 0;
				sprintf(cmdBuf,"anySSID"); /*only NOSEC have anySSID.*/
        	}
        else
        	{
            	memcpy(cmdBuf,netDeviceConfig->ssidName,sizeof(netDeviceConfig->ssidName));
				//iwReq.essid.length = strlen(netDeviceConfig->ssidName)+1;
				iwReq.essid.length = strlen(netDeviceConfig->ssidName);
        	}
		    //sprintf(cmdBuf,"%s", netDeviceConfig->ssidName);
       // ASSERT(IW_PRIV(dev, SIOCIWFIRSTPRIV + 1, NULL, NULL, cmdBuf) == 0);

	   iwReq.essid.flags = 1;
	   iwReq.essid.pointer = (u8*)cmdBuf;
       rtw_wx_set_essid(dev,NULL,&iwReq,cmdBuf);
...
int rtw_ioctl_smInfo(struct net_device *dev, struct net_device_info *netDeviceInfo)
{
    struct iw_request_info iwInfo;
    union iwreq_data iwReq;
    struct iw_event  iwEvt;
    int i, ret = 0, len;
    u8 buf[36];
    u8 *work, *scanResult;
	u8 wpa_ie[255];
    _adapter* pAdapter = (_adapter*) dev->priv;
    ...
    {
        ret = -1;
        printf("\nrtusb_ioctl_smInfo() device not ready!! \n");
        goto end;
    }


    if(netDeviceInfo->infoType == WLAN_INFO_WIFI_STATE)
    {
        ...
        netDeviceInfo->driverState = get_fwstate(pmlmepriv);
       // printf("WLAN_INFO_WIFI_STATE 0x%x, 0x%X 0x%x  adapter 0x%x \n",pmlmepriv,get_fwstate(pmlmepriv),pmlmepriv->fw_state,padapter);
        for (i = 0; i < MAX_CHANNEL_NUM; i++) {

            if(pmlmeext->channel_set[i].ChannelNum != 0 && pmlmeext->channel_set[i].ChannelNum <= 11)
            {
                if(pmlmeext->channel_set[i].rx_count < pmlmeext->channel_set[tempindex].rx_count)
                {
                    tempindex = i;
                    bestchannel = pmlmeext->channel_set[i].ChannelNum;
                }
            }
        }
        netDeviceInfo->channelId = bestchannel;
        netDeviceInfo->staCount = padapter->stapriv.asoc_sta_count;
    }
    else if(netDeviceInfo->infoType == WLAN_INFO_LINK)
    {
//      _adapter *padapter = (_adapter *)netdev_priv(dev);
	 	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
		...
		if (check_fwstate(&padapter->mlmepriv, WIFI_ASOC_STATE) == _TRUE)
			netDeviceInfo->linkInfo = WLAN_LINK_ON;
		else
			netDeviceInfo->linkInfo = WLAN_LINK_OFF;
    }
    else if (netDeviceInfo->infoType == WLAN_INFO_AP)
    {
        struct iw_statistics *iwstats;
		RESET_SETTING(iwInfo, iwReq);
	    memset((void*)buf, 0, 36);
        if (rtw_wx_get_wap(dev,NULL,&iwReq,NULL) == 0)
        {
            netDeviceInfo->linkInfo = WLAN_LINK_ON;
            memcpy(netDeviceInfo->apMacAddr, &iwReq.ap_addr.sa_data[0], 6);

            RESET_SETTING(iwInfo, iwReq);
            //ASSERT(IW_CONFIG(dev, SIOCGIWFREQ, NULL, &iwReq, NULL) == 0);
            if(rtw_wx_get_freq(dev,NULL,&iwReq,NULL) == 0)
				netDeviceInfo->channelId = iwReq.freq.m;

            RESET_SETTING(iwInfo, iwReq);
            //ASSERT(IW_CONFIG(dev, SIOCGIWESSID, NULL, &iwReq, buf) == 0);
            if(rtw_wx_get_essid(dev,NULL,&iwReq,buf) == 0)
            {
	            if(iwReq.data.length > 0)
	            	memcpy(netDeviceInfo->ssidName, buf, iwReq.data.length);
            }

			iwstats = rtw_get_wireless_stats(dev);
			netDeviceInfo->avgQuant = iwstats->qual.qual;
            netDeviceInfo->avgRSSI = iwstats->qual.level;
            //memcpy(netDeviceInfo->staMacAddr, dev->dev_addr, 6);
        }
        else
            netDeviceInfo->linkInfo = WLAN_LINK_OFF;
    ...
    {
        /*Trigger driver to scan*/
        //ASSERT(IW_CONFIG(dev, SIOCSIWSCAN, NULL, NULL, NULL) == 0);
        rtw_wx_set_scan(dev,NULL, NULL, NULL);
    }
    else if (netDeviceInfo->infoType == WLAN_INFO_SCAN_GET)
    {
        char *custom,*tend;
		/*Get Scan result*/
        RESET_SETTING(iwInfo, iwReq);
        scanResult = malloc(IW_SCAN_MAX_DATA);
        memset((void*)scanResult, 0, IW_SCAN_MAX_DATA);
        //if (IW_CONFIG(dev, SIOCGIWSCAN, NULL, &iwReq, scanResult) == (-EAGAIN))
        iwReq.data.length = IW_SCAN_MAX_DATA;
        if(rtw_wx_get_scan(dev,&iwInfo, &iwReq, scanResult)== (-EAGAIN))
		{
            goto end;
        }
        ...
        i = -1;
        work = scanResult;
		tend = work + iwReq.data.length;

        while((work - scanResult) < iwReq.data.length && i<63)
        {
            memcpy((void*)&iwEvt, (void*)work, IW_EV_LCP_LEN);

			if (iwEvt.len <= IW_EV_LCP_LEN)
				break;

			custom = work + IW_EV_POINT_LEN;
			if ((iwEvt.cmd == SIOCGIWESSID || iwEvt.cmd == SIOCGIWENCODE ||
		 		iwEvt.cmd == IWEVGENIE || iwEvt.cmd == IWEVCUSTOM)) {
				/* WE-19 removed the pointer from struct iw_point */
				char *dpos = (char *) &iwEvt.u.data.length;
				int dlen = dpos - (char *) &iwEvt;
				memcpy(dpos, work + IW_EV_LCP_LEN,
					  sizeof(struct iw_event) - dlen);
			} else {
				memcpy(&iwEvt, work, sizeof(struct iw_event));
            ...
            switch(iwEvt.cmd)
            {
                case SIOCGIWAP:
                    i++;    /*next AP*/
                    memcpy(netDeviceInfo->apList[i].apMacAddr,iwEvt.u.ap_addr.sa_data,6);
                    break;

                case SIOCGIWMODE:
                    if (iwEvt.u.mode = IW_MODE_INFRA || iwEvt.u.mode == IW_MODE_MASTER)
                        netDeviceInfo->apList[i].operationMode = WLAN_SCAN_OPMODE_INFRA;
                    else if (iwEvt.u.mode = IW_MODE_ADHOC)
                        netDeviceInfo->apList[i].operationMode = WLAN_SCAN_OPMODE_ADHOC;
                    else
                        netDeviceInfo->apList[i].operationMode = WLAN_SCAN_OPMODE_UNKNOW;
                    break;

                case SIOCGIWESSID:
                    memset(netDeviceInfo->apList[i].ssidName, 0 ,32);
                    memcpy(netDeviceInfo->apList[i].ssidName,custom,iwEvt.u.essid.length);
                    break;

                case SIOCGIWENCODE:
                    if (iwEvt.u.data.flags & IW_ENCODE_DISABLED)
                    {
                        netDeviceInfo->apList[i].securityOn = WLAN_SCAN_SECURITY_OFF;
						netDeviceInfo->apList[i].securityMode = WLAN_SEC_NOSEC;
                    }
                    else
                    {
                        netDeviceInfo->apList[i].securityOn = WLAN_SCAN_SECURITY_ON;
						netDeviceInfo->apList[i].securityMode = WLAN_SEC_WEP;
                    }
                    break;

                case SIOCGIWFREQ:
                    netDeviceInfo->apList[i].channelId = iwEvt.u.freq.m;
                    break;

                case SIOCGIWNAME:
					//memcpy(netDeviceInfo->apList[i].name,&iwEvt.u.name,IFNAMSIZ);
					break;
				case SIOCGIWRATE:
#if 0
					{
						int maxrate;
						char *tcustom = work + IW_EV_LCP_LEN;
						struct iw_param p;
						size_t clen;
						...
						while (((ssize_t) clen) >= (ssize_t) sizeof(struct iw_param)) {
							...
							memcpy(&p, tcustom, sizeof(struct iw_param));
							if (p.value > maxrate)
								maxrate = p.value;
							clen -= sizeof(struct iw_param);
							tcustom += sizeof(struct iw_param);
						}

						/* Convert the maxrate from WE-style (b/s units) to
						 * 802.11 rates (500000 b/s units).
						 */
						netDeviceInfo->apList[i].bitrate = maxrate / 500000;
					}
#endif
					break;
				case IWEVQUAL:
					netDeviceInfo->apList[i].rfQualityQuant = iwEvt.u.qual.qual;
					netDeviceInfo->apList[i].rfQualityRSSI = iwEvt.u.qual.level;
					break;
				case IWEVCUSTOM:
#if 0
					memset(wpa_ie, 0 ,255);
                    memcpy(wpa_ie,custom,iwEvt.u.data.length);

					if (iwEvt.u.data.length > 7 && strncmp(wpa_ie, "wpa_ie=", 7) == 0)
					{
						u8 wpa_oui[4]={0x0,0x50,0xf2,0x02};
						printf("wpa_ie = \n");
						for(cnt=0; cnt < (iwEvt.u.data.length-4); cnt++)
						{
                            printf("02%x",wpa_ie[cnt]);
							if(_rtw_memcmp(&wpa_ie[cnt], &wpa_oui[0],4)==_TRUE)
							{
                                psk = 1;
								netDeviceInfo->apList[i].securityMode = WLAN_SEC_WPAPSK_MIX;
							}
						}
						printf("\n");
					}
					else if(iwEvt.u.data.length > 7 && strncmp(wpa_ie, "rsn_ie=", 7) == 0)
					{
                        u8 rsn_oui[4]={0x0,0x0f,0xac,0x02};
						printf("rsn_ie = \n");
						for(cnt=0; cnt < (iwEvt.u.data.length-4); cnt++)
						{
							printf("02%x",wpa_ie[cnt]);

							if(_rtw_memcmp(&wpa_ie[cnt], &rsn_oui[0],4)==_TRUE)
							{
								psk = 1;
								netDeviceInfo->apList[i].securityMode = WLAN_SEC_WPAPSK_MIX;
							}
					...
					if(psk == 0)
					  netDeviceInfo->apList[i].securityMode = WLAN_SEC_EAP;
#else
					if(securityMode_psk[i] == 1)
						netDeviceInfo->apList[i].securityMode = WLAN_SEC_WPAPSK_MIX;
					else
						netDeviceInfo->apList[i].securityMode = WLAN_SEC_EAP;
        ...
        netDeviceInfo->apCnt = i+1;
        free(scanResult);
		//pthread_mutex_unlock(smMutex);
    }
   /* else if (netDeviceInfo->infoType == WLAN_INFO_WPS_STATUS)
    {
        netDeviceInfo->wpsStatus = pAdapter->PortCfg.WscControl.WscStatus;
    }
    else if (netDeviceInfo->infoType == WLAN_INFO_WPA_STATE)
    {
        netDeviceInfo->wpaState = pAdapter->PortCfg.WpaState;
    }*/
