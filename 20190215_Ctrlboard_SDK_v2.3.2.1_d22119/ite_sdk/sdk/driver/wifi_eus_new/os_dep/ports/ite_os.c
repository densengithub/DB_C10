...
static int gTaskInit = 0;
static int gTaskRunning = 0;
...
void tasklet_deinit()
{
	...
	printf("tasklet_deinit \n");

}

void* tasklet_doprocess(void *p)
{
printf("tasklet_doprocess \n");
	gTaskRunning = 1;
...
if(statictaskletCount > 0)
{
	...
	memset(statictasklet, 0, sizeof(struct tasklet_struct)*2);
	memset(tasklet, 0, sizeof(struct tasklet_struct)*1024);
	memset(tasklethi, 0, sizeof(struct tasklet_struct)*1024);

	statictaskletCount = 0;
	taskletR =0;
	taskletW =0;
	taskletCount = 0;
	tasklethiCount = 0;
	SYS_DelEvent(rw_event);

	printf("tasklet_doprocess end\n");
...
void start_tasklet_timer(void)
{
	...
	pthread_create(&tasklet_task, &attr, tasklet_doprocess, NULL);
...
void wireless_send_event(void* dev,unsigned int cmd,unsigned int lens,char* buff,struct eventsockaddr ap_addr,struct eventsockaddr addr)
{
...
#if 1
	{
		...
		if(netlinkbuffer.setflags >= NETLINK_NUM)
            ...
            printf("netlinkbuffer.setflags = %d, cmd: 0x%X\n",netlinkbuffer.setflags, cmd);
...
int copy_from_user(void *_des,const void *_src,int _len)
{
