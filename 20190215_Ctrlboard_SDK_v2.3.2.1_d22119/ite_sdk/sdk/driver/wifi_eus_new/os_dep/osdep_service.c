...
#if 1
//#define dev_kfree_skb_any(x)	dev_kfree_skb(x)
#define skb_copy(a,b)			pskb_copy(((struct sk_buff*)a),b)
#endif
...
void _rtw_init_sema(_sema	*sema, int init_val)
{
	...
	sem_init(sema, 0, init_val); //eason add
#endif
#ifdef PLATFORM_FREEBSD
...
#else
	pnetdev = alloc_netdevice(sizeof(struct rtw_netdev_priv_indicator));
#endif
	if (!pnetdev)
		goto RETURN;
...
int rtw_change_ifname(_adapter *padapter, const char *ifname)
{
	...
	if (rereg_priv->old_pnetdev) {
		free_netdevice(rereg_priv->old_pnetdev);
