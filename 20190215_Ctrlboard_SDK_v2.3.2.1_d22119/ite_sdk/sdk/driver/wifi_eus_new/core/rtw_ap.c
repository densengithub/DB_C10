...
#define KEEP_ALIVE_TRYCNT (3)
	...
	if (chk_alive_num) {
		...
		for (i = 0; i < chk_alive_num; i++) {
			...
			updated = rtw_ap_free_sta(padapter, psta, _FALSE, WLAN_REASON_DEAUTH_LEAVING, _FALSE);
			#ifdef CONFIG_RTW_MESH
			if (MLME_IS_MESH(padapter))
...
u8 rtw_ap_free_sta(_adapter *padapter, struct sta_info *psta, bool active, u16 reason, bool enqueue)
{
	_irqL irqL;
	u8 beacon_updated = _FALSE;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
...
int rtw_sta_flush(_adapter *padapter, bool enqueue)
{
	...
	/* call rtw_ap_free_sta() for each sta picked */
	for (i = 0; i < flush_num; i++) {
		...
		rtw_ap_free_sta(padapter, psta, _TRUE, WLAN_REASON_DEAUTH_LEAVING, enqueue);
		#ifdef CONFIG_RTW_MESH
		if (MLME_IS_MESH(padapter))
			rtw_mesh_expire_peer(padapter, sta_addr);
		#endif
