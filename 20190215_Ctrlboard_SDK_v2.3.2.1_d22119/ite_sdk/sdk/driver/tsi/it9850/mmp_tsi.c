...
#define HOST_WriteRegisterMask(add, data, mask) ithWriteRegMaskH(add, data, mask)
// AMBA
    ...
    MMP_UINT32 tsiId)
{
    ...
    ithGpioSetMode( ITH_GPIO_PIN19, ITH_GPIO_MODE1);
    ithGpioSetMode( ITH_GPIO_PIN20, ITH_GPIO_MODE1);
    ithGpioSetMode( ITH_GPIO_PIN21, ITH_GPIO_MODE1);
    ithGpioSetMode( ITH_GPIO_PIN22, ITH_GPIO_MODE1);
    {
        printf( "-- set tsi 1 (parallel) gpio, %s[#%d]\n", __FUNCTION__, __LINE__);
    }
    // set data pins
    ithGpioSetMode( ITH_GPIO_PIN23, ITH_GPIO_MODE1);
    ithGpioSetMode( ITH_GPIO_PIN24, ITH_GPIO_MODE1);
    ithGpioSetMode( ITH_GPIO_PIN25, ITH_GPIO_MODE1);
    ithGpioSetMode( ITH_GPIO_PIN26, ITH_GPIO_MODE1);
    ithGpioSetMode( ITH_GPIO_PIN27, ITH_GPIO_MODE1);
    ithGpioSetMode( ITH_GPIO_PIN28, ITH_GPIO_MODE1);
    ithGpioSetMode( ITH_GPIO_PIN29, ITH_GPIO_MODE1);
#else
    switch (tsiId)
    {
    case 0:
        ...
        {
            ithGpioSetMode( ITH_GPIO_PIN71, ITH_GPIO_MODE3);
            ithGpioSetMode( ITH_GPIO_PIN72, ITH_GPIO_MODE3);
            ithGpioSetMode( ITH_GPIO_PIN73, ITH_GPIO_MODE3);
            ithGpioSetMode( ITH_GPIO_PIN74, ITH_GPIO_MODE3);
        }
        else
        {
            ithGpioSetMode( ITH_GPIO_PIN15, ITH_GPIO_MODE1);
            ithGpioSetMode( ITH_GPIO_PIN16, ITH_GPIO_MODE1);
            ithGpioSetMode( ITH_GPIO_PIN17, ITH_GPIO_MODE1);
            ithGpioSetMode( ITH_GPIO_PIN18, ITH_GPIO_MODE1);
            {
...
{
    assert(tsiId == 0 || tsiId == 1);
    ithIntrDisableIrq(tsiId == 0 ? ITH_INTR_TSI0 : ITH_INTR_TSI1);
}
