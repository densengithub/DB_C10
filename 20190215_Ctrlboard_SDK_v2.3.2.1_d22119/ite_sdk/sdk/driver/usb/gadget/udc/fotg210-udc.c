...
#if PR_INFO
#define pr_info(string, args...)   do { ithPrintf("[UDC][INFO] "); ithPrintf(string, ## args); } while (0)
#else
#define pr_info(string, args...)
#endif

#if PR_ERR
#define pr_err(string, args...)   do { ithPrintf("[UDC][ERR] "); ithPrintf(string, ## args); } while (0)
#else
#define pr_err(string, args...)
#endif

static struct fotg210_udc* g_udc;

#define ioread32(reg)           ithReadRegA((u32)reg)
#define iowrite32(val, reg)     ithWriteRegA((u32)reg, val) 
...
static void fotg210_wait_dma_done(struct fotg210_ep *ep)
{
	...
	do {
		...
		if ((value & DISGR2_USBRST_INT) ||
			#if 1  // Irene Lin: fix dma can't complete when unplug
...
dma_reset:
	ithPrintf(" udc: Abort DMA !!!!! (0x%08X) \n", value);
	value = ioread32(ep->fotg210->reg + FOTG210_DMACPSR1);
	#if 1  // Irene Lin: fix dma can't complete when unplug
    value |= (DMACPSR1_DMA_ABORT|DMACPSR1_CLRFIFO_DMAABORT);
	#else
...
#if 0  // Irene Lin: fix dma can't complete when unplug
    /* reset fifo */
    if (ep->epnum) {
...
static void fotg210_epin_zero_byte(struct fotg210_ep *ep)
{
    struct fotg210_udc *fotg210 = ep->fotg210;

    if (ep->dir_in) {
		void __iomem *reg = fotg210->reg + FOTG210_INEPMPSR(ep->epnum);
		
		u32 value = ioread32(reg);
		value |= INOUTEPMPSR_TX0BYTE;
		iowrite32(value, reg);
	}
}
			...
			struct fotg210_request *req)
{
    ...
    if (ep->dir_in) {   
        ...
        ithFlushDCacheRange((void*)buffer, length);
		#endif
        ithFlushMemBuffer();

#if 1 // Irene Lin: zero packet
		/*
...
#else
	if (!ep->dir_in)
		 ithInvalidateDCacheRange((void*)buffer, length);
#endif
}
...
static void fotg210_ep0in(struct fotg210_udc *fotg210)
{
	...
	if ((!list_empty(&ep->queue)) && (ep->dir_in)) {
		...
		//if ((req->req.length - req->req.actual) < ep->ep.maxpacket)  //bug found by powei
		if ((req->req.length == req->req.actual) ||
		    (req->req.actual < ep->ep.maxpacket))
			fotg210_done(ep, req, 0);
	} else {
...
static void fotg210_irq(int irq, void *_fotg210)
{
	...
	if (int_grp & DIGR_INT_G2) {
		...
		if (int_grp2 & DISGR2_SUSP_INT) {
			...
			value &= ~DISGR2_SUSP_INT;
			iowrite32(value, reg);
			fotg210->gadget.speed = USB_SPEED_UNKNOWN; // Irene Lin
			ithPrintf("udc suspend\n");
		}
...
static void fotg210_init(struct fotg210_udc *fotg210)
{
	...
	/* disable out interrupt by powei */
	value = ioread32(fotg210->reg + FOTG210_DMISGR0);
	value |= DMISGR0_MCX_OUT_INT;
	iowrite32(value, fotg210->reg + FOTG210_DMISGR0);

	/* group 2: not used, these two interrupts will trigger when unplug from PC - by Irene Lin*/
	value = ioread32(fotg210->reg + FOTG210_DMISGR2);
	value |= ((0x1 << 9) | (0x1 << 10));;
	iowrite32(value, fotg210->reg + FOTG210_DMISGR2);
}
...
static int iteUdcTerminate(void)
{
	struct fotg210_udc *fotg210 = g_udc;
...
static int iteUdcInitialize(void)
{
	...
	ret = usb_add_gadget_udc(NULL, &fotg210->gadget);
	if (ret)
		goto err_add_udc;

	printf("udc initialized! \n");
