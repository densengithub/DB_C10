...
static void MacConfig(struct ftgmac030_ctrl *ctrl, const uint8_t* ioConfig)
{
	...
	/* RGMII TX CLK driving set to 3 */
	if (ctrl->is_grmii)
		ithGpioSetDriving(31, ITH_GPIO_DRIVING_3);	// TX CLK

	if (0) { // only for RMII: use internal refclk
...
int iteEthGetLink(void)
{
    ...
    if (ctrl->cfg->phy_link_status) /* for 8304mb switch, down/up hw here not in isr */
    {
        ...
        processLinkChang((void*)dev, (uint32_t)new_link);
    }
#else
