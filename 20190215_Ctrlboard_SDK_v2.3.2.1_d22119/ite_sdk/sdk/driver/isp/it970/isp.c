...
//#define     WEIGHT_NUM 9
    ...
    #if defined(CFG_LCD_ENABLE) && defined(CFG_LCD_HEIGHT) && defined(CFG_LCD_WIDTH)
    ...
    #endif
#else
    MMP_FLOAT ScaleRatio[WEIGHT_NUM] = {(800.0f / 200), (800.0f / 400), (480.0f / 340), (640.0f / 500), (800.0f / 640), (800.0f / 800)};
#endif
...
{
    // _11      // _12      // _13
    0x0100,     0x0000,     0x015F,
    // _21      // _22      // _23
    0x0100,     0x07AA,     0x074D,
    // _31      // _32      // _33
    0x0100,     0x01BB,     0x0000,
    // ConstR   // ConstG   // ConstB
    0x0351,     0x0084,     0x0322,
};
...
static const ISP_RGB_TO_YUV gtIspRgb2YuvColorMatrix_ForNonePlayVideoMode =
{
    ...
    // _21      // _22      // _23
    0x03DA,     0x03B6,     0x0070,
    // _31      // _32      // _33
    0x0070,     0x03A2,     0x03EE,
    // ConstY   // ConstU   // ConstV
...
#else
    return (MMP_FLOAT) (((MMP_FLOAT) (16384.0f * Input / (MMP_FLOAT)Output)) / 16384.0f);
#endif
    ...
    ISP_DEINTERLACE_CTRL *pDeInterlace)
{
    ...
    pDeInterlace->EnLPFBlend       = 0;
    pDeInterlace->EnLPFWeight      = 1;
...
_ISP_Deinter2D_Param(
    ISP_DEINTERLACE_CTRL *pDeInterlace)
{
    MMP_UINT16 EdgeBlendWeight = 64;
    MMP_UINT16 OrgBlendWeight  = 0;

    pDeInterlace->PostLPFilterEnable = 0;
...
{
    ISP_CONTEXT *pISPctxt     = (ISP_CONTEXT *)ptDev;
    MMP_UINT16  PreScaleWidth = pISPctxt->PreScaleFun.DstWidth;
    MMP_FLOAT   HCI;
    MMP_FLOAT   VCI;

    if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == MMP_TRUE && pISPctxt->PreScaleSel == MMP_ISP_PRESCALE_PREVIEW)
        PreScaleWidth = _MIN(pISPctxt->InInfo.PanelWidth, ISP_SCALE_MAX_LINE_BUFF_LEN_NORMAL);

    HCI = (pISPctxt->EngineMode.EnableBlockMode)
          ? _ISP_ScaleFactor(pISPctxt->InInfo.SrcWidth, pISPctxt->ScaleFun.DstWidth)
          : _ISP_ScaleFactor(PreScaleWidth, pISPctxt->ScaleFun.DstWidth);
...
//=============================================================================
/**
 * Calculate ISP Preview Scaling Factor.
 */
//=============================================================================
static void
_ISP_CalPVScaleHCI_VCI(
    ISP_DEVICE ptDev)
{
    ISP_CONTEXT *pISPctxt     = (ISP_CONTEXT *)ptDev;
    MMP_UINT16  PreScaleWidth = pISPctxt->PreScaleFun.DstWidth;
    ...
    if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == MMP_TRUE && pISPctxt->PreScaleSel == MMP_ISP_PRESCALE_NORMAL)
        PreScaleWidth = _MIN(pISPctxt->InInfo.PanelWidth, ISP_SCALE_MAX_LINE_BUFF_LEN_PREVIEW);

    HCI = (pISPctxt->EngineMode.EnableBlockMode)
          ? _ISP_ScaleFactor(pISPctxt->InInfo.SrcWidth, pISPctxt->PVScaleFun.DstWidth)
          : _ISP_ScaleFactor(PreScaleWidth, pISPctxt->PVScaleFun.DstWidth);

    VCI = (pISPctxt->EngineMode.EnableBlockMode)
          ? _ISP_ScaleFactor(pISPctxt->InInfo.SrcHeight, pISPctxt->PVScaleFun.DstHeight)
          : _ISP_ScaleFactor(pISPctxt->InInfo.PanelHeight, pISPctxt->PVScaleFun.DstHeight);

    if (HCI != pISPctxt->PVScaleFun.HCI)
        pISPctxt->UpdateFlags |= ISP_FLAGS_UPDATE_PVScaleMatrixH;

    if (VCI != pISPctxt->PVScaleFun.VCI)
        pISPctxt->UpdateFlags |= ISP_FLAGS_UPDATE_PVScaleMatrixV;

    pISPctxt->PVScaleFun.HCI = HCI;
    pISPctxt->PVScaleFun.VCI = VCI;
}
    ...
    MMP_ISP_CORE_INFO *coreInfo)
{
    ...
    ISP_CONTEXT      *pISPctxt   = (ISP_CONTEXT *)ptDev;
    MMP_ISP_CORE     ispCore     = ITH_ISP_CORE0;
    ISP_HW_OPERATION *gptIspHwOp = NULL;
    ...
    ispCore    = pISPctxt->ispCore;
    gptIspHwOp = pISPctxt->gptIspHwOp;

    isp_Memset((void *)pISPctxt, 0, sizeof(ISP_CONTEXT));

    pISPctxt->ispCore    = ispCore;
    pISPctxt->gptIspHwOp = gptIspHwOp;

    // Set Core
    if (coreInfo)
    {
        pISPctxt->EnPreview    = coreInfo->EnPreview;
        pISPctxt->PreScaleSel  = coreInfo->PreScaleSel;
        pISPctxt->UpdateFlags |= ISP_FLAGS_UPDATE_PreviewMode;
    }
    ...
    if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == MMP_TRUE)
    {
    ...
    for (index = 0; index < WEIGHT_NUM; index++)
    {
        ...
        for (j = 0; j < ISP_SCALE_TAP_SIZE; j++)
            for (i = 0; i < ISP_SCALE_TAP; i++)
                pISPctxt->WeightMatInt[index][j][i] = (MMP_UINT8)ISP_FloatToFix(WeightMat[j][i], 1, 6);
    }
    ...
    ISP_DEVICE ptDev)
{
    ...
    // Set Double Line Read
    if ((pISPctxt->InInfo.EnableYUVPlaneMode && pISPctxt->InInfo.PlaneFormat == YUV420) | pISPctxt->InInfo.EnableNVMode)
        if (pISPctxt->DeInterlace.Enable)
            pISPctxt->InInfo.EnableRdRqDoubleLine = MMP_TRUE;
    ...
    // Adjust the Preview Scaling Parameter
    if (pISPctxt->PVOutInfo.Width < (pISPctxt->PVScaleFun.DstPosX + pISPctxt->PVScaleFun.DstWidth))
    {
        if (pISPctxt->PVScaleFun.DstPosX > pISPctxt->PVOutInfo.Width)
            pISPctxt->PVScaleFun.DstPosX = pISPctxt->PVScaleFun.DstWidth =
            pISPctxt->PVScaleFun.DstPosY = pISPctxt->PVScaleFun.DstHeight = 0;
        else
            pISPctxt->PVScaleFun.DstWidth = pISPctxt->PVOutInfo.Width - pISPctxt->PVScaleFun.DstPosX;
        pISPctxt->UpdateFlags |= ISP_FLAGS_UPDATE_PVScaleParam;
    }
    if (pISPctxt->PVOutInfo.Height < (pISPctxt->PVScaleFun.DstPosY + pISPctxt->PVScaleFun.DstHeight))
    {
        if (pISPctxt->PVScaleFun.DstPosY > pISPctxt->PVOutInfo.Height)
            pISPctxt->PVScaleFun.DstPosX = pISPctxt->PVScaleFun.DstWidth =
            pISPctxt->PVScaleFun.DstPosY = pISPctxt->PVScaleFun.DstHeight = 0;
        else
            pISPctxt->PVScaleFun.DstHeight = pISPctxt->PVOutInfo.Height - pISPctxt->PVScaleFun.DstPosY;
        pISPctxt->UpdateFlags |= ISP_FLAGS_UPDATE_PVScaleParam;
    }
        ...
        (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_InputBuf))
    {
        ...
        if (pISPctxt->ispCore == MMP_ISP_CORE_0 || (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == MMP_FALSE))
            pISPctxt->PreScaleFun.DstWidth = _MIN(pISPctxt->InInfo.PanelWidth, ISP_SCALE_MAX_LINE_BUFF_LEN);
        else if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == MMP_TRUE && pISPctxt->PreScaleSel == MMP_ISP_PRESCALE_PREVIEW)
            pISPctxt->PreScaleFun.DstWidth = _MIN(pISPctxt->InInfo.PanelWidth, ISP_SCALE_MAX_LINE_BUFF_LEN_PREVIEW);
        else if (pISPctxt->ispCore == MMP_ISP_CORE_1 && pISPctxt->EnPreview == MMP_TRUE && pISPctxt->PreScaleSel == MMP_ISP_PRESCALE_NORMAL)
            pISPctxt->PreScaleFun.DstWidth = _MIN(pISPctxt->InInfo.PanelWidth, ISP_SCALE_MAX_LINE_BUFF_LEN_NORMAL);
        pISPctxt->UpdateFlags |= ISP_FLAGS_UPDATE_PreScaleParam;
    }
    else
        pISPctxt->UpdateFlags &= (~ISP_FLAGS_UPDATE_PreScaleParam);
    ...
    // Update Preview Scale Parameter of ISP
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVOutParameter)
        pISPctxt->UpdateFlags |= ISP_FLAGS_UPDATE_PVScaleParam;
    ...
    ISP_DEVICE ptDev)
{
    ...
    //
    //Set Engine Mode and Output Parameter
    //
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_EngineMode || pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_OutParameter)
    {
        if (pISPctxt->EngineMode.EnableLOWLATENCY)
        {
            if (pISPctxt->ispCore == MMP_ISP_CORE_0)
            {
                ISP_SetLowLatencySlice_Reg(pISPctxt, pISPctxt->OutInfo.LOWLATENCYSliceNum);
            }
        ...
        ISP_SetEngineModeOutParameter_Reg(pISPctxt, &pISPctxt->EngineMode, &pISPctxt->OutInfo);
    }
    ...
    //
    //Deinterlace Parameter and Input Parameter
    //
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_DeInterlaceParam || pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_InputParameter)
    {
        if (pISPctxt->ispCore == MMP_ISP_CORE_1)
            pISPctxt->InInfo.EnableRdRqDoubleLine = MMP_FALSE;
        ISP_SetDeInterlaceInputParam_Reg(pISPctxt, &pISPctxt->DeInterlace, &pISPctxt->InInfo);
    }
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_SubTitle0)
    {
        ...
        ISP_SetSubTitle0_Reg(pISPctxt, &pISPctxt->SubTitle0);
    }
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_SubTitle1)
    {
        ...
        ISP_SetSubTitle1_Reg(pISPctxt, &pISPctxt->SubTitle1);
    }
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_YUVtoRGBMatrix)
        ISP_SetYUVtoRGBMatrix_Reg(pISPctxt, pISPctxt->YUV2RGBFun);

    //
    //Color Correction Parameter
    //
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_CCMatrix)
        ISP_SetCCMatrix_Reg(pISPctxt, &pISPctxt->CCFun);
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PreScaleParam)
    {
        ...
        ISP_SetPreScaleParam_Reg(pISPctxt, &pISPctxt->PreScaleFun);
    }
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PreScaleMatrix)
    {
        ...
        //{
            for (index = 0; index < WEIGHT_NUM; index++)
            {
                ...
                {
                    ISP_SetIntPreScaleMatrix_Reg(pISPctxt, pISPctxt->WeightMatInt[index]);
                    break;
                }
        ...
        //    ISP_SetPreScaleMatrix_Reg(pISPctxt, &pISPctxt->PreScaleFun);
        //}
    }
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_ScaleParam)
    {
        ...
        ISP_SetScaleParam_Reg(pISPctxt, &pISPctxt->ScaleFun);
    }
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_ScaleMatrixH)
    {
        ...
        //{
            for (index = 0; index < WEIGHT_NUM; index++)
            {
                ...
                {
                    ISP_SetIntScaleMatrixH_Reg(pISPctxt, pISPctxt->WeightMatInt[index]);
                    break;
                }
                else if (index == WEIGHT_NUM - 1)
                    ISP_SetIntScaleMatrixH_Reg(pISPctxt, pISPctxt->WeightMatInt[index]);
        ...
        //    ISP_SetScaleMatrixH_Reg(pISPctxt, &pISPctxt->ScaleFun);
        //}
    }
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_ScaleMatrixV)
    {
        ...
        //{
            for (index = 0; index < WEIGHT_NUM; index++)
            {
                ...
                {
                    ISP_SetIntScaleMatrixV_Reg(pISPctxt, pISPctxt->WeightMatInt[index]);
                    break;
                }
                else if (index == WEIGHT_NUM - 1)
                    ISP_SetIntScaleMatrixV_Reg(pISPctxt, pISPctxt->WeightMatInt[index]);
        ...
        //    ISP_SetScaleMatrixV_Reg(pISPctxt, &pISPctxt->ScaleFun);
        //}
    }
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_FrmMatrix)
        ISP_SetFrmMatrix_Reg(pISPctxt, &pISPctxt->FrmMatrix);

    //
    // Frame Function 0
    //
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_FrameFun0)
        ISP_SetFrameFun0_Reg(pISPctxt, &pISPctxt->FrameFun0);

    //
    // Frame Function 1
    //
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_FrameFun1)
        ISP_SetFrameFun1_Reg(pISPctxt, &pISPctxt->FrameFun1);

    //
    // YUV to RGB Parameter
    //
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_RGBtoYUVMatrix)
        ISP_SetRGBtoYUVMatrix_Reg(pISPctxt, pISPctxt->RGB2YUVFun);
    ...
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_Clip0Fun)
    {
        ...
        ISP_SetClip0Fun_Reg(pISPctxt, &pISPctxt->ClipFun0);
    }
    ...
    //
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_OutAddress || pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_OutBufInfo)
        ISP_SetOutAddressBufInfo_Reg(pISPctxt, &pISPctxt->OutInfo);

    //
    // Destination Address of Write Buffer
    //
    if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_OutAddress)
    {
        if (pISPctxt->OutInfo.EnableSWFlipMode || pISPctxt->OutInfo.WrFlipNum)
        {
            if (pISPctxt->ispCore == MMP_ISP_CORE_0)
                ISP_SetWBAddress_0_Reg(pISPctxt, &pISPctxt->OutInfo);
            else if (pISPctxt->ispCore == MMP_ISP_CORE_1)
                ISP_SetWBAddress_1_Reg(pISPctxt, &pISPctxt->OutInfo);
        }
    }
    ...
    if (pISPctxt->ispCore == MMP_ISP_CORE_1)
    {
        if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PreviewMode)
            ISP_SetPVPreviewMode_Reg(pISPctxt);

        if (pISPctxt->EnPreview == MMP_TRUE)
        {
            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVScaleParam)
            {
                _ISP_CalPVScaleHCI_VCI(pISPctxt);
                ISP_SetPVScaleParam_Reg(pISPctxt, &pISPctxt->PVScaleFun);
            }

            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVScaleMatrixH)
            {
                //if(pISPctxt->ispMode == MMP_ISP_MODE_PLAY_VIDEO)
                //{
                    for (index = 0; index < WEIGHT_NUM; index++)
                    {
                        if (pISPctxt->PVScaleFun.HCI >= ScaleRatio[index])
                        {
                            ISP_SetIntPVScaleMatrixH_Reg(pISPctxt, pISPctxt->WeightMatInt[index]);
                            break;
                        }
                        else if (index == WEIGHT_NUM - 1)
                            ISP_SetIntPVScaleMatrixH_Reg(pISPctxt, pISPctxt->WeightMatInt[index]);
            ...
            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVScaleMatrixV)
            {
                //if(pISPctxt->ispMode == MMP_ISP_MODE_PLAY_VIDEO)
                //{
                    for (index = 0; index < WEIGHT_NUM; index++)
                    {
                        if (pISPctxt->PVScaleFun.VCI >= ScaleRatio[index])
                        {
                            ISP_SetIntPVScaleMatrixV_Reg(pISPctxt, pISPctxt->WeightMatInt[index]);
                            break;
                        }
                        else if (index == WEIGHT_NUM - 1)
                            ISP_SetIntPVScaleMatrixV_Reg(pISPctxt, pISPctxt->WeightMatInt[index]);
            ...
            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVOutParameter)
                ISP_SetPVOutParameter_Reg(pISPctxt, &pISPctxt->PVOutInfo);

            if (pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVOutAddress || pISPctxt->UpdateFlags & ISP_FLAGS_UPDATE_PVOutBufInfo)
                ISP_SetPVOutAddressBufInfo_Reg(pISPctxt, &pISPctxt->PVOutInfo);
        }
    }
}
