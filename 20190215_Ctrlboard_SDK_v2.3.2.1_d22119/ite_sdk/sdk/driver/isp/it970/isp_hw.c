    ...
    const MMP_ISP_CORE ispCore)
{
    ...
    end = REG_ADDR_TRANSFER(ispCore, ISP_REG_BASE + ISP_CORE_1_BASE);
...
{
    ISP_FireIspQueue(pISPctxt);

    ISP_CMD_QUEUE_WAIT(pISPctxt, 1 * 2 * 4); // 1 register = (addr + value), addr/value = 4 byte

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SET500), (MMP_UINT32)(
            ...
            ((pOutInfo->WrFlipNum             & ISP_BIT_WRFLIPNUM)                << ISP_SHT_WRFLIPNUM) |
            ...
            ((pOutInfo->ClearInterrupt        & ISP_BIT_ISP_CLEAR_INTERRUPT_EN)   << ISP_SHT_ISP_CLEAR_INTERRUPT_EN) |
            ((0x1 & ISP_BIT_DRIVER_FIRE_EN) << ISP_SHT_DRIVER_FIRE_EN)));

    ISP_CMD_QUEUE_FIRE(pISPctxt);
}
    ...
    const ISP_OUTPUT_INFO *pOutInfo)
{
    ...
    ISP_CMD_QUEUE_WAIT(pISPctxt, 1 * 2 * 4);
    ...
    const ISP_OUTPUT_INFO      *pOutInfo)
{
    ...
    ISP_CMD_QUEUE_WAIT(pISPctxt, 2 * 2 * 4);
    ...
    Value = ((pEngineMode->TotalSliceNum        & ISP_BIT_TOTALSLICENUM)        << ISP_SHT_TOTALSLICENUM) |
            ...
            ((pEngineMode->EnableLOWLATENCY     & ISP_BIT_LOWLATENCY_EN)        << ISP_SHT_LOWLATENCY_EN) |
            ((pOutInfo->NVFormat                & ISP_BIT_OUT_NV_FORMAT)        << ISP_SHT_OUT_NV_FORMAT) |
            ((pOutInfo->EnableNVMode            & ISP_BIT_OUT_NV_EN)            << ISP_SHT_OUT_NV_EN) |
            ((pOutInfo->EnableOutputFieldMode   & ISP_BIT_OUTPUT_FIELD_MODE)    << ISP_SHT_OUTPUT_FIELD_MODE) |
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_ENGINEMODE_PARM), Value);
    ...
    Value |= ((pOutInfo->EnableOutYUV235Range & ISP_BIT_OUTYUV235RANGE_EN) << ISP_SHT_OUTYUV235RANGE_EN) |
             ((pOutInfo->EnableInterrupt      & ISP_BIT_ISP_INTERRUPT_EN)  << ISP_SHT_ISP_INTERRUPT_EN) |
             ((0x0                            & ISP_BIT_POST_SUBTITLE_EN)  << ISP_SHT_POST_SUBTITLE_EN);

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_RAWDATA_HANDSHARK), Value);
    ...
    const ISP_CONTEXT *pISPctxt)
{
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PANEL_COLOR_V), (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);
    ...
    const ISP_PV_OUTPUT_INFO *pOutInfo)
{
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVOUTPUT_FORMAT), (MMP_UINT32)Value, ISP_MASK_HIGHBIT);
    ...
    const ISP_INPUT_INFO *pInInfo)
{
    ...
    ISP_CMD_QUEUE_WAIT(pISPctxt, 10 * 2 * 4);

    Value  = (MMP_UINT16)(pInInfo->SrcHeight & ISP_BIT_INPUT_HEIGHT);
    Value |= (MMP_UINT16)(pInInfo->SrcWidth  & ISP_BIT_INPUT_WIDTH) << 16;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_INPUT_HEIGHT), Value);
    ...
    if (pISPctxt->OutInfo.EnableFieldScale)
    {
        ...
        ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_INPUT_PITCH_Y), Value);
    }
    else
    {
        Value  = (MMP_UINT16)(pInInfo->PitchY & ISP_BIT_INPUT_PITCH_Y);
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SET_VC1),
            (MMP_UINT16)(pInInfo->SrcPosX & ISP_BIT_PANEL_SRCPOS_X) << 16, ISP_MASK_LOWBIT);

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PANEL_SRCPOS_Y),
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PANEL_SRC_HEIGHT),
            (MMP_UINT16)(pInInfo->PanelHeight & ISP_BIT_PANEL_SRC_HEIGHT) |
            (MMP_UINT16)Value << 16);

    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PANEL_COLOR_V),
    ...
    const ISP_INPUT_INFO *pInInfo)
{
    ...
    ISP_CMD_QUEUE_WAIT(pISPctxt, 12 * 2 * 4);
    ...
    if (pISPctxt->OutInfo.EnableFieldScale == MMP_TRUE)
    {
        ...
        ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_INPUT_ADDR_YL), Value);
        ...
        ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_INPUT_ADDR_YPL), Value);
        ...
        ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_INPUT_ADDR_UL), Value);
        ...
        ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_INPUT_ADDR_VL), Value);
        ...
        if (pISPctxt->DeInterlace.UVRepeatMode == MMP_TRUE)
        {
            ...
            ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_INPUT_ADDR_UPL), Value);

            Value = (pISPctxt->top_field_first)
                    ? (MMP_UINT32)pInInfo->AddrV  - vramBaseAddr + pInInfo->PitchUV
                    : (MMP_UINT32)pInInfo->AddrVp - vramBaseAddr + pInInfo->PitchUV;

            ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_INPUT_ADDR_VPL), Value);
        }
    ...
    const ISP_INPUT_INFO       *pInInfo)
{
    ...
    ISP_CMD_QUEUE_WAIT(pISPctxt, 5 * 2 * 4);

    Value = ((pDeInterlace->EnMotionDetection     & ISP_BIT_OUT_MOTIONDETECTION_EN) << ISP_SHT_OUT_MOTIONDETECTION_EN) |
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SET_DEINTERLACE), Value);

    if (pISPctxt->ispCore == MMP_ISP_CORE_0)
    {
        ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_LOWLEVELEDGE_START_X),
                (MMP_UINT16)pDeInterlace->LowLevelPosX |
                (MMP_UINT16)pDeInterlace->LowLevelPosY << 16);
        ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_LOWLEVELEDGE_WIDTH),
    ...
    if (pISPctxt->ispCore == MMP_ISP_CORE_0)
    {
        if (pDeInterlace->DeinterMode == DEINTER3D)
            ISP_Set3DDeInterlaceParm_Reg(pISPctxt);
        else if (pDeInterlace->DeinterMode == DEINTER2D)
            ISP_Set2DDeInterlaceParam_Reg(pISPctxt);
    }
}
...
{
    MMP_UINT32           Value         = 0;
    const ISP_DEINTERLACE_CTRL *pDeInterlace = &pISPctxt->DeInterlace;

    ISP_CMD_QUEUE_WAIT(pISPctxt, 3 * 2 * 4);
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_UV_PITCH), (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_3D_DEINTER_PARM_2), Value);
    ...
    const ISP_CONTEXT *pISPctxt)
{
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_2D_DEINTER_PARM_1), (MMP_UINT32)Value, ISP_MASK_HIGHBIT);
    ...
    const ISP_SUBTITLE_CTRL *pSubTitle)
{
    ...
    ISP_CMD_QUEUE_WAIT(pISPctxt, 15 * 2 * 4);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_SUBTITLE_0_ADDR_L,
            (MMP_UINT16)(Value & ISP_BIT_SUBTITLE_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_SUBTITLE_ADDR_H) << 16);

    // SrcWidth, SrcHeight
    ISP_WriteRegister(pISPctxt, ISP_REG_SUBTITLE_0_SRCWIDTH,
            (MMP_UINT16)(pSubTitle->SrcWidth & ISP_BIT_SUBTITLE_SRCWIDTH) |
            (MMP_UINT16)(pSubTitle->SrcHeight & ISP_BIT_SUBTITLE_SRCHEIGHT) << 16);

    // HCI, VCI
    ISP_WriteRegister(pISPctxt, ISP_REG_SUBTITLE_0_HCI,
            (MMP_UINT16)(HCI & ISP_BIT_SUBTITLE_HCI) |
            (MMP_UINT16)(VCI & ISP_BIT_SUBTITLE_VCI) << 16);

    // DstWidth, DstHeight, Pitch
    ISP_WriteRegister(pISPctxt, ISP_REG_SUBTITLE_0_DSTWIDTH,
            (MMP_UINT16)(pSubTitle->DstWidth & ISP_BIT_SUBTITLE_DSTWIDTH) |
            (MMP_UINT16)(pSubTitle->DstHeight & ISP_BIT_SUBTITLE_DSTHEIGHT) << 16);

    ISP_WriteRegisterMask(pISPctxt, ISP_REG_SUBTITLE_0_PITCH, (MMP_UINT32)(pSubTitle->Pitch & ISP_BIT_SUBTITLE_PITCH), ISP_MASK_HIGHBIT);

    // Start X/Y
    ISP_WriteRegister(pISPctxt, ISP_REG_SUBTITLE_0_START_X,
            (MMP_UINT16)(pSubTitle->StartX & ISP_BIT_SUBTITLE_START_X) |
            (MMP_UINT16)(pSubTitle->StartY & ISP_BIT_SUBTITLE_START_Y) << 16);

    // ui decompress
    ISP_WriteRegister(pISPctxt, ISP_REG_SUBTITLE_0_DEC_LINEBYTE,
            (MMP_UINT16)(pSubTitle->UiDecLineBytes & ISP_BIT_SUBTITLE_DEC_LINEBYTE) |
            (MMP_UINT16)(pSubTitle->UiDecTotalBytes & ISP_REG_SUBTITLE_DEC_TOTALBYTE_L) << 16);

    ISP_WriteRegisterMask(pISPctxt, ISP_REG_SUBTITLE_0_DEC_TOTALBYTE_H, (MMP_UINT32)((pSubTitle->UiDecTotalBytes >> 16) & ISP_REG_SUBTITLE_DEC_TOTALBYTE_H), ISP_MASK_HIGHBIT);
    ...
    ISP_WriteRegisterMask(pISPctxt, ISP_REG_COL_COR_DELTA_B, (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);
    ...
    const ISP_SUBTITLE_CTRL *pSubTitle)
{
    ...
    Value = (pSubTitle->Enable)
            ? (MMP_UINT32)pSubTitle->Addr - vramBaseAddr // byte align
            : 0;                                         // for Onfly
    ISP_WriteRegister(pISPctxt, ISP_REG_SUBTITLE_1_ADDR_L,
            (MMP_UINT16)(Value & ISP_BIT_SUBTITLE_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_SUBTITLE_ADDR_H) << 16);

    // SrcWidth, SrcHeight
    ISP_WriteRegister(pISPctxt, ISP_REG_SUBTITLE_1_SRCWIDTH,
            (MMP_UINT16)(pSubTitle->SrcWidth & ISP_BIT_SUBTITLE_SRCWIDTH) |
            (MMP_UINT16)(pSubTitle->SrcHeight & ISP_BIT_SUBTITLE_SRCHEIGHT) << 16);

    // HCI, VCI
    ISP_WriteRegister(pISPctxt, ISP_REG_SUBTITLE_1_HCI,
            (MMP_UINT16)(HCI & ISP_BIT_SUBTITLE_HCI) |
            (MMP_UINT16)(VCI & ISP_BIT_SUBTITLE_VCI) << 16);

    // DstWidth, DstHeight, Pitch
    ISP_WriteRegister(pISPctxt, ISP_REG_SUBTITLE_1_DSTWIDTH,
            (MMP_UINT16)(pSubTitle->DstWidth & ISP_BIT_SUBTITLE_DSTWIDTH) |
            (MMP_UINT16)(pSubTitle->DstHeight & ISP_BIT_SUBTITLE_DSTHEIGHT) << 16);

    ISP_WriteRegisterMask(pISPctxt, ISP_REG_SUBTITLE_1_PITCH, (MMP_UINT32)(pSubTitle->Pitch & ISP_BIT_SUBTITLE_PITCH), ISP_MASK_HIGHBIT);

    // Start X/Y
    ISP_WriteRegister(pISPctxt, ISP_REG_SUBTITLE_1_START_X,
            (MMP_UINT16)(pSubTitle->StartX & ISP_BIT_SUBTITLE_START_X) |
            (MMP_UINT16)(pSubTitle->StartY & ISP_BIT_SUBTITLE_START_Y) << 16);

    // ui decompress
    ISP_WriteRegisterMask(pISPctxt, ISP_REG_SUBTITLE_0_DEC_TOTALBYTE_H, (MMP_UINT16)(pSubTitle->UiDecLineBytes & ISP_BIT_SUBTITLE_DEC_LINEBYTE) << 16, ISP_MASK_LOWBIT);

    ISP_WriteRegister(pISPctxt, ISP_REG_SUBTITLE_1_DEC_TOTALBYTE_L,
            (MMP_UINT16)(pSubTitle->UiDecTotalBytes & ISP_REG_SUBTITLE_DEC_TOTALBYTE_L) |
    ...
    ISP_WriteRegisterMask(pISPctxt, ISP_REG_SUBTITLE_0_PITCH, (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);
    ...
    const ISP_YUV_TO_RGB *pYUVtoRGB)
{
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_UV_INTERPOLATION),
            (MMP_UINT16)(pYUVtoRGB->_11 & ISP_BIT_YUV_TO_RGB) << 16, ISP_MASK_LOWBIT);

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_YUV_TO_RGB_12),
            (MMP_UINT16)(pYUVtoRGB->_12    & ISP_BIT_YUV_TO_RGB) |
            (MMP_UINT16)(pYUVtoRGB->_13    & ISP_BIT_YUV_TO_RGB) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_YUV_TO_RGB_21),
            (MMP_UINT16)(pYUVtoRGB->_21    & ISP_BIT_YUV_TO_RGB) |
            (MMP_UINT16)(pYUVtoRGB->_22    & ISP_BIT_YUV_TO_RGB) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_YUV_TO_RGB_23),
            (MMP_UINT16)(pYUVtoRGB->_23    & ISP_BIT_YUV_TO_RGB) |
            (MMP_UINT16)(pYUVtoRGB->_31    & ISP_BIT_YUV_TO_RGB) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_YUV_TO_RGB_32),
            (MMP_UINT16)(pYUVtoRGB->_32    & ISP_BIT_YUV_TO_RGB) |
            (MMP_UINT16)(pYUVtoRGB->_33    & ISP_BIT_YUV_TO_RGB) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_YUV_TO_RGB_CONST_R),
            (MMP_UINT16)(pYUVtoRGB->ConstR & ISP_BIT_YUV_TO_RGB_CONST) |
            (MMP_UINT16)(pYUVtoRGB->ConstG & ISP_BIT_YUV_TO_RGB_CONST) << 16);

    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_YUV_TO_RGB_CONST_B),
    ...
    const ISP_COLOR_CORRECTION *pColorCorrect)
{
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_COL_COR_12),
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_COL_COR_21),
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_COL_COR_23),
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_COL_COR_32),
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_COL_COR_DELTA_R),
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_COL_COR_DELTA_B),
    ...
    const ISP_PRESCALE_CTRL *pPreScaleFun)
{
    ...
    // HCI
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PRESCALE_HCI_L),
            (MMP_UINT16)(HCI & ISP_BIT_PRESCALE_HCI_L) |
            (MMP_UINT16)((HCI >> 16) & ISP_BIT_PRESCALE_HCI_H) << 16);

    // PreScale Output Width
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PRESCALE_WIDTH),
    ...
    const ISP_PRESCALE_CTRL *pPreScaleFun)
{
    ...
    ISP_WriteRegisterMask(pISPctxt, ISP_REG_PRESCALE_WIDTH, (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_PRESCALE_WX0302, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_PRESCALE_WX1312, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_PRESCALE_WX2322, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_PRESCALE_WX3332, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegisterMask(pISPctxt, ISP_REG_PRESCALE_WX4342, (MMP_UINT32)Value, ISP_MASK_HIGHBIT);
    ...
    MMP_UINT8         WeightMatX[][ISP_SCALE_TAP])
{
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PRESCALE_WIDTH), (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PRESCALE_WX0302), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PRESCALE_WX1312), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PRESCALE_WX2322), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PRESCALE_WX3332), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PRESCALE_WX4342), (MMP_UINT32)Value, ISP_MASK_HIGHBIT);
    ...
    const ISP_SCALE_CTRL *pScaleFun)
{
    ...
    ISP_CMD_QUEUE_WAIT(pISPctxt, 14 * 2 * 4);

    // HCIP
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALE_HCIP_L),
            (MMP_UINT16)(HCIP & ISP_BIT_SCALE_HCIP_L) |
            (MMP_UINT16)((HCIP >> 16) & ISP_BIT_SCALE_HCIP_H) << 16);

    // VCIP and Output pixel counts per line in a block
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALE_VCIP),
            (MMP_UINT16)ISP_SCALE_MAX_BLOCK_HEIGHT |
            (MMP_UINT16)(OPNPLB & ISP_BIT_SCALE_OPNPLB) << 16);

    // HCI
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALE_HCI_L),
            (MMP_UINT16)(HCI & ISP_BIT_SCALE_L) |
            (MMP_UINT16)((HCI >> 16) & ISP_BIT_SCALE_H) << 16);

    // VCI
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALE_VCI_L),
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_BGCOLOR_RG),
            (MMP_UINT16)Value |
            (MMP_UINT16)(pScaleFun->BGColorB & ISP_BIT_BGCOLOR) << 16);

    // Scale Output Position X and Y
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALE_DSTPOS_X),
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALE_DSTWIDTH),
    ...
    const ISP_SCALE_CTRL *pScaleFun)
{
    ...
    // HCI
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALE_HCI_L),
            (MMP_UINT16)(HCI & ISP_BIT_PVSCALE_L) |
            (MMP_UINT16)((HCI >> 16) & ISP_BIT_PVSCALE_H) << 16);

    // VCI
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALE_VCI_L),
            (MMP_UINT16)(VCI & ISP_BIT_PVSCALE_L) |
            (MMP_UINT16)((VCI >> 16) & ISP_BIT_PVSCALE_H) << 16);

    // Scale Output Width and Height
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALE_DSTWIDTH),
    ...
    const ISP_SCALE_CTRL *pScaleFun)
{
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_SCALEWX0100, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_SCALEWX1110, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_SCALEWX2120, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_SCALEWX3130, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_SCALEWX4140, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    MMP_UINT8         WeightMatX[][ISP_SCALE_TAP])
{
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALEWX0100), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALEWX1110), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALEWX2120), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALEWX3130), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALEWX4140), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    const ISP_SCALE_CTRL *pScaleFun)
{
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_SCALEWY0100, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_SCALEWY1110, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_SCALEWY2120, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_SCALEWY3130, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, ISP_REG_SCALEWY4140, (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    MMP_UINT8         WeightMatY[][ISP_SCALE_TAP])
{
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALEWY0100), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALEWY1110), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALEWY2120), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALEWY3130), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SCALEWY4140), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    MMP_UINT8         WeightMatX[][ISP_SCALE_TAP])
{
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_COL_COR_DELTA_B), (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALEWX0302), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALEWX1312), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALEWX2322), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALEWX3332), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALEWX4342), (MMP_UINT32)Value, ISP_MASK_HIGHBIT);
    ...
    MMP_UINT8         WeightMatY[][ISP_SCALE_TAP])
{
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALEWX4342), (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALEWY0302), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALEWY1312), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALEWY2322), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALEWY3332), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALEWY4342), (MMP_UINT32)Value, ISP_MASK_HIGHBIT);
    ...
    const ISP_RGB_TO_YUV *pMatrix)
{
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRM_RGB2YUV_11),
            (MMP_UINT16)(pMatrix->_11    & ISP_BIT_FRM_RGB2YUV) |
            (MMP_UINT16)(pMatrix->_12    & ISP_BIT_FRM_RGB2YUV) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRM_RGB2YUV_13),
            (MMP_UINT16)(pMatrix->_13    & ISP_BIT_FRM_RGB2YUV) |
            (MMP_UINT16)(pMatrix->_21    & ISP_BIT_FRM_RGB2YUV) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRM_RGB2YUV_22),
            (MMP_UINT16)(pMatrix->_22    & ISP_BIT_FRM_RGB2YUV) |
            (MMP_UINT16)(pMatrix->_23    & ISP_BIT_FRM_RGB2YUV) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRM_RGB2YUV_31),
            (MMP_UINT16)(pMatrix->_31    & ISP_BIT_FRM_RGB2YUV) |
            (MMP_UINT16)(pMatrix->_32    & ISP_BIT_FRM_RGB2YUV) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRM_RGB2YUV_33),
            (MMP_UINT16)(pMatrix->_33    & ISP_BIT_FRM_RGB2YUV) |
            (MMP_UINT16)(pMatrix->ConstY & ISP_BIT_FRM_RGB2YUV_CONST) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRM_RGB2YUV_CONST_U),
    ...
    const ISP_FRMFUN_CTRL *pFrameFun)
{
    ...
    ISP_CMD_QUEUE_WAIT(pISPctxt, 18 * 2 * 4);

    // Starting address
    Value = (pFrameFun->Enable)
            ? (MMP_UINT32)pFrameFun->Addr - vramBaseAddr  // byte align
            : 0;                                          // for Onfly
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_0_ADDR_L),
            (MMP_UINT16)(Value & ISP_BIT_FRMFUN_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_FRMFUN_ADDR_H) << 16);

    // width, height, pitch
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_0_WIDTH),
            (MMP_UINT16)(pFrameFun->Width  & ISP_BIT_FRMFUN_WIDTH) |
            (MMP_UINT16)(pFrameFun->Height & ISP_BIT_FRMFUN_HEIGHT) << 16);

    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_0_PITCH),
            (MMP_UINT32)(pFrameFun->Pitch & ISP_BIT_FRMFUN_PITCH), ISP_MASK_HIGHBIT);

    // start X/Y
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_0_START_X),
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_0_KEY_RG),
            (MMP_UINT16)Value |
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SET_FRMFUN_0),
            (MMP_UINT16)Value |
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_CONST_DATA_R0G0_0),
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_CONST_DATA_G1B1_0), (MMP_UINT32)Value, ISP_MASK_HIGHBIT); //14

    // ui decompress
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_0_DEC_LINEBYTE),
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_0_DEC_TOTALBYTE_H),
            (MMP_UINT32)((pFrameFun->UiDecTotalBytes >> 16) & ISP_REG_FRMFUN_DEC_TOTALBYTE_H), ISP_MASK_HIGHBIT);

    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_UPDATE_PARA_1), (MMP_UINT32)pFrameFun->UIBufferIndex, ISP_MASK_HIGHBIT);
    ...
    const ISP_FRMFUN_CTRL *pFrameFun)
{
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_1_ADDR_H),
            (MMP_UINT16)((Value >> 16) & ISP_BIT_FRMFUN_ADDR_H) |
            (MMP_UINT16)(pFrameFun->StartX & ISP_BIT_FRMFUN_START_X) << 16);

    // height, pitch
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_1_HEIGHT),
            (MMP_UINT16)(pFrameFun->Height & ISP_BIT_FRMFUN_HEIGHT) |
            (MMP_UINT16)(pFrameFun->Pitch  & ISP_BIT_FRMFUN_PITCH) << 16);

    // start Y and width
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_1_START_Y),
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_1_KEY_B),
            (MMP_UINT16)(pFrameFun->ColorKeyB & ISP_BIT_FRMFUN_KEY) |
            Value2 << 16);

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_CONST_ALPHA_1),
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_0_PITCH), (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);

    // Const Data
    Value = ((pFrameFun->ConstColorR0 & ISP_BIT_CONST_DATA) << ISP_SHT_CONST_DATA_LOW) |
            ((pFrameFun->ConstColorG0 & ISP_BIT_CONST_DATA) << ISP_SHT_CONST_DATA_HI);

    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_CONST_DATA_G1B1_0), (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);
    ...
    Value2 = ((pFrameFun->ConstColorG1 & ISP_BIT_CONST_DATA) << ISP_SHT_CONST_DATA_LOW) |
             ((pFrameFun->ConstColorB1 & ISP_BIT_CONST_DATA) << ISP_SHT_CONST_DATA_HI);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_CONST_DATA_B0R1_1), (MMP_UINT16)Value | (MMP_UINT16)Value2 << 16);

    // ui decompress
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_0_DEC_TOTALBYTE_H),
            (MMP_UINT16)(pFrameFun->UiDecLineBytes & ISP_REG_FRMFUN_DEC_LINEBYTE) << 16, ISP_MASK_LOWBIT);

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_FRMFUN_1_DEC_TOTALBYTE_L),
    ...
    const ISP_RGB_TO_YUV *pRGBtoYUV)
{
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_RGB_TO_YUV_11),
            (MMP_UINT16)(pRGBtoYUV->_11 & ISP_BIT_RGB_TO_YUV) |
            (MMP_UINT16)(pRGBtoYUV->_12 & ISP_BIT_RGB_TO_YUV) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_RGB_TO_YUV_13),
            (MMP_UINT16)(pRGBtoYUV->_13 & ISP_BIT_RGB_TO_YUV) |
            (MMP_UINT16)(pRGBtoYUV->_21 & ISP_BIT_RGB_TO_YUV) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_RGB_TO_YUV_22),
            (MMP_UINT16)(pRGBtoYUV->_22 & ISP_BIT_RGB_TO_YUV) |
            (MMP_UINT16)(pRGBtoYUV->_23 & ISP_BIT_RGB_TO_YUV) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_RGB_TO_YUV_31),
            (MMP_UINT16)(pRGBtoYUV->_31 & ISP_BIT_RGB_TO_YUV) |
            (MMP_UINT16)(pRGBtoYUV->_32 & ISP_BIT_RGB_TO_YUV) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_RGB_TO_YUV_33),
            (MMP_UINT16)(pRGBtoYUV->_33 & ISP_BIT_RGB_TO_YUV) |
            (MMP_UINT16)(pRGBtoYUV->ConstY & ISP_BIT_RGB_TO_YUV_CONST) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_RGB_TO_YUV_CONST_U),
    ...
    const ISP_CLIP_FN_CTRL *pClipFun)
{
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_UPDATE_PARA_3), (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_CLIP_0_LEFT),
            (MMP_UINT16)(pClipFun->ClipLeft & ISP_REG_CLIP_LEFT) |
            (MMP_UINT16)(pClipFun->ClipRight & ISP_REG_CLIP_RIGHT) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_CLIP_0_TOP),
    ...
    const ISP_CLIP_FN_CTRL *pClipFun)
{
    ...
    Value = ((pISPctxt->Mpeg2BotBufferIndex & 0x1f)   << 4) |
            ...
            ((pClipFun->Format & ISP_BIT_CLIP_REGION) << ISP_SHT_CLIP_REGION);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SET_CLIP_1),
            (MMP_UINT16)Value |
            (MMP_UINT16)(pClipFun->ClipLeft & ISP_REG_CLIP_LEFT) << 16);

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_CLIP_1_RIGHT),
            (MMP_UINT16)(pClipFun->ClipRight & ISP_REG_CLIP_RIGHT) |
            (MMP_UINT16)(pClipFun->ClipTop & ISP_REG_CLIP_TOP) << 16);

    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_CLIP_1_BOTTOM),
    ...
    const ISP_CLIP_FN_CTRL *pClipFun)
{
    ...
    Value = ((pISPctxt->Mpeg2TopBufferIndex & 0x1f)   << 4) |
            ((pClipFun->Enable & ISP_BIT_CLIP_EN)     << ISP_SHT_CLIP_EN) |
            ((pClipFun->Format & ISP_BIT_CLIP_REGION) << ISP_SHT_CLIP_REGION);
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_CLIP_1_BOTTOM), (MMP_UINT16)Value << 16, ISP_MASK_LOWBIT);

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_CLIP_2_LEFT),
            (MMP_UINT16)(pClipFun->ClipLeft & ISP_REG_CLIP_LEFT) |
            (MMP_UINT16)(pClipFun->ClipRight & ISP_REG_CLIP_RIGHT) << 16);
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_CLIP_2_TOP),
            (MMP_UINT16)(pClipFun->ClipTop & ISP_REG_CLIP_TOP) |
            (MMP_UINT16)(pClipFun->ClipBottom & ISP_REG_CLIP_BOTTOM) << 16);
    ...
    const ISP_OUTPUT_INFO *pOutInfo)
{
    ...
    ISP_CMD_QUEUE_WAIT(pISPctxt, 6 * 2 * 4);

    // buffer address
    Value = (MMP_UINT32)pOutInfo->Addr0 - vramBaseAddr; // byte align

    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_SUBTITLE_1_PITCH),
    ...
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_ADDR_0H), Value2);

    Value2  = (MMP_UINT32)((Value >> 16) & ISP_BIT_OUT_ADDR_H);
    Value   = (MMP_UINT32)pOutInfo->Addr2 - vramBaseAddr; // byte align
    Value2 |= (MMP_UINT16)(Value & ISP_BIT_OUT_ADDR_L) << 16;

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_ADDR_1H), Value2);

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_ADDR_2H),
            (MMP_UINT16)((Value >> 16) & ISP_BIT_OUT_ADDR_H) |
            (MMP_UINT16)(pOutInfo->Width & ISP_BIT_OUT_WIDTH) << 16);

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_HEIGHT),
            (MMP_UINT16)(pOutInfo->Height & ISP_BIT_OUT_HEIGHT) |
            (MMP_UINT16)(pOutInfo->PitchYRGB & ISP_BIT_OUT_PITCH) << 16);

    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_UV_PITCH),
    ...
    const ISP_OUTPUT_INFO *pOutInfo)
{
    ...
    Value = (MMP_UINT32)pOutInfo->Addr0_1 - vramBaseAddr; // byte align
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_1_ADDR_0L),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr1_1 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_1_ADDR_1L),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr2_1 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_1_ADDR_2L),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr0_2 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_2_ADDR_0L),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr1_2 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_2_ADDR_1L),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr2_2 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_2_ADDR_2L),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr0_3 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_3_ADDR_0L),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr1_3 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_3_ADDR_1L),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr2_3 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_3_ADDR_2L),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr0_4 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_4_ADDR_0L),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr1_4 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_4_ADDR_1L),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);

    Value = (MMP_UINT32)pOutInfo->Addr2_4 - vramBaseAddr;
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_4_ADDR_2L),
            (MMP_UINT32)(Value & ISP_BIT_WB_OUT_ADDR_L), ISP_MASK_HIGHBIT);
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_2D_DEINTER_PARM_1),
            (MMP_UINT16)((Value >> 16) & ISP_BIT_OUT_ADDR_H) << 16, ISP_MASK_LOWBIT);
    ...
    const ISP_OUTPUT_INFO *pOutInfo)
{
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_UV_PITCH),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) << 16, ISP_MASK_LOWBIT);

    Value2  = (MMP_UINT32)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H);
    Value   = (MMP_UINT32)pOutInfo->Addr1_1 - vramBaseAddr;
    Value2 |= (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) << 16;

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_1_ADDR_0H_CORE1), Value2);

    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_1_ADDR_1H_CORE1),
            (MMP_UINT32)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H), ISP_MASK_HIGHBIT);

    Value = (MMP_UINT32)pOutInfo->Addr2_1 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_1_ADDR_2L_CORE1),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr0_2 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_2_ADDR_0L_CORE1),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr1_2 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_2_ADDR_1L_CORE1),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    Value = (MMP_UINT32)pOutInfo->Addr2_2 - vramBaseAddr;
    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_OUT_2_ADDR_2L_CORE1),
            (MMP_UINT16)(Value & ISP_BIT_WB_OUT_ADDR_L) |
            (MMP_UINT16)((Value >> 16) & ISP_BIT_WB_OUT_ADDR_H) << 16);
    ...
    const ISP_PV_OUTPUT_INFO *pOutInfo)
{
    ...
    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVSCALEWY4342),
            (MMP_UINT16)(pOutInfo->Width & ISP_BIT_PVOUT_WIDTH) << 16, ISP_MASK_LOWBIT);

    Value = (MMP_UINT32)pOutInfo->Addr0 - vramBaseAddr;

    ISP_WriteRegister(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVOUT_HEIGHT),
            (MMP_UINT16)(pOutInfo->Height & ISP_BIT_PVOUT_HEIGHT) |
            (MMP_UINT16)(Value & ISP_BIT_PVOUT_ADDR_L) << 16);

    ISP_WriteRegisterMask(pISPctxt, REG_ADDR_TRANSFER(pISPctxt->ispCore, ISP_REG_PVOUT_ADDR_0H),
    ...
    const MMP_UINT16  sliceCount)
{
    ...
    ISP_WriteRegisterMask(pISPctxt, ISP_REG_COL_COR_DELTA_B, (sliceCount & ISP_BIT_LOWLATENCY_SLICE) << 16, ISP_MASK_LOWBIT);
...
{
    isp_EnginClockOn();
    isp_EnginReset(ITH_ISP_CORE0);
    isp_EnginReset(ITH_ISP_CORE1);
}
