...
static int sdhci_thread_irq(void *_host);
...
{
	pr_reg(DRIVER_NAME ": =========== REGISTER DUMP (%s)===========\n",
		mmc_hostname(host->mmc));

	pr_reg(DRIVER_NAME ": Sys addr: 0x%08x | Version:  0x%08x\n",
		sdhci_readl(host, SDHCI_DMA_ADDRESS),
		sdhci_readw(host, SDHCI_HOST_VERSION));
	pr_reg(DRIVER_NAME ": Blk size: 0x%08x | Blk cnt:  0x%08x\n",
		sdhci_readw(host, SDHCI_BLOCK_SIZE),
		sdhci_readw(host, SDHCI_BLOCK_COUNT));
	pr_reg(DRIVER_NAME ": Argument: 0x%08x | Trn mode: 0x%08x\n",
		sdhci_readl(host, SDHCI_ARGUMENT),
		sdhci_readw(host, SDHCI_TRANSFER_MODE));
	pr_reg(DRIVER_NAME ": Present:  0x%08x | Host ctl: 0x%08x\n",
		sdhci_readl(host, SDHCI_PRESENT_STATE),
		sdhci_readb(host, SDHCI_HOST_CONTROL));
	pr_reg(DRIVER_NAME ": Power:    0x%08x | Blk gap:  0x%08x\n",
		sdhci_readb(host, SDHCI_POWER_CONTROL),
		sdhci_readb(host, SDHCI_BLOCK_GAP_CONTROL));
	pr_reg(DRIVER_NAME ": Wake-up:  0x%08x | Clock:    0x%08x\n",
		sdhci_readb(host, SDHCI_WAKE_UP_CONTROL),
		sdhci_readw(host, SDHCI_CLOCK_CONTROL));
	pr_reg(DRIVER_NAME ": Timeout:  0x%08x | Int stat: 0x%08x\n",
		sdhci_readb(host, SDHCI_TIMEOUT_CONTROL),
		sdhci_readl(host, SDHCI_INT_STATUS));
	pr_reg(DRIVER_NAME ": Int enab: 0x%08x | Sig enab: 0x%08x\n",
		sdhci_readl(host, SDHCI_INT_ENABLE),
		sdhci_readl(host, SDHCI_SIGNAL_ENABLE));
	pr_reg(DRIVER_NAME ": AC12 err: 0x%08x | Slot int: 0x%08x\n",
		sdhci_readw(host, SDHCI_ACMD12_ERR),
		sdhci_readw(host, SDHCI_SLOT_INT_STATUS));
	pr_reg(DRIVER_NAME ": Caps:     0x%08x | Caps_1:   0x%08x\n",
		sdhci_readl(host, SDHCI_CAPABILITIES),
		sdhci_readl(host, SDHCI_CAPABILITIES_1));
	pr_reg(DRIVER_NAME ": Cmd:      0x%08x | Max curr: 0x%08x\n",
		sdhci_readw(host, SDHCI_COMMAND),
		sdhci_readl(host, SDHCI_MAX_CURRENT));
	pr_reg(DRIVER_NAME ": Host ctl2: 0x%08x\n",
		sdhci_readw(host, SDHCI_HOST_CONTROL2));
#if 0
	if (host->flags & SDHCI_USE_ADMA) {
		if (host->flags & SDHCI_USE_64_BIT_DMA)
			pr_reg(DRIVER_NAME ": ADMA Err: 0x%08x | ADMA Ptr: 0x%08x%08x\n",
				 readl(host->ioaddr + SDHCI_ADMA_ERROR),
				 readl(host->ioaddr + SDHCI_ADMA_ADDRESS_HI),
				 readl(host->ioaddr + SDHCI_ADMA_ADDRESS));
		else
			pr_reg(DRIVER_NAME ": ADMA Err: 0x%08x | ADMA Ptr: 0x%08x\n",
				 readl(host->ioaddr + SDHCI_ADMA_ERROR),
				 readl(host->ioaddr + SDHCI_ADMA_ADDRESS));
	}
#endif
	pr_reg(DRIVER_NAME ": ===========================================\n");
	pr_reg(DRIVER_NAME ": 0x%02x: 0x%08x | 0x%02x: 0x%08x\n", SDHCI_DMA_ADDRESS,
		sdhci_readl(host, SDHCI_DMA_ADDRESS),
		SDHCI_HOST_VERSION,
		sdhci_readw(host, SDHCI_HOST_VERSION));
	pr_reg(DRIVER_NAME ": 0x%02x: 0x%08x | 0x%02x: 0x%08x\n", SDHCI_BLOCK_SIZE, 
	...
	pr_reg(DRIVER_NAME ": 0x%02x: 0x%08x | 0x%02x: 0x%08x\n", SDHCI_COMMAND, 
		...
		SDHCI_MAX_CURRENT,
		sdhci_readl(host, SDHCI_MAX_CURRENT));
	pr_reg(DRIVER_NAME ": 0x%02x: 0x%08x\n", SDHCI_HOST_CONTROL2,
		sdhci_readw(host, SDHCI_HOST_CONTROL2));
	pr_reg(DRIVER_NAME ": ===========================================\n");
}
...
#define dma_address(data)   (u32)(data->tmp_buf ? data->tmp_buf : data->buf)
				  ...
				  struct mmc_data *data, int cookie)
{
    ...
    #endif
	#if !defined(WIN32) && defined(WR_SD_4BYTE_ALIGN) /* for 4-byte alignment issue */
    if ((int)buf_addr & 0x3) {
        data->tmp_buf = (void*)itpVmemAlloc(total_size);
        if (data->flags & MMC_DATA_WRITE)
            ithDmaMemcpy((uint32_t)data->tmp_buf, buf_addr, total_size);
    }
    #endif

    #if defined(CFG_CPU_WB)
    ithFlushDCacheRange((void*)dma_address(data), total_size);
    #endif
	ithFlushMemBuffer();
...
clock_set:
	...
	printf("mmc%d: clock:%d, actual clock:%d, div=%d \n", host->index, clock, (*actual_clock), real_div);
	return clk;
}
...
static void sdhci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	#if !defined(SD_IRQ_ENABLE) // TODO
    {
		...
		if ((int)timeout <= 0) {
			...
			sdhci_timeout_timer((unsigned long)host);
		}
    }
	#endif
}
...
static void sdhci_tasklet_finish(unsigned long param)
{
	...
	if (host->flags & SDHCI_REQ_USE_DMA) {
        ...
        if (data) {
            #if !defined(WIN32) && defined(WR_SD_4BYTE_ALIGN)/* for 4-byte alignment issue */
            if (data->tmp_buf && (data->flags & MMC_DATA_READ))
                ithDmaMemcpy((uint32_t)data->buf, (uint32_t)data->tmp_buf, data->blksz * data->blocks);
            #endif
...
static void sdhci_cmd_irq(struct sdhci_host *host, u32 intmask, u32 *mask)
{
	...
	if (intmask & (SDHCI_INT_TIMEOUT | SDHCI_INT_CRC |
		...
		pr_err("%s: cmd error, intmask = 0x%08X \n", mmc_hostname(host->mmc), intmask);
		/*
...
static void sdhci_data_irq(struct sdhci_host *host, u32 intmask)
{
	...
	if (host->data->error) {
		pr_err("%s: data error, intmask = 0x%08X \n", mmc_hostname(host->mmc), intmask);
		sdhci_finish_data(host);
	}
	else {
...
static void sdhci_irq(void* arg)
{
	...
	do {
		...
		if (intmask & SDHCI_INT_CARD_INT) {
			...
			wake_up_process(host->thread_irq);
			//if (in_interrupt())
...
static int sdhci_thread_irq(void *_host)
{
	struct sdhci_host *host = (struct sdhci_host *)_host;
	unsigned long flags;
	u32 isr;

	struct sched_param param = { .sched_priority = 4/*1*/ };
	...
	do {
		...
		schedule(host->thread_irq);
	} while (!kthread_should_stop(host->thread_irq));
...
static void sdc_interrupt_enable(struct sdhci_host *host)
{
	...
	#if 1 // Irene Lin
	host->thread_irq =
		kthread_run(sdhci_thread_irq, host,
				"ksdhciirqd/%s", mmc_hostname(host->mmc));
	#endif
...
int sdhci_add_host(struct sdhci_host *host)
{
    ...
    */
    ...
    if (caps[0] & SDHCI_CAN_VDD_330) {
        ocr_avail |= MMC_VDD_32_33 | MMC_VDD_33_34 | MMC_VDD_34_35;
    ...
    */
	...
	if(1)
	{
		u32 reg;// = sdhci_readw(host, 0x100);
		reg = 0x00000101;
		sdhci_writel(host, reg, 0x100);
		printf("delay: 0x%08X \n", sdhci_readl(host, 0x100));
	}
