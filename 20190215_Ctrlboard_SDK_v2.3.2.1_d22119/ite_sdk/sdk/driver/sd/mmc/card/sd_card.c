...
static int sd_check_result(struct mmc_request *mrq)
{
	...
	if (ret) {
		printf("%s: %s(): cmd %d(%X) ", mmc_hostname(mrq->host), __func__, mrq->cmd->opcode, mrq->cmd->error);
		printf("data %d(%X) ", mrq->data->error, mrq->data->error);
		if (mrq->stop)
			printf("stop %d(%X) ", mrq->stop->error, mrq->stop->error);
