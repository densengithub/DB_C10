    ...
    GFXSurfaceSrc *srcSurface)
{
    ...
    if (srcSurface->srcSurface)
    {
        if (srcSurface->srcX || srcSurface->srcY)
        {
            uint8_t bpp = 0;
            uint8_t *lcdAddr = 0;
            int size = srcSurface->srcSurface->pitch * actualSrcHeight;            
            ...
            surf = gfxCreateSurfaceByPointer(actualSrcWidth, actualSrcHeight, srcSurface->srcSurface->pitch, srcSurface->srcSurface->format, (uint8_t*)lcdAddr, size);
    ...
    GFXColorFormat format)
{
    ...
    ithInvalidateDCacheRange((void*)surface->imageData, surface->imageDataLength);
 ...
 */
    if (srcSurface->srcSurface->flags)
    {
    ...
    GFXTransformType transformType)
{
    ...
    //topX = (int32_t)floor(MIN(MIN(x0, x1), MIN(x3, x2)));
    //topY = (int32_t)floor(MIN(MIN(y0, y1), MIN(y3, y2)));
    //botX = (int32_t)ceil(MAX(MAX(x0, x1), MAX(x3, x2)));
    //botY = (int32_t)ceil(MAX(MAX(y0, y1), MAX(y3, y2)));

    //newDestWidth  = (newDestWidth > botX - topX + 1) ? botX - topX + 1 : newDestWidth;
    //newDestheight = (newDestheight > botY - topY + 1) ? botY - topY + 1 : newDestheight;
    ...
    int32_t    lineWidth)
{
    ...
    if (lineWidth == 1 && surface->quality == GFX_QUALITY_FASTER)
    {
    ...
    else
    {
        ...
        if (surface->quality == GFX_QUALITY_BETTER)
            regs->regCR1.data |= GFX_REG_CR1_ALPHABLEND_EN;
