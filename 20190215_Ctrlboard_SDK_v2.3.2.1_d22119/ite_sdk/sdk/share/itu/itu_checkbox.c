...
void ituCheckBoxDraw(ITUWidget* widget, ITUSurface* dest, int x, int y, uint8_t alpha)
{
	...
	ITUButton* btn = &checkbox->btn;
    assert(checkbox);
    assert(dest);

	if ((checkbox->checked && checkbox->checkedSurf) || (widget->active && checkbox->checked && checkbox->focusCheckedSurf) || (ituButtonIsPressed(btn) && (btn->pressSurf)))
    {
		...
		if (ituButtonIsPressed(btn) && (btn->pressSurf))
		{
