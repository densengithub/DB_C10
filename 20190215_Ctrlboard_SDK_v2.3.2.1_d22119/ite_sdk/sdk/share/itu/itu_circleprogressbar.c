...
bool ituCircleProgressBarUpdate(ITUWidget* widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    if (ev == ITU_EVENT_LAYOUT)
    {
        ...
        if (!bar->cacheSurf && bar->cacheCount > 0 && bar->progressIcon && bar->progressIcon->surf && bar->value > 0 && (bar->endAngle != bar->startAngle))
        {
            ...
            {
                int size = bar->progressIcon->surf->width > bar->progressIcon->surf->height ? bar->progressIcon->surf->width : bar->progressIcon->surf->height;
                bar->cacheSurf = ituCreateSurface(size, size, 0, ITU_ARGB8888, NULL, 0);
                if (bar->cacheSurf)
                {
                    int i;
                    ITUColor color = { 0, 0, 0, 0 };
                    ituColorFill(bar->cacheSurf, 0, 0, size, size, &color);
                    ...
                    for (i = 0; i < bar->cacheCount; ++i)
                    {
                        ...
                        ituRotate(bar->cacheSurf, 0, 0, bar->progressIcon->surf, 0, bar->progressIcon->surf->height, angle, 1.0f, 1.0f);
                    #else
                        ituRotate(bar->cacheSurf, 0, bar->progressIcon->surf->height, bar->progressIcon->surf, 0, bar->progressIcon->surf->height, angle, 1.0f, 1.0f);
                    #endif
                    }
                }
            }
        ...
        if (bar->progressIcon && bar->progressIcon->surf && bar->value > 0 && (bar->endAngle != bar->startAngle))
        {
            ...
            if (bar->cacheSurf && bar->value >= bar->cacheCount)
            {
                for (; i + bar->cacheCount <= bar->value; i += bar->cacheCount)
                {
                    ...
                    {
                    #ifndef CFG_WIN32_SIMULATOR
                        ituRotate(dest, destx + bar->progressIcon->widget.rect.x, desty + bar->progressIcon->widget.rect.y, bar->cacheSurf, 0, bar->progressIcon->surf->height, angle, 1.0f, 1.0f);
                    #else
                        ituRotate(dest, destx + bar->progressIcon->widget.rect.x, desty + bar->progressIcon->widget.rect.y + bar->progressIcon->surf->height, bar->cacheSurf, 0, bar->progressIcon->surf->height, angle, 1.0f, 1.0f);
                    #endif
                    }
                    else
                    {
                        ...
                        if (surf)
                        {
                            ...
                            ituRotate(surf, bar->progressIcon->widget.rect.x, bar->progressIcon->widget.rect.y + bar->progressIcon->surf->height, bar->cacheSurf, 0, bar->progressIcon->surf->height, angle, 1.0f, 1.0f);
                        ...
                        ituTransform(
                            ...
                            bar->cacheSurf, 0, 0, bar->progressIcon->surf->width, bar->progressIcon->surf->height,
