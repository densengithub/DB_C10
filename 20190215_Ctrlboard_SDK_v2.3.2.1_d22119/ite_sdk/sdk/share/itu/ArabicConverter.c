...
static CharRep charsMap[MAP_LENGTH] =
{
...
static CombCharRep combCharsMap[COMB_MAP_LENGTH] =
{
...
static unsigned int transChars[TRANS_CHARS_LENGTH] =
{
...
wchar_t* convertArabic(wchar_t* normal)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if (CharacterMapContains(current))
        {
            ...
            if ((prevID < 0) || !CharacterMapContains(prev = normal[prevID]) 
    ...
    toReturn = (wchar_t*)calloc(writeCount + 1, sizeof(wchar_t));
    if (!toReturn)
        goto end;

    for(d=0; d < writeCount; d++)
    {
        toReturn[d] = shaped[writeCount - d - 1];
    }
end:
    free(shaped);
    return toReturn; 
