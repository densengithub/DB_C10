        ...
        for (i = 0; i < len; i++)
        {
			...
			if (bitmapGlyph->bitmap.buffer)
			{
				...
				else
				{
					...
					ituDrawGlyph(surf, x + x_advance + sbit->left, y + tfont->face->size->metrics.y_ppem - sbit->top, format, bitmapGlyph->bitmap.buffer, bitmapGlyph->bitmap.width, bitmapGlyph->bitmap.rows);
				}
			}
			else
			{
...
void ituFtGetTextDimension(const char* text, int* width, int* height)
{
    ...
    for (i = 0; i < len; i++)
    {
        ...
        if (tfont->scaler.width < MAX_SBIT_CACHE && tfont->scaler.height < MAX_SBIT_CACHE)
        {
			...
			if (tfont->face->size->metrics.height > (abs(tfont->face->size->metrics.descender) * 4))
				h = (tfont->face->size->metrics.height - (tfont->face->size->metrics.descender / 2)) >> 6;
			else
    ...
    if (len > 0)
    {
        if ((ft.style & ITU_FT_STYLE_BOLD) && ft.bold_size > 0)
        {
			...
			if (bitmapGlyph->bitmap.buffer)
			{
				...
				{
					int yy = y + tfont->face->size->metrics.y_ppem - sbit->top;
                    if (yy < 0)
                    {
                        ITURectangle prevClip;
