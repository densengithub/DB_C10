...
void ituTextDraw(ITUWidget* widget, ITUSurface* dest, int x, int y, uint8_t alpha)
{
	...
	if ((text->fontHeight >= 128) && (text->fontWidth >= 128))
	{
		ituFtResetCache();
	...
	*/
    ...
    else if (text->stringSet)
    {
        ...
        if ((text->textFlags & ITU_TEXT_ARABIC) && (((1u << text->lang) & text->arabicIndices0) || ((1u << text->lang) & text->arabicIndices1)))
            style |= ITU_FT_STYLE_ARABIC;
    }
    else
    {
    ...
    if (desta == 255)
    {
		...
		else if (text->layout == ITU_LAYOUT_TOP_LEFT)
		{
			ituFtGetTextDimension(string, &w, &h);
		}
