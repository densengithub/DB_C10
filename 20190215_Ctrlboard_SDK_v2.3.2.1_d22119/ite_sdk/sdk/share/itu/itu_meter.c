...
//ITUMETER_ANI_FIT_COUNT [2..7] [fast..smooth]
//#define ITUMETER_ANI_FIT_COUNT 5
...
void ituMeterDraw(ITUWidget* widget, ITUSurface* dest, int x, int y, uint8_t alpha)
{
    ...
    {
		int diff = abs(meter->lastvalue - meter->value);
        float angle;
		float last_angle;
		...
		if ((diff > 0) && (diff < 3) && (meter->anifitcount > 1))
		{
			int i = 0;
			float step = (angle - last_angle) / meter->anifitcount;
			float target = last_angle;

			for (i = 1; i < meter->anifitcount; i++)
			{
				target += step;
				ituRotate(dest, x + meter->pointerIcon->widget.rect.x + meter->pointerX, y + meter->pointerIcon->widget.rect.y + meter->pointerY, meter->pointerIcon->surf, meter->pointerX, meter->pointerY, target, 1.0f, 1.0f);
				//printf("[%d][target %.3f last_angle %.3f step %.3f]\n", i, target, last_angle, step);
			}
