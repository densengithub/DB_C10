...
mainProcessorExcuteFileOpen()
{
    ...
    pBuf = (unsigned short *)iteAudioGetAudioCodecAPIBuffer(&nLength);
#endif
...
int mainProcessorExcuteI2SInitAdc()
{
    ...
    spec_ad.sample_rate   = nSampeRate;
    spec_ad.buffer_size   =nBufferLength;

    spec_ad.is_big_endian = 0;
    spec_ad.base_i2s      = I2SBuf;
...
#endif
    printf("[Main Processor Message Queue] 1 i2s_init_ADC 0x%x  channels %d sample rate %d  buffer length %d \n",I2SBuf,nChannels,nSampeRate,nBufferLength);
    return 0;
}
