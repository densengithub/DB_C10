...
#define STC_TIME_BASE 1000000
#else
#define STC_TIME_BASE 90000
#endif
...
typedef struct PlayerProps {
    ...
    bool                isFilePlayer_audio_thread_cread;
...
void video_refresh_timer(PlayerInstance *is, double pts)
{
    ...
    #endif
            ref_clock = (double)stc_time / STC_TIME_BASE;
            diff      = pts - ref_clock;
            if (diff > 0)
                usleep(diff * 1000000);
        }
    ...
    #endif        
    #ifdef CFG_BUILD_ITV
        fc_sync_settime(&is->stc, pts * STC_TIME_BASE);
    #endif
    ...
    {
    #ifdef CFG_BUILD_ITV
        fc_sync_settime(&pprop->stc, (int64_t)pts * STC_TIME_BASE);
    #endif
    ...
    for (;;)
    {
        ...
        else if (ret > 0)
        {
            ...
            if (got_picture)
            {
                ...
                //printf("YC: pts = %f\n", pts);
                if(is->video_speed > 1.5 && is->video_speed <= 2.0)
                {
                    static int count = 0;
                    count++;
                    if(count%4 == 0)
...
static void * audio_thread(void *arg)
{
    ...
    #ifdef CFG_BUILD_ITV
                fc_sync_settime(&is->stc, audio_pts * STC_TIME_BASE);
    #endif
...
static void * video_thread_for_videoloop(void *arg)
{
    int            ret;
    PlayerInstance *is = (PlayerInstance *) arg;
    //AVPacket pkt1, *pkt = &pkt1;
...
static int stream_component_open_for_videoloop(PlayerInstance *is, int stream_index, void *opaque)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        pprop->isFilePlayer_audio_thread_cread = true;
...
static void stream_component_close(PlayerInstance *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
...
#endif
        {
            ...
            packet_queue_end(&is->audioq);
            pprop->isFilePlayer_audio_thread_cread = false;
#ifndef WIN32
            is->audio_tid = 0;
#endif
        }
...
//
///////////////////////////////////////////////////////////////////////////////////////////
static void * video_thread_for_rtsp_client(void *arg)
{
    int            ret;
    PlayerProps    *pProp = (PlayerProps *) arg;
    PlayerInstance *is    = pProp->inst;
...
static void * audio_thread_for_rtsp_client(void *arg)
{
    int            ret;
    PlayerProps    *pProp = (PlayerProps *) arg;
    PlayerInstance *is    = pProp->inst;
    ...
    for (;;)
    {
        ...
        else if (ret > 0)
        {
            ...
            else if (!is->audio_is_sync)
            {
                ...
                {
    #ifdef CFG_BUILD_ITV
                    fc_sync_settime(&is->stc, (audio_pts - audio_threshold) * STC_TIME_BASE);
    #endif
                    is->audio_is_sync = true;
                }
                ...
                if (diff < 0)
                {
                    ...
                    {
        #ifdef CFG_BUILD_ITV
                        fc_sync_settime(&pProp->stc, (audio_pts - audio_threshold) * STC_TIME_BASE);
        #endif
                        is->audio_is_sync = true;
                    }
                    else
...
static int ithMediaPlayer_deinit()
{
    ...
    iteAudioStopEngine(); //stop Audio Engine
...
static int ithMediaPlayer_seekto(int pos)
{
    PlayerProps    *pprop = global_player_prop;
    PlayerInstance *is    = NULL;
    ...
    //#ifdef CFG_BUILD_ITV
    //    fc_sync_settime(&pprop->stc, (int64_t)pos*STC_TIME_BASE);
    //#endif
    pthread_mutex_unlock(&player_mutex);
    return 0;
}

static int ithMediaPlayer_slow_fast_play(float speed)
{
    PlayerProps    *pprop = global_player_prop;
    PlayerInstance *is    = NULL;
...
static bool ithMediaPlayer_check_fileplayer_with_audio()
{
    PlayerProps    *pprop = global_player_prop;
    ...
    return pprop->isFilePlayer_audio_thread_cread;
}
...
ithMediaPlayer fileplayer = {
    ...
    ithMediaPlayer_check_fileplayer_with_audio,
...
ithMediaPlayer fileplayer = {
    ...
    .check_fileplayer_with_audio = ithMediaPlayer_check_fileplayer_with_audio,
