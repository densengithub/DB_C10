...
#ifdef VLD_ERR_NOT_DISPLAY
static MMP_BOOL               gbSkip2IDR = MMP_FALSE;
static MMP_UINT32             gSkipDelayCnt = 0;
#endif
    ...
    AVCodecContext *avctx)
{
    ...
    while (return_state == 0 && gptAVCVideoDecoder->remainByte > 0)
    {
        ...
        else
        {
            ...
            if (nalSize < 4 || !(pPreStartCodeAddr[3] == 0x27 || pPreStartCodeAddr[3] == 0x47 || pPreStartCodeAddr[3] == 0x67 
                                || pPreStartCodeAddr[3] == 0x28 || pPreStartCodeAddr[3] == 0x48 || pPreStartCodeAddr[3] == 0x68 
                                || pPreStartCodeAddr[3] == 0x25 || pPreStartCodeAddr[3] == 0x45 || pPreStartCodeAddr[3] == 0x65 
                                || pPreStartCodeAddr[3] == 0x21 || pPreStartCodeAddr[3] == 0x41 || pPreStartCodeAddr[3] == 0x61 
                                || pPreStartCodeAddr[3] == 0x06 || pPreStartCodeAddr[3] == 0x09))
            {
...
#ifdef VLD_ERR_NOT_DISPLAY
    if (currDisplayFrameBufIndex != 0xFF && !gbSkip2IDR)
#else    
    if (currDisplayFrameBufIndex != 0xFF)
#endif    	
    {
    ...
    AVCodecContext *avctx)
{
    ...
    if (img->type == I_SLICE && gbSkip2IDR && gSkipDelayCnt == 0)
        gSkipDelayCnt = 2;
#endif        
