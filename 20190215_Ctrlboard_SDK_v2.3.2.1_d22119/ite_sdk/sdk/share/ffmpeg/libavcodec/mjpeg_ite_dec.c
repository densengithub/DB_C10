...
#include "../driver/jpg/it970/jpg_reg.h"
	...
	void)
{
	...
	if (ithGetDeviceId() == 0x9850 && ithGetRevisionId() > 0 &&  ithIsTilingModeOn())
	{
		...
		{
			gptJPG_DECODER->OutAddrY[0] = (uint32_t)Jbuf_sys_addr;
    ...
    {
	    case 0:
	        gptJPG_DECODER->DisplayAddrY = (uint8_t*)gptJPG_DECODER->OutAddrY[0];
	        gptJPG_DECODER->DisplayAddrU = (uint8_t*)gptJPG_DECODER->OutAddrU[0];
	        gptJPG_DECODER->DisplayAddrV = (uint8_t*)gptJPG_DECODER->OutAddrV[0];
	        break;

	    case 1:
	        gptJPG_DECODER->DisplayAddrY = (uint8_t*)gptJPG_DECODER->OutAddrY[1];
	        gptJPG_DECODER->DisplayAddrU = (uint8_t*)gptJPG_DECODER->OutAddrU[1];
	        gptJPG_DECODER->DisplayAddrV = (uint8_t*)gptJPG_DECODER->OutAddrV[1];
	        break;
    }
