...
static void nor_release (F_DRIVER *driver)
{
    ...
    pthread_mutex_lock(&NorContext.accessMutex);
    pthread_mutex_unlock(&NorContext.accessMutex);

    if (NorContext.accessMutex)
    {
        pthread_mutex_destroy(&NorContext.accessMutex);
    }
}
...
F_DRIVER *nor_initfunc(unsigned long driver_panor)
{
    ...
    if (pthread_mutex_init(&NorContext.accessMutex, NULL) != 0)
    {
        NorContext.accessMutex = (pthread_mutex_t)NULL;
        return NULL;
    }
