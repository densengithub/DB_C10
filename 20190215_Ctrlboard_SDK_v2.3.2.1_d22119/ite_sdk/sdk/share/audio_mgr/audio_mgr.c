...
void AudioThreadFunc()
{
    ...
    for (;;)
    {
        ...
        {
            usleep(20000);
            if (audioMgr.ptNetwork.nSpecialCase == 1) break; //keysound stuck here
...
stop:
        ...
        if (audioMgr.mode == SMTK_AUDIO_NORMAL)
        {
            ...
            if (smtkAudioMgrGetInterruptStatus() == 1 && smtkAudioMgrGetInterruptOverwriteStatus() == 0)
            {
                ...
                if (i2s_get_DA_running() && mtal_pb_check_fileplayer_with_audio() == 0)
                {
...
int smtkAudioMgrSetCallbackFunction(int (*pApi)(int))
{
