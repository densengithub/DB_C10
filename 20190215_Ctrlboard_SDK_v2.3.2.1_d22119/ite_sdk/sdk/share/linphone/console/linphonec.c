...
#include <errno.h>
#include <signal.h>
#include "coreapi/private.h" /*coreapi/private.h, needed for LINPHONE_VERSION */
#include "coreapi/sal_eXosip2.h"
#endif /*_WIN32_WCE*/
#include <limits.h>
#include <ctype.h>
#include <stdlib.h>
#if 0//!defined(CFG_SENSOR_ENABLE) && defined(ENABLE_VIDEO_MULTICAST)
#include <assert.h>
...
static ortp_thread_t prompt_reader_th;
#if !defined(_WIN32_WCE)
...
static BUF_CALLBACK  groupCallback = NULL;
#endif

bool_t linphonec_camera_enabled=TRUE;

LinphoneCastor3 linphoneCastor3;
...
static void linphonec_call_imcoming_received_notify_callback_default(char* username, char* addr, int id, int video)
{
}
void (*linphonec_call_imcoming_received_notify_callback)(char* username, char* addr, int id, int video) = linphonec_call_imcoming_received_notify_callback_default;

static void linphonec_call_connected_notify_callback_default(char* addr, int id)
{
}
void (*linphonec_call_connected_notify_callback)(char* addr, int id) = linphonec_call_connected_notify_callback_default;

#if 0//!defined(CFG_SENSOR_ENABLE) && defined(ENABLE_VIDEO_MULTICAST)
static char* GetMulticastGroup_from_AddressBook(char* ipaddr)
{
	...
	//printf("++++++++++++++ %s\n", ipaddr);
	abDoc = xmlParseFile(CFG_PRIVATE_DRIVE ":/addressbook.xml");
    assert(abDoc);
	
	xpathCtx = xmlXPathNewContext(abDoc);
	if (!xpathCtx)
		return ret;

	strcpy(buf, "/AddrList//dev[@ip='");
	strcat(buf, ipaddr);
...
static void linphonec_call_state_changed(LinphoneCore *lc, LinphoneCall *call, LinphoneCallState st, const char *msg){
    ...
    long id=(long)linphone_call_get_user_pointer (call);
    char addr[32], *ptr, *saveptr;
    int len;
#if !defined(CFG_SENSOR_ENABLE) && defined(ENABLE_VIDEO_MULTICAST)
	char* buf;
	char *remote_ipaddr=linphone_call_get_remote_address_as_string(call);
#endif
    ...
    switch(st){
        ...
        case LinphoneCallIncomingReceived:			
            ...
            {
                const SalStreamDescription *vstream=sal_media_description_find_stream(call->op->result,
                    SalProtoRtpAvp,SalVideo);
                char *to=linphone_address_as_string(call->log->to);
                ...
                if (to) {
                    ...
                    if(strlen(call->op->result->ipcam_address) != 0)
                    {
                        linphonec_call_imcoming_received_notify_callback(idn, addr, (int)id, 2);
                    }
                    else
                    linphonec_call_imcoming_received_notify_callback(idn, addr, (int)id, vstream ? 1 : 0);
                    ms_free(to);
                } else {
                    linphonec_call_imcoming_received_notify_callback("toto", addr, (int)id, vstream ? 1 : 0);
                }
            }
#if !defined(CFG_SENSOR_ENABLE) && defined(ENABLE_VIDEO_MULTICAST)
			strtok_r(remote_ipaddr, "@", &saveptr);
			buf = groupCallback(strtok_r(NULL, ">", &saveptr));
			if(buf)
			{
        ...
        case LinphoneCallConnected:
            ...
            strncpy(addr, ptr, len);
            addr[len] = '\0';
            linphonec_call_connected_notify_callback(addr, (int)id);
            break;
        ...
        case LinphoneCallError:
            ...
            if (strcmp(msg, "ping_timeout") == 0)
            {
                ptr = strchr(from, '@') + 1;
                len = strchr(from, '>') - ptr;
                strncpy(addr, ptr, len);
                addr[len] = '\0';
                linphonec_call_error_notify_callback(addr, id, 408);
            }
            break;
        default:
            break;
...
static void *prompt_reader_thread(void *arg){
    ...
    for (;;)
    {
        ...
        if (mq_receive(linphoneCastor3.mq, tmp, sizeof(tmp), 0) > 0)
        {
            ms_mutex_lock(&prompt_mutex);
            strcpy(received_prompt,tmp);
            have_prompt=TRUE;
            ms_mutex_unlock(&prompt_mutex);
            if (strcmp(received_prompt, "quit") == 0)
                break;
        }
    }
    return NULL;
...
static void start_prompt_reader(void){
    ...
    ortp_thread_create(&prompt_reader_th,NULL,prompt_reader_thread,NULL);
}
#if !defined(_WIN32_WCE)
...
static void stop_prompt_reader(void){
    ortp_thread_join(prompt_reader_th,NULL);
}
    ...
    if (unix_socket || !BOOL_HAVE_READLINE ){
        ...
        while(1){
            ...
            if (have_prompt){
                ...
                char *ret=Rstrdup(received_prompt, RM_FILE_POS);
#else
...
static void linphonec_call_log_add_callback_default(int type, char* ip, char* time, int duration, int refkey, int init)
{
}
void (*linphonec_call_log_add_callback)(int type, char* ip, char* time, int duration, int refkey, int init) = linphonec_call_log_add_callback_default;

static void linphonec_call_log_updated(struct _LinphoneCore *lc, struct _LinphoneCallLog *newcl)
{
    char buf[32];

    if (newcl->dir == LinphoneCallIncoming)
    {
        if (newcl->status == LinphoneCallMissed || newcl->status == LinphoneCallAborted)
        {
            char* ptr = linphone_address_as_string_uri_only(newcl->from);

            linphonec_call_log_add_callback(1, ptr + 9, newcl->start_date, newcl->duration, linphoneCastor3.calllog_refkey, FALSE);

            free(ptr);

            sprintf(buf, "%d", linphoneCastor3.calllog_refkey);
            linphone_call_log_set_ref_key(newcl, buf);
            linphoneCastor3.calllog_refkey++;
        }
        else if (newcl->status == LinphoneCallSuccess)
        {
            char* ptr = linphone_address_as_string_uri_only(newcl->from);

            linphonec_call_log_add_callback(2, ptr + 9, newcl->start_date, newcl->duration, linphoneCastor3.calllog_refkey, FALSE);
    ...
    else if (newcl->dir == LinphoneCallOutgoing)
    {
        char* ptr = linphone_address_as_string_uri_only(newcl->to);

        if (strncmp(ptr, "sip:camera@", 11) == 0)
        {
            free(ptr);
        }
        else if (strncmp(ptr, "sip:ext@", 8) == 0)
        {
            free(ptr);
        }
        else
        {
            linphonec_call_log_add_callback(3, ptr + 9, newcl->start_date, newcl->duration, linphoneCastor3.calllog_refkey, FALSE);
...
static void linphonec_initialized_notify_callback_default(void)
{
    // DO NOTHING
}
void (*linphonec_initialized_notify_callback)(void) = linphonec_initialized_notify_callback_default;

static void castor3_init_call_logs(LinphoneCore *lc)
{
    MSList *elem;
	MSList *lastelem = lc->call_logs;
    char buf[32];
	int i =0;

    linphoneCastor3.calllog_refkey = 0;
	
	//search the last elem,because show logs need from last elem to first elem.
	for(elem=lc->call_logs; elem !=NULL; elem = elem->next){
		if(elem->next == NULL)
			lastelem = elem;
	}
    //for (elem=lc->call_logs; elem !=NULL; elem = elem->next)
    for (elem = lastelem; elem !=NULL; elem = elem->prev)
    {
        LinphoneCallLog* cl = (LinphoneCallLog*)elem->data;

        char* key = (char*)linphone_call_log_get_ref_key(cl);
        if (key && *key == 'D')
            continue;

        if (cl->dir == LinphoneCallIncoming && (cl->status == LinphoneCallMissed || cl->status == LinphoneCallAborted))
        {
            char* ptr = linphone_address_as_string_uri_only(cl->from);

            linphonec_call_log_add_callback(1, ptr + 9, cl->start_date, cl->duration, linphoneCastor3.calllog_refkey, TRUE);

            free(ptr);
            sprintf(buf, "%d", linphoneCastor3.calllog_refkey);
            linphone_call_log_set_ref_key(cl, buf);
            linphoneCastor3.calllog_refkey++;
        }
        else if (cl->dir == LinphoneCallIncoming && cl->status == LinphoneCallSuccess)
        {
            char* ptr = linphone_address_as_string_uri_only(cl->from);

            linphonec_call_log_add_callback(2, ptr + 9, cl->start_date, cl->duration, linphoneCastor3.calllog_refkey, TRUE);
        ...
        else if (cl->dir == LinphoneCallOutgoing)
        {
            char* ptr = linphone_address_as_string_uri_only(cl->to);
            ...
            if (strncmp(ptr, "sip:ext@", 8) == 0)
            {
                free(ptr);
                continue;
            }

            linphonec_call_log_add_callback(3, ptr + 9, cl->start_date, cl->duration, linphoneCastor3.calllog_refkey, TRUE);
...
linphonec_main (int argc, char *argv[]) {
	...
	/*seems no used, remove dtmf_received by Barry*/
    ...
    linphonec_vtable.call_log_updated=linphonec_call_log_updated;

    if (! linphonec_init(argc, argv) ) exit(EXIT_FAILURE);

    linphoneCastor3.play_level = linphone_core_get_play_level(linphonec);
    linphoneCastor3.rec_level = linphone_core_get_rec_level(linphonec);
    linphoneCastor3.ring_level = linphone_core_get_ring_level(linphonec);
    lpc_cmd_castor3_get_friends(linphonec, NULL);
    castor3_init_call_logs(linphonec);

    linphoneCastor3.initialized = 1;
    linphonec_initialized_notify_callback();

    linphonec_main_loop (linphonec);

    linphonec_finish(EXIT_SUCCESS);

    linphoneCastor3.quit = 1;
...
#elif 1
    strcpy(configfile_name, CFG_PUBLIC_DRIVE ":/linphonerc");
    strcpy(zrtpsecrets, CFG_PUBLIC_DRIVE ":/linphone-zidcache");
#else
...
#if !defined(CFG_SENSOR_ENABLE) && defined(ENABLE_VIDEO_MULTICAST)
void
Group_SetBufCallBack(
   BUF_CALLBACK bufCallback)
{	 	
	 groupCallback = bufCallback;
}
#endif
/****************************************************************************
 *
