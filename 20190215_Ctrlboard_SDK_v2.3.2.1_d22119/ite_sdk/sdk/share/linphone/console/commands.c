...
static int lpc_cmd_castor3_bypass_echo_cancellation(LinphoneCore *lc, char *args);
...
static LPC_COMMAND commands[] = {
    ...
    { "castor3-bypass-echo-cancellation", lpc_cmd_castor3_bypass_echo_cancellation, "", NULL },
...
static int lpc_cmd_conference(LinphoneCore *lc, char *args){
    ...
    if (n == 2){
        ...
        if (strcmp(subcommand,"add-all")==0){
            ...
            return 1;
        }else if (strcmp(subcommand,"terminate")==0){
...
static int lpc_cmd_castor3_play_voicesound(LinphoneCore *lc, char *args)
{
    ...
    if (!args || (call && call->audiostream->soundwrite)){//calling case mix voice
        AudioStream *st;
        if (call && (st=call->audiostream)){
            voice_set_mix_level (st,0.7);//mix level 0~1
            voice_mix_flag(st,file_path);
        }
        return 1;
    }else if(lc->ringstream==NULL && call->audiostream->mixvoice){
        return 1;
    }else{
        if (lc->ringstream && lc->ringstream->mixer){//ringing case mix sound
            ring_set_mix_level(lc->ringstream,1.0);//mix level 0~1
            ring_mix_pcm(lc->ringstream,file_path);
        }else{//play hint sound
            if (lc->ringstream!=NULL){
                ring_stop(lc->ringstream);
                lc->ringstream=NULL;
                lc->dmfs_playing_start_time=0;
...
static int lpc_cmd_castor3_add_friend(LinphoneCore *lc, char *args)
{
	...
	linphone_friend_apply(new_friend,lc);
...
static int lpc_cmd_castor3_bypass_echo_cancellation(LinphoneCore *lc, char *args)
{
    char *opt1,*opt2;
    LinphoneCall* call=linphone_core_get_current_call(lc);
    opt1=strstr(args,"--bypass-echo");
    opt2=strstr(args,"--enable-echo");
