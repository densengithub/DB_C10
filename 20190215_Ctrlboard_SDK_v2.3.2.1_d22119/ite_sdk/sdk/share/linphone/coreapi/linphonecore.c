...
#define LOCAL_RING "rings/ring1.wav"
/* same for remote ring (ringback)*/
#define REMOTE_RING "rings/ringback.wav"
//#endif
...
static void linphonec_new_log_notify_callback_default(time_t start_time)
{
}
void (*linphonec_new_log_notify_callback)(time_t start_time) = linphonec_new_log_notify_callback_default;
...
LinphoneCallLog * linphone_call_log_new(LinphoneCall *call, LinphoneAddress *from, LinphoneAddress *to){
    ...
    linphonec_new_log_notify_callback(call->start_time);
    cl->from=from;
    cl->to=to;
...
/*     gain=lp_config_get_float(lc->config,SOUND,"playback_gain_db",0);
    //gain = (float)CFG_SPEAKER_GAIN;
	linphone_core_set_playback_gain_db (lc,gain);
	(move to linphonecall.c ) */
...
#ifdef CFG_IOT_ENABLE
	lc->sip_conf.ping_with_options=0;
#else
...
static void misc_config_read (LinphoneCore *lc) {
    LpConfig *config=lc->config;
    lc->max_call_logs_original=lp_config_get_int(config,"misc","history_max_size",15);
    ...
    const char *factory_config_path, void * userdata)
{
    ...
    linphone_core_assign_payload_type(lc,&payload_type_opus48000,-1,NULL);
    linphone_core_assign_payload_type(lc,&payload_type_opus32000,-1,NULL);
    linphone_core_assign_payload_type(lc,&payload_type_opus16000,-1,NULL);
#endif
...
//#endif
    ...
    linphone_core_assign_payload_type(lc,&payload_type_h264,102,"profile-level-id=428014;packetization-mode=1");
#endif
    ...
    linphone_core_assign_payload_type(lc, &payload_type_ts_cam, -1, NULL);
#endif
...
void linphone_core_get_local_ip(LinphoneCore *lc, const char *dest, char *result){
    ...
    if (linphone_core_get_local_ip_for(lc->sip_conf.ipv6_enabled ? AF_INET6 : AF_INET,dest,result)==0)
        return;
#ifdef CFG_IOT_ENABLE
	SYSTEM_IP tSystemIp = { 0 };
	IotSystemGetIp(&tSystemIp);
	sprintf(result, "%d.%d.%d.%d", tSystemIp.ip[0], tSystemIp.ip[1], tSystemIp.ip[2], tSystemIp.ip[3]);
	//printf("localsystem ip=%s\n", result);
#else
...
void linphone_core_iterate(LinphoneCore *lc){
    ...
    if (lc->ringstream && lc->ringstream_autorelease && lc->dmfs_playing_start_time!=0
        && (curtime-lc->dmfs_playing_start_time)>0){
        ring_stop(lc->ringstream);
        lc->ringstream=NULL;
        lc->dmfs_playing_start_time=0;
    }
    ...
    while(calls!= NULL){
        ...
        if (call->state==LinphoneCallOutgoingInit && (curtime-call->start_time>=3)){
            ...
            printf("The call ping timeout, abort it.\n");
            sal_call_terminate(call->op);
            linphone_call_stop_media_streams(call);
            linphone_call_set_state(call,LinphoneCallError,"ping_timeout");
...
/**
 * Terminates a call sip only.
 *
 * @ingroup call_control
 * @param lc the LinphoneCore
 * @param the_call the LinphoneCall object representing the call sip to be terminated.
**/
int linphone_core_terminate_call_sip_only(LinphoneCore *lc, LinphoneCall *the_call)
{
    LinphoneCall *call;
    if (the_call == NULL){
...
int linphone_core_terminate_call(LinphoneCore *lc, LinphoneCall *the_call)
{
    ...
    linphone_call_set_state(call,LinphoneCallEnd,"Call terminated");
    lc->isSpecialCaseForTerminate = FALSE;
    return 0;
}
...
void linphone_core_mute_mic(LinphoneCore *lc, bool_t val){
    ...
    if (st!=NULL){
        audio_stream_set_mic_gain(st,(val==TRUE) ? -1 : lp_config_get_float(lc->config,SOUND,"mic_gain",(float)CFG_MIC_GAIN));
        //audio_stream_set_mic_gain(st,(val==TRUE) ? 0 : (float)CFG_MIC_GAIN);
...
void linphone_core_set_play_file(LinphoneCore *lc, const char *file){
    ...
    if (file!=NULL) {
        ...
        if (call && call->audiostream && call->audiostream->ms.sessions.ticker)
...
void linphone_core_set_ipcam_address(LinphoneCore *lc, const char *url){
    if (lc->ipcam_address!=NULL){
        ms_free(lc->ipcam_address);
        lc->ipcam_address=NULL;
    }
    if (url!=NULL) {
        lc->ipcam_address=ms_strdup(url);
    }
}
...
/**
 * Sets a mkv file to be played when putting somebody on hold.
**/
void linphone_core_start_video_memo_play(LinphoneCore *lc, const char *file){
    if (NULL == lc)
        return;
...
#ifdef ENABLE_GENERAL_PLAYER
        mtal_pb_init(linphonec_video_memo_playback_event_callback);
        strcpy(mtal_spec.srcname, file);
        mtal_spec.vol_level = linphone_core_get_play_level(lc);
        mtal_pb_select_file(&mtal_spec);
        mtal_pb_play();
#endif
        lc->isVideoMemoPlaying = true;
    }
}

void linphone_core_stop_video_memo_play(LinphoneCore *lc){
    if (NULL == lc)
        return;
...
static void linphonec_call_stop_video_play_callback_default(void)
{
}

void (*linphonec_call_stop_video_play_callback)(void) = linphonec_call_stop_video_play_callback_default;

void linphone_core_stop_media_video_play(LinphoneCore *lc){
    if (NULL == lc)
        return;
    ...
    if(lc->isMediaVideoPlaying)
    {
        ...
        mtal_pb_exit();
#endif
        linphonec_call_stop_video_play_callback();
        lc->isMediaVideoPlaying = false;
    }
}

static void linphonec_call_stop_audio_play_callback_default(void)
{
}
void (*linphonec_call_stop_audio_play_callback)(void) = linphonec_call_stop_audio_play_callback_default;
void linphone_core_stop_media_audio_play(LinphoneCore *lc){
    if (NULL == lc)
        return;

    if(lc->isMediaAudioPlaying)
    {
        linphonec_call_stop_audio_play_callback();
        lc->isMediaAudioPlaying = false;
    }
}

void linphone_core_stop_ipcam_camera_watch(LinphoneCore *lc){
    if (NULL == lc)
        return;
...
/**
 * Sets a mkv file where incoming stream is to be recorded.
**/
static void linphonec_call_camera_connect_error_callback_default(char bError)
{
    return;
}
void (*linphonec_call_camera_connect_error_callback)(char bError) = linphonec_call_camera_connect_error_callback_default;
void linphone_core_start_video_memo_record(LinphoneCore *lc, const char *file){
    LinphoneCall *call=linphone_core_get_current_call(lc);
    if (!call->resultdesc)  // this statement is used to make sure the connection is already constructed.
    {
        linphonec_call_camera_connect_error_callback(true);
        return;
    }
    if (lc->rec_file!=NULL){
        ms_free(lc->rec_file);
    ...
    if (file!=NULL) {
        ...
        if (call && call->videostream)
        {
            video_stream_start_record_mkv(call->videostream,file);
        }
    }
}

void linphone_core_stop_video_memo_record(LinphoneCore *lc)
{
    LinphoneCall *call=linphone_core_get_current_call(lc);
    if (lc->rec_file && call && call->videostream)
    {
        video_stream_stop_record_mkv(call->videostream);
    }
}

void linphone_core_start_audio_memo_record(LinphoneCore *lc, const char *file){
    LinphoneCall *call=linphone_core_get_current_call(lc);
    if (!call->resultdesc)  // this statement is used to make sure the connection is already constructed.
    {
        linphonec_call_camera_connect_error_callback(true);
    ...
    if (file!=NULL) {
        ...
        if (call && call->audiostream)
        {
            audio_stream_start_record_wav(call->audiostream,file);
        }
    }
}

void linphone_core_stop_audio_memo_record(LinphoneCore *lc)
{
    LinphoneCall *call=linphone_core_get_current_call(lc);
    if (lc->rec_file && call && call->audiostream)
    {
...
void linphone_core_init_default_params(LinphoneCore*lc, LinphoneCallParams *params) {
    ...
    params->video_from_ipcam=FALSE;
    params->play_video_msg=FALSE;
    params->audio_recive_graph=TRUE;
}
