...
#define CALL_LOG_SIZE_DEFAULT 200
...
static void engeneer_audio_set(AudioStream *st,LinphoneCore *lc)
{    
    int tmp;
    printf("[DAC]   play_lev    = %d\n",tmp=lp_config_get_int(lc->config,SOUND,"play_lev",50));
    linphone_core_set_play_level(lc,tmp);
    printf("[DAC]   rec_lev     = %d\n",tmp=lp_config_get_int(lc->config,SOUND,"rec_lev",50));
    linphone_core_set_rec_level(lc,tmp);
    printf("[FILTER]use_ec      = %d\n",lp_config_get_int(lc->config,SOUND,"echocancellation",1));
    printf("[FILTER]use_volsend = %d\n",lp_config_get_int(lc->config,SOUND,"use_volsend",1));
    printf("        mic_gain    = %f\n",lp_config_get_float(lc->config,SOUND,"mic_gain",(float)CFG_MIC_GAIN));
    printf("        use_ng      = %d\n",lp_config_get_int(lc->config,SOUND,"noisegate",1));
    printf("        ng_coef     = %f\n",lp_config_get_float(lc->config,SOUND,"ngcoef",(float)CFG_NOISE_GATE_THRESHOLD));
    printf("[FILTER]use_volrecv = %d\n",lp_config_get_int(lc->config,SOUND,"use_volrecv",1));
    printf("        SPK_gain    = %f\n",lp_config_get_float(lc->config,SOUND,"playback_gain_db",(float)CFG_SPEAKER_GAIN));
    printf("[FILTER]eq_SPK      = %d\n",lp_config_get_int(lc->config,SOUND,"eq_SPK",1));
    printf("        eq_SPK      = %s\n",lp_config_get_string(lc->config,SOUND,"eq_SPKgains",CFG_EQUALIZER_SPKGAIN_SET));
    printf("[FILTER]eq_MIC      = %d\n",lp_config_get_int(lc->config,SOUND,"eq_MIC",1));
    printf("        eq_MIC      = %s\n",lp_config_get_string(lc->config,SOUND,"eq_MICgains",CFG_EQUALIZER_MICGAIN_SET));
    printf("[FILTER]use_mix     = %d\n",lp_config_get_int(lc->config,SOUND,"use_mix",1));        
}
#endif

void _pre_configure_audio_stream(AudioStream *st,LinphoneCore *lc){
    
#if ENABLE_AUDIO_ENGENEER_MODEL
    char buf[128];
    sprintf(buf, "%c:/linphonerc",Get_Storage_path());
    lp_config_read_file(lc->config,buf);
    engeneer_audio_set(st,lc);        
#endif         
#ifdef ENABLE_AUDIO_NOISE_GATE
    st->use_ng=lp_config_get_int(lc->config,SOUND,"noisegate",1);
#endif
#ifdef ENABLE_AUDIO_SPK_EQUALIZER
    st->eq_SPK=lp_config_get_int(lc->config,SOUND,"eq_SPK",1);
#endif
#ifdef ENABLE_AUDIO_MIC_EQUALIZER
    st->eq_MIC=lp_config_get_int(lc->config,SOUND,"eq_MIC",1);
#endif
#ifdef ENABLE_AEC_ENABLE
    #ifdef ENABLE_AEC_WITH_FILTER
    st->use_ec=lp_config_get_int(lc->config,SOUND,"echocancellation",1);
    st->ecdelay=lp_config_get_int(lc->config,SOUND,"ecdelay",CFG_AEC_DELAY_MS);
    #else
    st->use_ec=lp_config_get_int(lc->config,SOUND,"echocancellation",2);
    #endif
#endif
#ifdef ENABLE_SEND_FILTER
    st->use_volsend=lp_config_get_int(lc->config,SOUND,"use_volsend",1);
#endif
#ifdef ENABLE_RECV_FILTER        
    st->use_volrecv=lp_config_get_int(lc->config,SOUND,"use_volrecv",1);
#endif        
    st->use_mix=lp_config_get_int(lc->config,SOUND,"use_mix",1);       
}
 ...
 */    
    _pre_configure_audio_stream(audiostream,lc);
...
void _post_configure_audio_stream(AudioStream *st, LinphoneCore *lc, bool_t muted){
    ...
    if (st->volsend){      
        ...
        int dc_removal = -1;
        int sustain = -1;
        ms_filter_call_method(st->volsend,MS_FILTER_SET_SAMPLE_RATE,&st->audiorate);
    ...
    if (st->volrecv){
        ...
        ms_filter_call_method(st->volrecv,MS_FILTER_SET_SAMPLE_RATE,&st->audiorate);
    ...
    if (st->equalizerSPK){      
        ...
        ms_filter_call_method(st->equalizerSPK,MS_FILTER_SET_SAMPLE_RATE,&st->audiorate);
    ...
    if (st->equalizerMIC){
        ...
        ms_filter_call_method(st->equalizerMIC,MS_FILTER_SET_SAMPLE_RATE,&st->audiorate);
...
static void linphone_call_set_audio_stream_graph(AudioStream *stream,LinphoneAudioStreamFlow flow,char *filepath){
...
static void linphone_call_start_audio_stream(LinphoneCall *call, const char *cname, bool_t muted, bool_t send_ringbacktone, bool_t use_arc){
    ...
    AudioStream *astream = call->audiostream;

    /* look for savp stream first */
    const SalStreamDescription *stream=sal_media_description_find_stream(call->resultdesc,SalProtoRtpSavp,SalAudio);
    /* no savp audio stream, use avp */
    if (!stream) stream=sal_media_description_find_stream(call->resultdesc, SalProtoRtpAvp,SalAudio);

    if (stream && stream->dir!=SalStreamInactive && stream->port!=0){
        MSSndCard *playcard=lc->sound_conf.lsd_card ? lc->sound_conf.lsd_card : lc->sound_conf.play_sndcard;
        MSSndCard *captcard=lc->sound_conf.capt_sndcard;
        
        call->audio_profile=make_profile(call,call->resultdesc,stream,&used_pt);

        if (used_pt!=-1){
            if (playcard==NULL) ms_warning("No card defined for playback !");
            if (captcard==NULL) ms_warning("No card defined for capture !");
            ...
            audio_stream_enable_adaptive_bitrate_control(astream,use_arc);

            if (playcard) ms_snd_card_set_level(playcard,MS_SND_CARD_PLAYBACK,lc->sound_conf.play_lev);
            if (captcard) 
			{
				astream->send_graph = TRUE;
				ms_snd_card_set_level(captcard,MS_SND_CARD_CAPTURE,lc->sound_conf.rec_lev);            
            }
			else if (call->params.in_conference)
				astream->send_graph = TRUE;
			else
				astream->send_graph = FALSE;
			
            if(lc->presence_mode == LinphoneStatusDoNotDisturb)
            {
                //captcard = NULL;//we do not need AEC & captcard when message record
                linphone_call_set_audio_stream_graph(astream,AudioFromRtpRecv,NULL);
				...
				if (call->state==LinphoneCallPausing){
                	linphone_call_set_audio_stream_graph(astream,AudioBroadcastSend,CFG_PRIVATE_DRIVE ":/rings/ringback.wav");
				}else
					astream->receive_graph = TRUE;
                audio_stream_start_full(
                    astream,
                    call->audio_profile,
                    stream->addr[0]!='\0' ? stream->addr : call->resultdesc->addr,
            ...
            if (muted && !send_ringbacktone){
                audio_stream_set_mic_gain(astream,-1);
            }
            if (stream->dir==SalStreamSendOnly && playfile!=NULL){
                int pause_time=500;
                ms_filter_call_method(astream->soundread,MS_FILE_PLAYER_LOOP,&pause_time);
            }
            if (send_ringbacktone){
                setup_ring_player(lc,call);
            }
            audio_stream_set_rtcp_information(astream, cname, LINPHONE_RTCP_SDES_TOOL);
    ...
    if (vstream!=NULL && vstream->dir!=SalStreamInactive && vstream->port!=0) {
        ...
        if (used_pt!=-1){
            ...
            if (!is_inactive){
                ...
                if(call->params.call_only_ring && call->videostream->source != NULL)
...
void linphone_call_start_media_streams(LinphoneCall *call, bool_t all_inputs_muted, bool_t send_ringbacktone){
    ...
    {
#if defined(CFG_SENSOR_ENABLE)		
        if(video_start && !call->params.call_mobile && call->videostream->source != NULL)
        {
...
static void update_max_call_logs(LinphoneCore *lc){
    uint64_t disk_avail = 0;
    uint64_t disk_avail_min = 0;
    struct statvfs info = {0};

#ifndef CFG_BACKUP_ENABLE
    memset(&info, 0x0, sizeof(struct statvfs));
    if (statvfs(CFG_PUBLIC_DRIVE ":/", &info) == 0){
        disk_avail = info.f_bfree * info.f_bsize;
        printf("free space of %s disk is %llu bytes\n", CFG_PUBLIC_DRIVE, disk_avail);
    }else{
        return;
    }

    lc->max_call_logs = (int)(disk_avail / CALL_LOG_SIZE_DEFAULT) + ms_list_size(lc->call_logs) - 1;
#else
    memset(&info, 0x0, sizeof(struct statvfs));
    if (statvfs(CFG_PUBLIC_DRIVE ":/", &info) == 0){
        disk_avail = info.f_bfree * info.f_bsize;
        printf("free space of %s disk is %llu bytes\n", CFG_PUBLIC_DRIVE, disk_avail);
        disk_avail_min = disk_avail;
    }else{
        return;
    }
    memset(&info, 0x0, sizeof(struct statvfs));
    if (statvfs(CFG_BACKUP_DRIVE ":/", &info) == 0){
        disk_avail = info.f_bfree * info.f_bsize;
        printf("free space of %s disk is %llu bytes\n", CFG_BACKUP_DRIVE, disk_avail);
        disk_avail_min = (disk_avail < disk_avail_min) ? disk_avail : disk_avail_min;
    }else{
        return;
    }

    lc->max_call_logs = (int)(disk_avail_min / CALL_LOG_SIZE_DEFAULT) + ms_list_size(lc->call_logs) - 1;
#endif

    if (lc->max_call_logs_original < lc->max_call_logs)
        lc->max_call_logs = lc->max_call_logs_original;

    if (lc->max_call_logs < 0)
        lc->max_call_logs = 0;

    printf("set the max size of call history for %d\n", lc->max_call_logs);
}
...
void linphone_call_log_completed(LinphoneCall *call){
    ...
    // find free disk space and update max size of call history
    update_max_call_logs(lc);
    // remove redundant call log
    while (ms_list_size(lc->call_logs)>lc->max_call_logs){
