...
static void call_received(SalOp *h){
...
#if !defined(WIN32) && defined(CFG_SENSOR_ENABLE)  
    to_ptr = sal_op_get_to(h);
	strcpy(to_name, strchr(to_ptr, ':') + 1);
	ptr = strchr(to_name, '@');
	if (ptr)
	{
		*ptr = '\0';
	...
	printf("check call type %s(%s)\n", to_ptr, to_name);
#ifdef CFG_DUAL_STREAM 	
	if(!strcmp(to_name, "toto") && (VideoEncoder_GetSreamstate(0) || VideoEncoder_GetSreamstate(1)))
#else
	if(!strcmp(to_name, "toto") && VideoEncoder_GetSreamstate(0))
#endif	
	{
		if(linphonec_call_is_camera_viewing_callback())
...
#if !defined(ENABLE_VIDEO_MULTICAST)	
	else if(!strcmp(to_name, "camera") && VideoEncoder_GetSreamUserNum(0) >= MAX_VIDEO_USER_NUM)
	{
		printf("too much user(%d)!!!\n", VideoEncoder_GetSreamUserNum(0));	
		sal_call_decline(h,SalReasonBusy,NULL);
		sal_op_release(h);
        return;
	}
#endif	
#ifdef CFG_DUAL_STREAM 	
	else if(strcmp(to_name, "toto") && strcmp(to_name, "camera") && VideoEncoder_GetSreamUserNum(1))
	{
		sal_call_decline(h,SalReasonBusy,NULL);
		sal_op_release(h);
        return;
