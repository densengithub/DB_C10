...
void leaf_init_audio_streams (LeafCall *call, unsigned short port) {
    ...
    audiostream->use_ec=iniparser_getint(call->cfgini, "echocancellation", 1);
    audiostream->ecdelay=iniparser_getint(call->cfgini, "ecdelay", CFG_AEC_DELAY_MS);
    #else
    audiostream->use_ec=iniparser_getint(call->cfgini, "echocancellation", 2);
    #endif
#endif
