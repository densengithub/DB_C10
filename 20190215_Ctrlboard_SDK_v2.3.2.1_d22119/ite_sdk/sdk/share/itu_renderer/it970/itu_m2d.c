    ...
    unsigned int flags)
{
    ...
    if (format == ITU_A8)
    {
        if (flags == ITU_STATIC)
        {
            gfxSurface = gfxCreateSurfaceByPointer(
                w,
                h,
                surface->surf.pitch,
                GFX_COLOR_A_8,
        ...
        {
            gfxSurface = gfxCreateSurfaceByBuffer(
                w,
                h,
                surface->surf.pitch,
                GFX_COLOR_A_8,
        ...
        if (!gfxSurface)
        {
            LOG_ERR "gfxCreateSurfaceByBuffer fail!\n"LOG_END
                free(surface);
            return NULL;
        }
    ...
    else
    {
        ...
        if (!gfxSurface)
        {
            LOG_ERR "gfxCreateSurfaceByPointer fail!\n"LOG_END
            free(surface);
            return NULL;
        }
...
static uint8_t* M2dLockSurface(ITUSurface* surf, int x, int y, int w, int h)
{
    ...
    case ITU_MASK_A8:
    case ITU_MONO:
        vram_addr = m2dSurface->m2dSurf->imageData + surf->pitch * y + (x + 7) / 8;
        break;
    case ITU_ARGB8888:
        vram_addr = m2dSurface->m2dSurf->imageData + surf->pitch * y + x * 4;
        break;
    default:
        vram_addr = m2dSurface->m2dSurf->imageData + surf->pitch * y + x * 2;
        break;
    }
...
static void M2dSetRotation(ITURotation rot)
{
    ...
    //    {
    //    case ITU_ROT_90:
    //        ithLcdSetRotMode(ITH_LCD_Vertical, ITH_LCD_ROT180);
    //        break;
    //    case ITU_ROT_180:
    //        ithLcdSetRotMode(ITH_LCD_Horizontal, ITH_LCD_ROT180);
    //        break;
    //    case ITU_ROT_270:
    //        ithLcdSetRotMode(ITH_LCD_Vertical, ITH_LCD_ROT0);
    //        break;
    //    }
    //}
    ...
    if (!gRotateBuffer[0] && m2dRotation != ITU_ROT_0)
    {
        ...
        gRoateSurf = M2dCreateSurface(ithLcdGetWidth(), ithLcdGetHeight(), ithLcdGetPitch(), format, ithLcdGetBaseAddrA(), ITU_STATIC);
    ...
    int sy)
{
    ...
    memcpy(&destSurf->m2dSurf->fgColor, &destSurf->surf.fgColor, sizeof(GFXColor));
