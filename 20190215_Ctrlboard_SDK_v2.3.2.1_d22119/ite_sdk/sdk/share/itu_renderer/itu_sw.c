...
static void SWBitBlt(ITUSurface *dest, int dx, int dy, int w, int h, ITUSurface *src, int sx, int sy)
{
    ...
    if (dest->format == ITU_RGB565)
    {
        ...
        else if (src->format == ITU_A8)
        {
            uint8_t *srcPtr = (uint8_t *)ituLockSurface(src, sx, sy, w, h);
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    sc = srcPtr[src->width * y + x];
                    dc = destPtr[dest->width * y + x];
                    r = ((((dc & 0xf800) >> 11) << 3) * (255 - sc) + dest->fgColor.red * sc) / 255;
                    g = ((((dc & 0x07e0) >> 5) << 2) * (255 - sc) + dest->fgColor.green * sc) / 255;
                    b = (((dc & 0x001f) << 3) * (255 - sc) + dest->fgColor.blue * sc) / 255;
                    destPtr[dest->width * y + x] = ITH_RGB565(r, g, b);
                }
            }
        }
        ...
        else if (src->format == ITU_A8)
        {
            ...
            for (y = 0; y < h; y++)
            {
                ...
                for (x = 0; x < w; x++)
                {
                    ...
                    a = sc;
                    a = a * alpha / 255;
                    r = ((((dc & 0xf800) >> 11) << 3) * (255 - a) + dest->fgColor.red * a) / 255;
                    g = ((((dc & 0x07e0) >> 5) << 2) * (255 - a) + dest->fgColor.green * a) / 255;
                    b = (((dc & 0x001f) << 3) * (255 - a) + dest->fgColor.blue * a) / 255;
                    destPtr[dest->width * y + x] = ITH_RGB565(r, g, b);
                }
            }
            ituUnlockSurface(src);
...
static void SWFlipSync(int* points, int count)
{
    ITUSurface *lcdSurf = ituGetDisplaySurface();
    ITUSurface frontSurf = *lcdSurf;
    int i;

    if (swLcdAddr == ithLcdGetBaseAddrA())
    {
        frontSurf.addr = ithLcdGetBaseAddrB();
    }
    else
    {
        frontSurf.addr = ithLcdGetBaseAddrA();
    }

    for (i = 0; i < count; ++i)
    {
        int* rect = &points[i * 4];
        SWBitBlt(lcdSurf, rect[0], rect[1], rect[2], rect[3], &frontSurf, rect[0], rect[1]);
    }
}
