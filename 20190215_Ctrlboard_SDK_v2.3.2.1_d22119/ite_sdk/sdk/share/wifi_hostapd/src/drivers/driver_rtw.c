...
{	
	...
	os_strlcpy(iwr.ifr_ifrn.ifrn_name, drv->iface, IFNAMSIZ);
					    ...
					    char *data, int len)
{
	...
	while (pos + IW_EV_LCP_LEN <= end) {
        ...
        os_memcpy(&ap_addr, pos + sizeof(struct iw_event),sizeof(struct sockaddr));
		os_memcpy(&addr, pos + sizeof(struct iw_event) + sizeof(struct sockaddr),sizeof(struct sockaddr));
		
		custom = pos +	sizeof(struct iw_event) + sizeof(struct sockaddr) + sizeof(struct sockaddr);
		//printf("+%s, %02x:%02x:%02x:%02x:%02x:%02x rtw\n", __func__, MAC2STR(addr.sa_data));

		//printf("got wireless event, iwe->cmd=%d\n", iwe->cmd);

		switch (iwe->cmd) {
		case IWEVEXPIRED:
			rtl871x_del_sta(drv, (u8 *)addr.sa_data);
			break;
		case IWEVREGISTERED:
			if(rtl871x_new_sta(drv, (u8 *)addr.sa_data))
			{
				printf("Failed to add new sta: "MACSTR" \n", MAC2STR((u8 *)addr.sa_data));
			}
			break;
		case IWEVCUSTOM:
				   ...
				   struct ifinfomsg *ifi, u8 *buf, size_t len)
{
	...
	rtl871x_wireless_event_wireless(drv,buf,len);
...
static int rtl871x_wireless_event_init(struct rtl871x_driver_data *drv)
{
	...
	drv->netlink = netlink_init_hostapd(cfg);
	if (drv->netlink == NULL) {
		os_free(cfg);
		return -1;
	}
		   ...
		   int encrypt, const u8 *own_addr, u32 flags)
{
	...
	status = l2_packet_send_hostapd(drv->l2_sock, addr, ETH_P_EAPOL, bp, len);
...
static int rtl871x_mgnt_sock_init(struct rtl871x_driver_data *drv, const char *name)
{
	...
	os_strlcpy(ifr.ifru.ifr_name, name, sizeof(ifr.ifru.ifr_name));
				  ...
				  int flags)
{
	...
	res = l2_packet_send_hostapd(drv->mgnt_l2_sock, NULL, ETH_P_80211_RAW, msg, len);
#else
...
static void *rtl871x_driver_init_ops(struct hostapd_data *hapd, struct wpa_init_params *params)
{
	...
	drv->ifindex = ifr.ifr_ifru.ifru_index;
	printf("drv->ifindex=%d\n", drv->ifindex);

	drv->l2_sock = l2_packet_init_hostapd(drv->iface, NULL, ETH_P_EAPOL,
					rtl871x_handle_read, drv, 1);

	if (drv->l2_sock == NULL)
		goto bad;

	if (l2_packet_get_own_addr_hostapd(drv->l2_sock, params->own_addr))
		goto bad;
	...
	drv->mgnt_l2_sock = l2_packet_init_hostapd(ifrn_name, NULL, ETH_P_80211_RAW,
...
bad:
	...
	if (drv->l2_sock_recv != NULL && drv->l2_sock_recv != drv->l2_sock)
		l2_packet_deinit_hostapd(drv->l2_sock_recv);
	
	if (drv->l2_sock != NULL)
		l2_packet_deinit_hostapd(drv->l2_sock);
...
#ifdef CONFIG_MGNT_L2SOCK
	if ( drv->mgnt_l2_sock != NULL)
		l2_packet_deinit_hostapd(drv->mgnt_l2_sock);
#else
	if (drv->mgnt_sock >= 0)
		close(drv->mgnt_sock);
#endif
