...
#define TICKER_MEASUREMENTS 0 //1
...
static void call_postprocess(MSFilter *f){
	printf("[debug]%s call_postprocess\n", f->desc->name);
...
static int set_high_prio(MSTicker *obj){
    ...
    if (prio>MS_TICKER_PRIO_NORMAL){
        ...
        env_prio = min_prio + 1; //MAX(MIN(env_prio, max_prio), min_prio);
