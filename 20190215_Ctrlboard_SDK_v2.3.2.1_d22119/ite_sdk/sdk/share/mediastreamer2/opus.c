...
#define MAX_FRAME_SIZE (SAMPLE_RATE*120/1000) //max decode frame 120ms
    ...
    if (ms_concealer_context_is_concealement_required(dt->concealer, obj->ticker->time)) {
        ...
        { // do PLC: PLC doesn't seem to be able to generate more than 960 samples (20 ms at 48000 Hz), get PLC until we have the correct number of sample
			frame_len = 0;
			while (frame_len < dt->lastPacketLength) {
				frame_len += ms_opus_decode(dt->decoder, NULL, 0, (opus_int16 *)(o->b_wptr + (frame_len*sizeof(uint16_t))), dt->lastPacketLength-frame_len, 0);
			}
		}
		if (frame_len < 0) {
