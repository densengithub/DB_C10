...
static void audio_stream_free(AudioStream *stream) {
    ...
    if (stream->separate!=NULL) ms_filter_destroy(stream->separate);
    ms_free(stream);
}
...
/*static void audio_dualmic_separate_graph_link(MSConnectionHelper *h,AudioStream *stream){
    ms_connection_helper_link(h, stream->separate, 0, 0);
    if(stream->ec) ms_filter_link(stream->separate, 1, stream->ec, 2);
}

static void audio_dualmic_separate_graph_unlink(MSConnectionHelper *h,AudioStream *stream){
    ms_connection_helper_unlink(h, stream->separate, 0, 0);
    if(stream->ec) ms_filter_unlink(stream->separate, 1, stream->ec, 2);
}*/
...
int audio_stream_start_full(AudioStream *stream, RtpProfile *profile, const char *rem_rtp_ip,int rem_rtp_port,
    ...
    Castor3snd_reinit_for_diff_rate(sample_rate,16,1);//check if IIS need reinited or not (sampling rate ,bitsize)
    ...
    if (playcard!=NULL && stream->receive_graph) 
    ...
    if(stream->use_volsend){
        ...
        ms_filter_call_method(stream->volsend,MS_FILTER_SET_SAMPLE_RATE,&sample_rate);
    }
    if(stream->use_volrecv){
        stream->volrecv=ms_filter_new(MS_VOLUME_ID);
        ms_filter_call_method(stream->volrecv,MS_FILTER_SET_SAMPLE_RATE,&sample_rate);
    }
    ...
    if(stream->dual_mic)
        stream->separate=ms_filter_new(MS_SEPARATE_ID);
 ...
 */ 
    ...
    ms_ticker_set_priority(stream->ms.sessions.ticker,MS_TICKER_PRIO_HIGH);
#endif
    ms_ticker_set_name(stream->ms.sessions.ticker,"Audio MSTicker");
    
    if(stream->send_graph && stream->soundread)
		ms_ticker_attach(stream->ms.sessions.ticker,stream->soundread);
    if(stream->receive_graph && stream->ms.rtprecv && stream->soundwrite) 
...
//    MSFilterDesc *ec_desc=ms_filter_lookup_by_name("MSOslec");
    ...
    stream->audiorate=CFG_AUDIO_SAMPLING_RATE;
    stream->play_dtmfs=TRUE;
    stream->use_gc=FALSE;
    stream->use_agc=FALSE;
    stream->use_ng=FALSE;
    stream->use_mix=FALSE;
#ifdef ENABLE_DUAL_MIC_ICA_SEPARATION
    stream->dual_mic=TRUE;
#else
    stream->dual_mic=FALSE;
#endif
    ...
    if (stream->ms.sessions.ticker){
        ...
        if(stream->send_graph)
			if(stream->soundread)
				...
				ms_ticker_detach(stream->ms.sessions.ticker,stream->volsend);
        if(stream->ms.rtprecv && stream->soundwrite) ms_ticker_detach(stream->ms.sessions.ticker,stream->ms.rtprecv);
#ifdef PURE_WAV_RECORD
...
MS2_PUBLIC void voice_memo_stop_record (VoiceMemoRecordStream * stream)
{
    ...
    if(stream->ms.encoder) ms_filter_destroy(stream->ms.encoder);
...
MS2_PUBLIC void voice_set_mix_level (AudioStream* stream,double level)
{
    if(stream->mixvoice)
        ms_filter_call_method(stream->mixvoice,MS_MIXVOISE_SET_LEVEL,&level); 
}