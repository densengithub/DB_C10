...
//	float recv_gain=(float)CFG_SPEAKER_GAIN;
    ...
    if (stream->volsend){      
        ...
        int dc_removal = -1;
        int sustain = -1;
        ms_filter_call_method(stream->volsend,MS_FILTER_SET_SAMPLE_RATE,&stream->audiorate);
    ...
    if (stream->volrecv){
        ...
        ms_filter_call_method(stream->volrecv,MS_FILTER_SET_SAMPLE_RATE,&stream->audiorate);
