    ...
    MSFilter *f)
{
    ...
    if (itpGetTickDuration(lastTick) > 1000)
    {
        printf("reset sensor\n");
        //Set GPIO29 to Low
        ithGpioClear(ITH_GPIO_PIN29);
        //Set GPIO29 Output Mode
        ithGpioSetOut(ITH_GPIO_PIN29);
        //Set GPIO29 Mode0
        ithGpioSetMode(ITH_GPIO_PIN29, ITH_GPIO_MODE0);
        usleep(10000);
        ithGpioSet(ITH_GPIO_PIN29);

        ithCapDeviceInitialize(MMP_CAP_DEV_SENSOR);
    ...
    if (gptVideoSample[k][gVideoSendIdx[k]]->bGetStream)
    {
        ...
        if (gVideoSendIdx[k] > gChkIndex)
        	printf("************************************ %d %d\n", gVideoSendIdx[k], gChkIndex); 
        ...
        if (gptVideoSample[k][gVideoSendIdx[k]]->bIFrame)
        {
            ...
            if (m != NULL)
            {
                ...
                if (!ms_queue_empty(&nalus[k]))
                {
					...
					//set timestamp 0, for NT99141 sensor frame rate not stable.
					//rfc3984_pack(pEnc_info->packer, &nalus[k], f->outputs[0], 0);
				}
    #endif
            }
        ...
        if (gptVideoSample[k][gVideoSendIdx[k]]->m_video != NULL)
        {
            ...
            if (gptVideoSample[k][gVideoSendIdx[k]]->bLastSlice)            
                bSendEnd = true;
