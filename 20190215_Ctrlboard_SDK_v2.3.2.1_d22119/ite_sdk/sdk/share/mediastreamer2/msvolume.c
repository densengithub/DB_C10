...
static const float max_e = (32768* (1-CFG_NOISE_GATE_THRESHOLD));/* (1-CFG_NOISE_GATE_THRESHOLD)=0.7 - is RMS factor */
...
static int volume_set_gain(MSFilter *f, void *arg){
    ...
    if(*farg == -1)
        ...
        gain = pow(10,( *farg)/10);
	v->gain = v->target_gain = v->static_gain = gain;
	return 0;
}
...
static int volume_set_max_energy(MSFilter *f, void *arg){
	Volume *v=(Volume*)f->data;
	v->max_energy=32768*(1-(*(float*)arg));
	return 0;
}
...
static MSFilterMethod methods[]={
    ...
    {	MS_VOLUME_SET_MAX_ENERGY, volume_set_max_energy },
	{	0			,	NULL			}
};
