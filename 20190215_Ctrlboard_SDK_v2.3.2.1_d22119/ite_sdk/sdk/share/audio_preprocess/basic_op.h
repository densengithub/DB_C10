...
#include "type_def.h"
#include "./webrtc_lib/include/signal_processing_library.h"

void circshift(Word32 a[], Word32 m, Word32 size);
Word32 float2q15(Float32 p);
float fixed2float(Word32 p, Word32 q);
...
extern Word16 exp_adj(Word16 a, Word16 b);
#endif
...
//asm volatile ("l.maci %0, 1" : :"r"(c));
	...
	asm volatile ("l.mtspr r0, %0, 0x2801" ::"r"(u.lo32));
	asm volatile ("l.mtspr r0, %0, 0x2802" ::"r"(u.hi32));
#elif defined(__arm__)
	MAC_HI_LO = b;
#endif
}
...
static __inline Word64 SMULL(Word32 x, Word32 y) {
	...
	asm volatile("l.mfspr %0, r0, 0x2801" :"=r"(u.lo32));
	asm volatile("l.macrc %0, 32" :"=r"(u.hi32));
	return (u.a);
#else
...
static __inline Word32 lsl(Word32 a, Word32 b) { // left shift with saturations
	if (b > 0) {
		Word32 tmp32no1 = a;
		tmp32no1 = (Word64) tmp32no1 << b;
		return (tmp32no1 >> b == a ? tmp32no1 : a >> 31 ^ 0x7fffffff);
	} else if (b < 0) {
		return ((a + (1LL << (-b - 1))) >> -b);
	} else {
		return (a);
...
static __inline float sign(float a) {
	return (a >= 0 ? 1.f : -1.f);
}
...
static __inline Word32 asr(Word32 a, Word32 b) {
	if (b > 0) {
		return ((a + (1LL << (b - 1))) >> b); //may overflows
	} else if (b < 0) {
		return (lsl(a, -b));
	} else {
		return (a);
...
static __inline Complex16_t complex_asr(Complex16_t a, Word32 b) {
	...
	a.real =
			(b > 0) ?
					sat_32_16(a.real + (1 << (b - 1))) >> b :
					sat_32_16(lsl(a.real, -b));
	a.imag =
			(b > 0) ?
					sat_32_16(a.imag + (1 << (b - 1))) >> b :
...
#else
	UWord32 c, d;
	c = ((UWord32*) a)[0] + ((UWord32*) b)[0]; //never,overflows
