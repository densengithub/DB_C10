...
static void _tpDoPowerOnSeq_vendor(void)
{
    ...
    if( (gTpSpec.tpResetPin>0) && (gTpSpec.tpResetPin<128) )
    {
    	...
    	printf("~~~ DO RESET FLOW: reset pin (%d) ~~~\n",gTpSpec.tpResetPin);
    	usleep(10000);
...
static int _tpProbeSample(struct ts_sample *samp, int nr)
{
	...
	if(gTpInfo.tpIntr4Probe)
	{
        ...
        //printf("prb0::%d,%d,%d\n", g_sample.pressure, g_sample.x, g_sample.y);
	}
 ...
 */ 
static int st1727_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops st1727_ops =
{
	st1727_read,
};

TSAPI struct tslib_module_info *st1727_mod_init(struct tsdev *dev, const char *params)
{
	...
	TSLIB_MODULE_INIT(st1727_mod_init);
#endif
