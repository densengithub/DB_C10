...
/****************************************************************************
 * MACRO define of SIS9255
 ****************************************************************************/
#define TP_I2C_DEVICE_ID       (0x5C)
...
/*
 INT pin:low avtive(without program) type:TP_INT_TYPE_KEEP_STATE
 RESET pin: has reset-pin, but power-on sequency is NOT necessary
 I2C:slave ID is 0x5C
 X/Y:zero orient@Left-Top(TP-I/O PAD is bottom), MAX X/Y: 399 & 1279
*/
/****************************************
 *
 ***************************************/
...
static void _tpInitSpec_vendor(void)
{
    ...
    gTpSpec.tpIntTriggerType    = (char)TP_INT_LEVLE_TRIGGER; //from Kconfig setting   level/edge
...
static int _tpParseRawPxy_vendor(struct ts_sample *s, unsigned char *buf)
{
	...
	if( (buf[2]==0x41) )
	{
		if( (buf[3]==0x03) )
		{
			s->x = (int)((((unsigned int)buf[8]<<8)&0x0F00) | ((unsigned int)buf[7]));  
			s->y = (int)((((unsigned int)buf[6]<<8)&0x0F00) | ((unsigned int)buf[5]));	
			s->pressure = 1;			//?? how to get ft5xxx's presure?
			a=2;
		}
		
		#ifdef	ENABLE_TOUCH_RAW_POINT_MSG
       	printf("	RAW1->[%x]-->%d %d %d\n\n", a, s->pressure, s->x, s->y);
		#endif
 ...
 */ 
static int sis9255_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops sis9255_ops =
{
	sis9255_read,
};

TSAPI struct tslib_module_info *sis9255_mod_init(struct tsdev *dev, const char *params)
{
	...
	TSLIB_MODULE_INIT(sis9255_mod_init);
#endif
