...
static int _tpParseRawPxy_vendor(struct ts_sample *s, unsigned char *buf)
{
	...
	if( (buf[2]==0x41) )
	{
		if( (buf[3]==0x03) )
		{
			s->x = (int)((((unsigned int)buf[6]<<8)&0x0F00) | ((unsigned int)buf[5]));
			s->y = (int)((((unsigned int)buf[8]<<8)&0x0F00) | ((unsigned int)buf[7]));  			
 ...
 */ 
static int sis9252_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops sis9252_ops =
{
	sis9252_read,
};

TSAPI struct tslib_module_info *sis9252_mod_init(struct tsdev *dev, const char *params)
{
	...
	TSLIB_MODULE_INIT(sis9252_mod_init);
#endif
