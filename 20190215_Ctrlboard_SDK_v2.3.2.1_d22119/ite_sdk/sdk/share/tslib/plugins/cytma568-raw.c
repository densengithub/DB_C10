...
#endif 
#else
#define TP_GPIO_RESET_PIN	TP_GPIO_PIN_NO_DEF
#endif 
...
static int _tpDoInitProgram_vendor(void)
{
	...
	//try to read TP controller if cytma568
...
static uint32_t _tpSendKeypadValue(void)
{
	uint32_t var=0;
    pthread_mutex_lock(&gTpMutex);	
    var = gTpKeypadValue;
...
static int _tpDoInitial(void)
{
	...
	#ifdef	CFG_TOUCH_BUTTON
	if(gTpSpec.tpHasTouchKey)	ts_read_kp_callback = _tpSendKeypadValue;
	#endif
