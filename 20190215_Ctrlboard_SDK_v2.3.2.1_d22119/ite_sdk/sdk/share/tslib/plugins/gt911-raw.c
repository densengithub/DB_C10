...
static int _tpReadPointBuffer_vendor(unsigned char *buf, int cnt)
{
    ...
    //printf("	gtp: tmp=%x, x=%02x,%02x, y=%02x,%02x\r\n",lastTmp, buf[0],buf[1],buf[2],buf[3]);
    
    //backup the buffer value for restore
    for(int k=0; k<4; k++)  gLastSmp[k] = buf[k];
...
exit_work_func: 
    ...
    if(gSkipSmp)
    {
        //restore the last value of buffer
        for(int k=0; k<4; k++)  buf[k] = gLastSmp[k];
    }
...
static int _tpParseRawPxy_vendor(struct ts_sample *s, unsigned char *buf)
{
	...
	s->finger = 1;		//it`s just Setting to one point to touch.	
...
static void* _tpProbeHandler(void* arg)
{
    ...
    while(1)
    {
        if(_tpNeedToGetSample())	_tpGetSample(&tpSmp[0], 1);
