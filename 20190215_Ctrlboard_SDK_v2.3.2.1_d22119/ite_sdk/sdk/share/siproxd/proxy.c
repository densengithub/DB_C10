...
extern int urlmap_index[];

extern struct lcl_if_s local_addresses;

//gProxyInIndoorIP
//#define siproxd_in_indoor "192.168.190.61"

#define PROXY_CONTACT "Contact"

//gProxyInternalIndoorIP
//#define PROXY_INTERNAL_INDOOR "192.168.190."

//gProxyOutsideIP
#define PROXY_OUTBOUND  "192.168.191."
static  char gTempIP[16];
static  char gProxyInIndoorIP[16];
static  char gProxyInternalIndoorIP[12];

static  char gProxyOutsideIP[12];

int proxy_set_in_indoor()
{
    ...
    get_interface_real_ip(IF_INBOUND, &map_addr);
    
    memcpy(gProxyInIndoorIP,inet_ntoa(map_addr),sizeof(gProxyInIndoorIP));
    memcpy(gProxyInternalIndoorIP,gProxyInIndoorIP,sizeof(gProxyInternalIndoorIP));
    ipaddr_ntoa_r((const ip_addr_t *)&info.address, gProxyOutsideIP, sizeof(gProxyOutsideIP));

    //printf(" IF_INBOUND %s , %s, outside %s ,%s \n",gProxyInIndoorIP,gProxyInternalIndoorIP,gProxyOutsideIP,gProxyOutsideIP);
    return 0;
    
}

// return 1:internal indoor, 0: outside device
int  proxy_check_invitation_contact(sip_ticket_t *ticket)
{
    char * pch= NULL;
    char * pchIP= NULL;
    char cTemp[16];
   ...
   if (strstr(ticket->raw_buffer,PROXY_CONTACT))
   {
       pch =strstr(ticket->raw_buffer,PROXY_CONTACT);
       pchIP = strstr(pch,"@");
       memcpy(cTemp,pchIP+1,sizeof(cTemp));

       //  printf(" ip %s \n",cTemp);

       //
       pch =strstr(cTemp,gProxyInternalIndoorIP);
       if (pch){
           pch = strrchr(cTemp,'.');
           for (i=0;i<3;i++){
               if ( isdigit(pch[1+i]) ){
           ...
           memset(gTempIP,0,sizeof(gTempIP));
           memcpy(gTempIP,cTemp,(pch+1+i-cTemp));
           return 1;
       } else{
           return 0;
       }
...
int proxy_request (sip_ticket_t *ticket) {
    ...
    int register_user_number;
    int destport;
    int nRTP = 0;
    char *change_sendto_addr;
   ...
   memset(urlmap_index,0,sizeof(int)*URLMAP_INDEX_SIZE);
//gnUdp++;
   register_user_number = sip_find_register_user(ticket);
   for (j=0;j<register_user_number;j++)
   {
       /*
       ...
       sip_find_direction_byIndex(ticket, urlmap_index[j]);
       i = urlmap_index[j];
    	...
    	request->from->url->username? request->from->url->username:"*NULL*",
         ...
         //    sts = proxy_rewrite_invitation_body(ticket, DIR_INCOMING);
          ...
          /* check for incoming request */
             ...
             /* Rewrite the body */
             sts = proxy_rewrite_invitation_body(ticket, DIR_INCOMING,nRTP);
    ...
    /* careful - an internal UA might send an request to another internal UA.
       ...
       caught in the default part of the CASE statement below */
          ...
          if (MSG_IS_INVITE(request)) {
             ...
             sts = proxy_rewrite_invitation_body(ticket, DIR_OUTGOING,nRTP);
          } else if (MSG_IS_ACK(request) || MSG_IS_PRACK(request)) {
             sts = proxy_rewrite_invitation_body(ticket, DIR_OUTGOING,nRTP);
          } else if (MSG_IS_UPDATE(request)) {
             sts = proxy_rewrite_invitation_body(ticket, DIR_OUTGOING,nRTP);
    ...
    &&&*/
		  if ( !strcmp(ticket->sipmsg->sip_method,"BYE") &&
			 !strcmp(request->from->url->host, gProxyInIndoorIP)){
			 change_sendto_addr = urlmap[i].true_url->host;
			 sts = get_ip_by_host(change_sendto_addr, &sendto_addr);
		  }else{
          ...
          destport = atoi(urlmap[i].masq_url->port);
          if (url->port) {
             port=atoi(url->port);
             /*if (port != destport)
             {
                printf("[sip proxy]proxy port %d != %d \n",port,destport);
                port = destport;
             } */
...
//printf("proxy sipsock_send  %s : %d  \n",utils_inet_ntoa(sendto_addr),port);
//printf(" Request to %s \n %s \n",utils_inet_ntoa(sendto_addr),buffer);
...
int proxy_response (sip_ticket_t *ticket) {
   ...
   DEBUGC(DBCLASS_PROXY_RESPONSE,"proxy_response");
   ...
   if (sts == STS_FAILURE) {
      DEBUGC(DBCLASS_PROXY_RESPONSE,"not addressed to my VIA, ignoring response");
      return STS_FAILURE;
   }
 ...
 */
   ...
   */
   case RESTYP_INCOMING:
      DEBUGC(DBCLASS_PROXY_RESPONSE,"incoming response for %s@%s from outbound",
	...
	response->from->url->host? response->from->url->host : "*NULL*");
      ...
      if ((MSG_IS_RESPONSE_FOR(response,"INVITE")) ||
         ...
         if ((MSG_IS_STATUS_100(response)) ||
              (MSG_IS_STATUS_200(response))) {
          // massage status value change to 100 and 200,avoid rtp table error
        // if ((MSG_IS_STATUS_1XX(response)) ||
        //      (MSG_IS_STATUS_2XX(response))) {
                // ring response should not rewrite rtp proxy
            /*if (strcmp(response->reason_phrase,"Ringing")==0){

            }else*/ if (configuration.rtp_proxy_enable == 1) {

               //  check if indoor or internal indoor
               nTemp = proxy_check_invitation_contact(ticket);
               if (nTemp==0){
                   sts = proxy_rewrite_invitation_body(ticket, DIR_INCOMING,0);
               } else if (nTemp == 1){
                    // internal indoor
                   sts = proxy_rewrite_invitation_indoor_body(ticket, DIR_INCOMING);
               }
            }
       ...
       */
{
      ...
      if (ua_hdr && ua_hdr->hvalue &&
         ...
         DEBUGC(DBCLASS_PROXY_RESPONSE, "proxy_response: Grandstream hack 202->404");
   ...
   */
   case RESTYP_OUTGOING:
      DEBUGC(DBCLASS_PROXY_RESPONSE,"outgoing response for %s@%s from inbound",
...
sts=sip_obscure_callid(ticket);
      ...
      if ((MSG_IS_RESPONSE_FOR(response,"INVITE")) ||
         ...
         if ((MSG_IS_STATUS_1XX(response)) ||
            ...
            sts = proxy_rewrite_invitation_body(ticket, DIR_OUTGOING,0);
   ...
   default:
      DEBUGC(DBCLASS_PROXY_RESPONSE, "response from/to unregistered UA (%s@%s)",
	   response->from->url->username? response->from->url->username:"*NULL*",
	   response->from->url->host? response->from->url->host : "*NULL*");
    ...
    */
    if (type == RESTYP_INCOMING) {
       DEBUGC(DBCLASS_PROXY_RESPONSE,"Adding my Record-Route");
       route_add_recordroute(ticket);
    } else {
       /*
        * outgoing packets must not have my record route header, as
        * this likely will contain a private IP address (my inbound).
        */
       DEBUGC(DBCLASS_PROXY_RESPONSE,"Purging Record-Routes (outgoing packet)");
       route_purge_recordroute(ticket);
    }
 ...
 */
   ...
   if ((ticket->protocol == PROTO_TCP) &&
      ...
      DEBUGC(DBCLASS_PROXY_RESPONSE, "proxy_response: have received/rport to %s:%i",
             utils_inet_ntoa(sendto_addr), port);
   /*
    * Route present?
    * If so, fetch address from topmost Route: header and remove it.
    */
      ...
      sts=route_determine_nexthop(ticket, &sendto_addr, &port);
      if (sts == STS_FAILURE) {
         DEBUGC(DBCLASS_PROXY_RESPONSE, "proxy_response: route_determine_nexthop failed");
         return STS_FAILURE;
      }
      DEBUGC(DBCLASS_PROXY_RESPONSE, "proxy_response: have Route header to %s:%i",
             utils_inet_ntoa(sendto_addr), port);
   /*
    ...
    */
       ...
       (sip_find_outbound_proxy(ticket, &sendto_addr, &port) == STS_SUCCESS)) {
      DEBUGC(DBCLASS_PROXY_RESPONSE, "proxy_response: have outbound proxy %s:%i",
             utils_inet_ntoa(sendto_addr), port);
   } else {
      ...
      if (sts == STS_FAILURE) {
         DEBUGC(DBCLASS_PROXY_RESPONSE, "proxy_response: cannot resolve VIA [%s]",
                via->host);
         return STS_FAILURE;
      }
...
//  Outdoor
#define siproxd_inDevice "192.168.190.132"
// Lobby
//#define siproxd_inDevice "192.168.190.253"
 ...
 *	STS_FAILURE on error
 */
int proxy_rewrite_invitation_body(sip_ticket_t *ticket, int direction,int nRTP){
   osip_message_t *mymsg=ticket->sipmsg;
   osip_body_t *body;
   sdp_message_t  *sdp;
   struct in_addr map_addr, addr_sess, addr_media, outside_addr, inside_addr;
   struct in_addr local_addr,remote_addr,ethernet_addr,wifi_addr;
   int sts;
   char *buff;
   size_t buflen;
...
//   if (nRTP==1)
   ...
   sts = sdp_message_parse (sdp, buff);
    ...
    inet_aton(sdp->o_addr,&remote_addr);
    if (!strncmp(sdp->o_addr,siproxd_inDevice, 15))
   {
        get_interface_ip(IF_INBOUND, &local_addr);
    }else{
        get_interface_ip(IF_OUTBOUND, &local_addr);
   }
   get_interface_ip(IF_INBOUND, &ethernet_addr);    
   get_interface_ip(IF_OUTBOUND, &wifi_addr);
   
        
   printf("sdp %s, local %s remote %s e %s w%s  \n",sdp->c_connection->c_addr,inet_ntoa(local_addr),inet_ntoa(remote_addr),inet_ntoa(ethernet_addr),inet_ntoa(wifi_addr));
       

#if 1

   if (!strncmp(sdp->c_connection->c_addr, siproxd_inDevice, 15))
   {
      memcpy(sdp->c_connection->c_addr,inet_ntoa(wifi_addr),15);
   } else if(!strncmp(sdp->c_connection->c_addr, PROXY_OUTBOUND, 12)) {
      formEhternet = 1;
      memcpy(sdp->c_connection->c_addr,gProxyInIndoorIP ,15);
      printf("Set indoor ip %s \n\n",sdp->c_connection->c_addr);

   }

   if (!strncmp(sdp->o_addr, siproxd_inDevice, 15))
   {
      memcpy(sdp->o_addr,inet_ntoa(wifi_addr),15);
   }else if(!strncmp(sdp->c_connection->c_addr, PROXY_OUTBOUND, 12)) {
      memcpy(sdp->o_addr,gProxyInIndoorIP,15);
   }
...
printf("Rewrite !!!! \n \n ");
        if (direction == DIR_INCOMING){
            sprintf(sdp->c_connection->c_addr, "%s", inet_ntoa(wifi_addr));
         } else {
...
printf("Rewrite VVVV  \n \n ");
        ...
        if (direction == DIR_INCOMING){
            sprintf(sdp->o_addr, "%s", inet_ntoa(wifi_addr));
         } else {
            sprintf(sdp->o_addr, "%s", utils_inet_ntoa(map_addr));
        }

      if (formEhternet == 1){
          sprintf(sdp->o_addr, "%s", gProxyInIndoorIP);
          printf("Set indoor ip %s \n\n",sdp->o_addr);
      }
      ...
      if (sdp_conn && sdp_conn->c_addr) {
         if (strcmp(sdp_conn->c_addr, "0.0.0.0") != 0) {
            sts = get_ip_by_host(sdp_conn->c_addr, &addr_media);
             printf("proxy_rewrite_invitation_body: %s %s",sdp_conn->c_addr,utils_inet_ntoa(addr_media));
            have_c_media=1;
...
printf("rtp_start_fwd local %s , remote %s \n",inet_ntoa(map_addr),inet_ntoa(addr_media));
            /*
...
/*
 * proxy_rewrite_invitation_indoor_body
 *
 * rewrites the outgoing INVITATION request or response packet
 *
 * RETURNS
 ...
 *	STS_FAILURE on error
 */
int proxy_rewrite_invitation_indoor_body(sip_ticket_t *ticket, int direction){
    osip_message_t *mymsg=ticket->sipmsg;
    osip_body_t *body;
    sdp_message_t  *sdp;
    struct in_addr map_addr, addr_sess, addr_media, outside_addr, inside_addr;
    ...
    int have_c_media=0;
    int isrtp = 0 ;
printf("proxy_rewrite_invitation_indoor_body  \n\n");
    if (configuration.rtp_proxy_enable == 0) return STS_SUCCESS;
    ...
    */
    ...
    if (sdp->c_connection && sdp->c_connection->c_addr) {
      ...
      if (strcmp(sdp->c_connection->c_addr, "0.0.0.0") != 0) {
        ...
        osip_free(sdp->c_connection->c_addr);
        sdp->c_connection->c_addr=osip_malloc(HOSTNAME_SIZE);
        sprintf(sdp->c_connection->c_addr, "%s", gTempIP);
...
// try to rewrite uri another
   ...
   if (urlmap[idx].masq_url->scheme) {
      DEBUGC(DBCLASS_BABBLE,"proxy_rewrite_request_uri: scheme=%s",
             urlmap[idx].masq_url->scheme);
      scheme = (char *)osip_malloc(strlen(urlmap[idx].masq_url->scheme)+1);
      memcpy(scheme, urlmap[idx].masq_url->scheme,
             strlen(urlmap[idx].masq_url->scheme));
      scheme[strlen(urlmap[idx].masq_url->scheme)]='\0';
      osip_uri_set_scheme(url, scheme);
   }

   /* set the true username */
   if (url->username) {osip_free(url->username);url->username=NULL;}
   if (urlmap[idx].masq_url->username) {
      DEBUGC(DBCLASS_BABBLE,"proxy_rewrite_request_uri: username=%s",
             urlmap[idx].masq_url->username);
      username = (char*)osip_malloc(strlen(urlmap[idx].masq_url->username)+1);
      memcpy(username, urlmap[idx].masq_url->username,
             strlen(urlmap[idx].masq_url->username));
      username[strlen(urlmap[idx].masq_url->username)]='\0';
      osip_uri_set_username(url, username);
   }

   /* set the true host */
   if (url->host) {osip_free(url->host);url->host=NULL;}
   if (urlmap[idx].masq_url->host) {
      DEBUGC(DBCLASS_BABBLE,"proxy_rewrite_request_uri: host=%s",
             urlmap[idx].masq_url->host);
      host = (char *)osip_malloc(strlen(urlmap[idx].masq_url->host)+1);
      memcpy(host, urlmap[idx].masq_url->host, strlen(urlmap[idx].masq_url->host));
      host[strlen(urlmap[idx].masq_url->host)]='\0';
      osip_uri_set_host(url, host);
   }

   /* set the true port */
   if (url->port) {osip_free(url->port);url->port=NULL;}
   if (urlmap[idx].masq_url->port) {
      DEBUGC(DBCLASS_BABBLE,"proxy_rewrite_request_uri: port=%s",
             urlmap[idx].masq_url->port);
      port = (char *)osip_malloc(strlen(urlmap[idx].masq_url->port)+1);
      memcpy(port, urlmap[idx].masq_url->port, strlen(urlmap[idx].masq_url->port));
      port[strlen(urlmap[idx].masq_url->port)]='\0';
